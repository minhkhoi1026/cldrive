...
#else
        if (TOLOWER(text[l]) != pattern[hook(2, l)]) return 0;
#endif
    }
    return 1;
}
...
{  
...
#else
        localPattern[hook(7, idx)] = TOLOWER(pattern[idx]);
#endif
    }
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    if(localIdx == 0) resultCountPerWG[hook(5, groupIdx)] = groupSuccessCounter;
}
