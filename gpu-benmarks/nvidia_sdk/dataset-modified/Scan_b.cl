//{"N":4,"arrayLength":5,"buf":8,"d_Buf":1,"d_Data":0,"d_Dst":1,"d_Src":2,"l_Data":3,"size":3}
...
////////////////////////////////////////////////////////////////////////////////
    ...
    inline uint scan1Inclusive(uint idata, __local uint *l_Data, uint size){
        ...
        for(uint offset = 1; offset < size; offset <<= 1){
            ...
            uint t = l_Data[hook(3, pos)] + l_Data[hook(3, pos - offset)];
            barrier(CLK_LOCAL_MEM_FENCE);
            l_Data[hook(3, pos)] = t;
        }
...
void scanExclusiveLocal1(
    ...
    //Load data
    uint4 idata4 = d_Src[hook(2, get_global_id(0))];

    //Calculate exclusive scan
    uint4 odata4  = scan4Exclusive(idata4, l_Data, size);

    //Write back
    d_Dst[hook(1, get_global_id(0))] = odata4;
}
...
void scanExclusiveLocal2(
    ...
    data =
        d_Dst[hook(1, (4 * 256 - 1) + (4 * 256) * get_global_id(0))] + 
        d_Src[hook(2, (4 * 256 - 1) + (4 * 256) * get_global_id(0))];
    ...
    if(get_global_id(0) < N)
        d_Buf[hook(1, get_global_id(0))] = odata;
}
...
void uniformUpdate(
    ...
    uint4 data4 = d_Data[hook(0, get_global_id(0))];

    if(get_local_id(0) == 0)
        buf[hook(8, 0)] = d_Buf[hook(1, get_group_id(0))];

    barrier(CLK_LOCAL_MEM_FENCE);
    data4 += (uint4)buf[hook(8, 0)];
    d_Data[hook(0, get_global_id(0))] = data4;
}
