...
float4 bestFitLine(__local const float4 * colors, float4 color_sum, __local float* covariance)
{
    ...
    float4 diff = colors[hook(10, idx)] - color_sum * 0.0625f; // * 1.0f / 16.0f

    covariance[hook(11, 6 * idx + 0)] = diff.x * diff.x;    // 0, 6, 12, 2, 8, 14, 4, 10, 0
...
void sortColors(__local const float * values, __local int * ranks)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        if (tid > i && ranks[hook(13, tid)] == ranks[hook(13, i)]) ++ranks[hook(13, tid)];
    }
}
...
void loadColorBlock(__global const uint * image, __local float4 * colors, __local float4 * sums, __local int * xrefs, __local float* temp, int groupOffset)
{
    ...
    if (idx < 16)
    {
        ...
        uint c = image[hook(1, (bid) * 16 + idx)];
    
        colors[hook(10, idx)].x = ((c >> 0) & 0xFF) * 0.003921568627f;    // * (1.0f / 255.0f);
        colors[hook(10, idx)].y = ((c >> 8) & 0xFF) * 0.003921568627f;    // * (1.0f / 255.0f);
        colors[hook(10, idx)].z = ((c >> 16) & 0xFF) * 0.003921568627f;   //* (1.0f / 255.0f);
              ...
              __constant float* alphaTable3, __constant int* prods3)
{
    ...
    for(int i = 0; i < 16; i++)
    {
      int pidx = idx + NUM_THREADS * i;
        ...
        ushort start, end;
        uint permutation = permutations[hook(0, pidx)];
        if (pidx < 160) s_permutations[hook(16, pidx)] = permutation;
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        ushort start, end;
        uint permutation = s_permutations[hook(16, pidx)];
