//{"<recovery-expr>()":5,"<recovery-expr>(g_odata)":6,"<recovery-expr>(sdata)":4,"g_idata":0,"g_odata":1,"n":2,"sdata":3}
...
__kernel void reduce0(__global T *g_idata, __global T *g_odata, unsigned int n, __local T* sdata)
{
    ...
    sdata[hook(4, tid)] = (i < n) ? g_idata[hook(5, i)] : 0;
    ...
    if (tid == 0) g_odata[hook(6, get_group_id(0))] = sdata[hook(5, 0)];
}
...
__kernel void reduce1(__global T *g_idata, __global T *g_odata, unsigned int n, __local T* sdata)
{
    ...
    for(unsigned int s=1; s < get_local_size(0); s *= 2) 
    {
        ...
        {
            sdata[hook(4, index)] += sdata[hook(5, index + s)];
        }
...
__kernel void reduce3(__global T *g_idata, __global T *g_odata, unsigned int n, __local T* sdata)
{
    ...
    if (i + get_local_size(0) < n) 
        sdata[hook(4, tid)] += g_idata[hook(5, i + get_local_size(0))];  
...
__kernel void reduce4(__global T *g_idata, __global T *g_odata, unsigned int n, __local volatile T* sdata)
{
    ...
    if (tid < 32)
    {
        if (blockSize >=  64) { sdata[hook(4, tid)] += sdata[hook(5, tid + 32)]; }
        if (blockSize >=  32) { sdata[hook(4, tid)] += sdata[hook(5, tid + 16)]; }
        if (blockSize >=  16) { sdata[hook(4, tid)] += sdata[hook(5, tid + 8)]; }
        if (blockSize >=   8) { sdata[hook(4, tid)] += sdata[hook(5, tid + 4)]; }
        if (blockSize >=   4) { sdata[hook(4, tid)] += sdata[hook(5, tid + 2)]; }
        if (blockSize >=   2) { sdata[hook(4, tid)] += sdata[hook(5, tid + 1)]; }
    }
...
__kernel void reduce5(__global T *g_idata, __global T *g_odata, unsigned int n, __local volatile T* sdata)
{
    ...
    // do reduction in shared mem
    if (blockSize >= 512) { if (tid < 256) { sdata[hook(4, tid)] += sdata[hook(5, tid + 256)]; } barrier(CLK_LOCAL_MEM_FENCE); }
    if (blockSize >= 256) { if (tid < 128) { sdata[hook(4, tid)] += sdata[hook(5, tid + 128)]; } barrier(CLK_LOCAL_MEM_FENCE); }
    if (blockSize >= 128) { if (tid <  64) { sdata[hook(4, tid)] += sdata[hook(5, tid + 64)]; } barrier(CLK_LOCAL_MEM_FENCE); }
...
__kernel void reduce6(__global T *g_idata, __global T *g_odata, unsigned int n, __local volatile T* sdata)
{
    ...
    while (i < n)
    {         
        sdata[hook(4, tid)] += g_idata[hook(5, i)];
