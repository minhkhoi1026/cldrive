//{"N":2,"d_CellEnd":5,"d_CellStart":4,"d_Data":0,"d_Hash":4,"d_Index":3,"d_Pos":6,"d_ReorderedPos":1,"d_ReorderedVel":2,"d_Vel":0,"deltaTime":3,"localHash":8,"numParticles":7,"params":6,"val":1}
...
__kernel void integrate(
    ...
    float4 pos = d_Pos[hook(6, index)];
    float4 vel = d_Vel[hook(0, index)];
    ...
    d_Pos[hook(6, index)] = pos;
    d_Vel[hook(0, index)] = vel;
}
...
__kernel void calcHash(
    ...
    d_Hash[hook(4, index)] = gridHash;
    d_Index[hook(3, index)] = index;
}
...
__kernel void Memset(
    ...
    if(get_global_id(0) < N)
        d_Data[hook(0, get_global_id(0))] = val;
}
...
__kernel void findCellBoundsAndReorder(
    ...
    if(index < numParticles){
        hash = d_Hash[hook(4, index)];

        //Load hash data into local memory so that we can look 
        //at neighboring particle's hash value without loading
        //two hash values per thread
        localHash[hook(8, get_local_id(0) + 1)] = hash;

        //First thread in block must load neighbor particle hash
        if(index > 0 && get_local_id(0) == 0)
            localHash[hook(8, 0)] = d_Hash[hook(4, index - 1)];
    }
    ...
    if(index < numParticles){
        //Border case
        if(index == 0)
            d_CellStart[hook(4, hash)] = 0;

        //Main case
        else{
            if(hash != localHash[hook(8, get_local_id(0))])
                d_CellEnd[hook(5, localHash[ghook(8, get_local_id(0)))]  = d_CellStart[hook(4, hash)] = index;
        };

        //Another border case
        if(index == numParticles - 1)
            d_CellEnd[hook(5, hash)] = numParticles;


        //Now use the sorted index to reorder the pos and vel arrays
        uint sortedIndex = d_Index[hook(3, index)];
        float4 pos = d_Pos[hook(6, sortedIndex)];
        float4 vel = d_Vel[hook(0, sortedIndex)];

        d_ReorderedPos[hook(1, index)] = pos;
        d_ReorderedVel[hook(2, index)] = vel;
    }
}
...
__kernel void collide(
    ...
    float4   pos = d_ReorderedPos[hook(1, index)];
    float4   vel = d_ReorderedVel[hook(2, index)];
    float4 force = (float4)(0, 0, 0, 0);
    ...
    for(int z = -1; z <= 1; z++)
        for(int y = -1; y <= 1; y++)
            for(int x = -1; x <= 1; x++){
                ...
                uint startI = d_CellStart[hook(4, hash)];
                ...
                uint endI = d_CellEnd[hook(5, hash)];
                for(uint j = startI; j < endI; j++){
                    if(j == index)
                        continue;

                    float4 pos2 = d_ReorderedPos[hook(1, j)];
                    float4 vel2 = d_ReorderedVel[hook(2, j)];
    ...
    d_Vel[hook(0, d_Index[ihook(3, index))] = vel + force;
}
