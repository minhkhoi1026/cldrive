//{"N":3,"c_Table":1,"d_Output":0,"iDevice":2,"nDevice":3,"pathN":1,"seed":2}
				                   ...
				                   unsigned int N)
{
    ...
    for (unsigned int pos = globalID_x; pos < N; pos += globalSize_x) {
        ...
        for(int bit = 0; bit < QRNG_RESOLUTION; bit++, data >>= 1)
            if(data & 1) result ^= c_Table[hook(1, bit + localID_y * 31)];

        d_Output[hook(0, mul24(localID_y, N) + pos)] = (float)(result + 1) * INT_SCALE;
    }
}
