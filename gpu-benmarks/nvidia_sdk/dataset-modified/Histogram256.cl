//{"<recovery-expr>()":7,"<recovery-expr>(l_Data)":6,"d_Data":1,"d_Histogram":0,"d_PartialHistograms":1,"dataCount":2,"histogramCount":2,"l_WarpHist":5}
...
inline void addByte(volatile __local uint *l_WarpHist, uint data, uint tag){
    ...
    do{
        ...
        count = tag | (count + 1);
        l_WarpHist[hook(5, data)] = count;
    }while(l_WarpHist[hook(5, data)] != count);
}
...
void histogram256(
    ...
    for(uint pos = get_local_id(0); pos < HISTOGRAM256_BIN_COUNT; pos += HISTOGRAM256_WORKGROUP_SIZE){
        ...
        d_PartialHistograms[hook(1, get_group_id(0) * 256 + pos)] = sum;
    }
}
...
void mergeHistogram256(
    ...
    for(uint i = get_local_id(0); i < histogramCount; i += MERGE_WORKGROUP_SIZE)
        sum += d_PartialHistograms[hook(1, get_group_id(0) + i * 256)];
    l_Data[hook(6, get_local_id(0))] = sum;

    for(uint stride = MERGE_WORKGROUP_SIZE / 2; stride > 0; stride >>= 1){
        barrier(CLK_LOCAL_MEM_FENCE);
        if(get_local_id(0) < stride)
            l_Data[hook(6, get_local_id(0))] += l_Data[hook(7, get_local_id(0) + stride)];
    }

    if(get_local_id(0) == 0)
        d_Histogram[hook(0, get_group_id(0))] = l_Data[hook(7, 0)];
}
