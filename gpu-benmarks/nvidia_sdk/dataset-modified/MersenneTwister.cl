//{"d_MT":1,"d_Rand":0,"mt":3,"nPerRng":1}
			      ...
			      int nPerRng)
{
    ...
    matrix_a = d_MT[hook(1, globalID)].matrix_a;
    mask_b   = d_MT[hook(1, globalID)].mask_b;
    mask_c   = d_MT[hook(1, globalID)].mask_c;
        
    //Initialize current state
    mt[hook(3, 0)] = d_MT[hook(1, globalID)].seed;
    for (iState = 1; iState < MT_NN; iState++)
        mt[hook(3, iState)] = (1812433253U * (mt[hook(3, iState - 1)] ^ (mt[hook(3, iState - 1)] >> 30)) + iState) & MT_WMASK;
    ...
    for (iOut = 0; iOut < nPerRng; iOut++) {
        ...
        mti  = mti1;
        mti1 = mt[hook(3, iState1)];
        mtiM = mt[hook(3, iStateM)];
        ...
        d_Rand[hook(0, globalID + iOut * 4096)] = ((float)x + 1.0f) / 4294967296.0f;
    }
}
...
__kernel void BoxMuller(__global float *d_Rand, int nPerRng) 
{
    ...
    for (int iOut = 0; iOut < nPerRng; iOut += 2)
        BoxMullerTrans(&d_Rand[hook(0, globalID + (iOut + 0) * 4096)],
		       &d_Rand[hook(0, globalID + (iOut + 1) * 4096)]);
}
