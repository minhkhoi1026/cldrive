//{"<recovery-expr>()":6,"<recovery-expr>(l_Data)":5,"d_Data":1,"d_Histogram":0,"d_PartialHistograms":1,"dataCount":2,"histogramCount":2}
...
void histogram64(
    ...
    if(get_local_id(0) < HISTOGRAM64_BIN_COUNT){
        ...
        d_PartialHistograms[hook(1, get_group_id(0) * 64 + get_local_id(0))] = sum;
    }
}
...
void mergeHistogram64(
    ...
    for(uint i = get_local_id(0); i < histogramCount; i += MERGE_WORKGROUP_SIZE)
        sum += d_PartialHistograms[hook(1, get_group_id(0) + i * 64)];
    l_Data[hook(5, get_local_id(0))] = sum;

    for(uint stride = MERGE_WORKGROUP_SIZE / 2; stride > 0; stride >>= 1){
        barrier(CLK_LOCAL_MEM_FENCE);
        if(get_local_id(0) < stride)
            l_Data[hook(5, get_local_id(0))] += l_Data[hook(6, get_local_id(0) + stride)];
    }

    if(get_local_id(0) == 0)
        d_Histogram[hook(0, get_group_id(0))] = l_Data[hook(6, 0)];
}
