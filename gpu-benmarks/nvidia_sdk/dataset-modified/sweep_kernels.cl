//{"<recovery-expr>()":13,"<recovery-expr>(a)":12,"a":14,"a_d":0,"b_d":1,"block":4,"c_d":2,"d_d":3,"height":3,"idata":1,"num_systems":5,"odata":0,"w_d":6,"width":2,"x_d":4}
...
__kernel void sweep_small_systems_local_kernel(__global float *a_d, __global float *b_d, __global float *c_d, __global float *d_d, __global float *x_d, int num_systems)
{
	...
	c1 = c_d[hook(2, base_idx)];
	c2 = b_d[hook(1, base_idx)];
	f_i = d_d[hook(3, base_idx)];

#ifndef NATIVE_DIVIDE
	a[1] = - c1 / c2;
	x_prev = f_i / c2;
#else
	a[hook(12, 1)] = - native_divide(c1, c2);
	x_prev = native_divide(f_i, c2);
#endif

	// forward trace
	int idx = base_idx;
	x_d[hook(4, base_idx)] = x_prev;
	...
	{
		idx -= stride;
		x_next = x_d[hook(4, idx)];
		x_next += x_prev * a[hook(13, k + 1)];
		x_d[hook(4, idx)] = x_prev = x_next;
	}
}
...
__kernel void sweep_small_systems_global_kernel(__global float *a_d, __global float *b_d, __global float *c_d, __global float *d_d, __global float *x_d, int num_systems, __global float *w_d)
{
	...
	x_prev = f_i / c2;
#else
	w_d[hook(6, getLocalIdx(i, 1, num_systems))] = - native_divide(c1, c2);
	x_prev = native_divide(f_i, c2);
#endif
	...
	for (int k = 1; k < system_size-1; k++)
	{
		...
		float q = (c3 * w_d[hook(6, getLocalIdx(i, k, num_systems))] + c2);
#ifndef NATIVE_DIVIDE
		float t = 1 / q; 
#else
		float t = native_recip(q);
		...
		w_d[hook(6, getLocalIdx(i, k + 1, num_systems))] = - c1 * t;
	}
	...
	for (int k = system_size-2; k >= 0; k--)
	{
		idx -= stride;
		x_next = x_d[hook(4, idx)];
		x_next += x_prev * w_d[hook(6, getLocalIdx(i, k + 1, num_systems))];
		x_d[hook(4, idx)] = x_prev = x_next;
	}
}
...
{
	return (float4)(a[hook(14, i)], a[hook(14, i + 1)], a[hook(14, i + 2)], a[hook(14, i + 3)]);
}
