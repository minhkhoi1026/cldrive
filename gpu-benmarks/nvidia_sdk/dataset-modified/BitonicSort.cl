//{"<recovery-expr>()":10,"<recovery-expr>(l_key)":8,"<recovery-expr>(l_val)":9,"arrayLength":4,"d_DstKey":0,"d_DstVal":1,"d_SrcKey":2,"d_SrcVal":3,"size":6,"sortDir":7,"stride":5}
...
void bitonicSortLocal(
    ...
    for(uint size = 2; size < arrayLength; size <<= 1){
        ...
        for(uint stride = size / 2; stride > 0; stride >>= 1){
            ...
            ComparatorLocal(
                &l_key[hook(10, pos + 0)], &l_val[hook(10, pos + 0)],
                &l_key[hook(10, pos + stride)], &l_val[hook(10, pos + stride)],
                dir
            );
        }
    }
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    d_DstKey[                     hook(0, 0)] = l_key[hook(10, get_local_id(0) + 0)];
    d_DstVal[                     hook(1, 0)] = l_val[hook(10, get_local_id(0) + 0)];
