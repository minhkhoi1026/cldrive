...
inline float2 sincos_f2(const float val);
inline float2 sincos_f2(const float val) {
  return (float2)(sin(val), cos(val));
}

inline void init_particles(float4* out_position, float4* out_dir, unsigned int* random_seed, const unsigned int type, const float spawn_rate_ts, const float living_time, const unsigned int particle_count, const float velocity, const float4 angle, const float4 extents, const float4 direction, const float4 position_offset);
inline void init_particles(float4* out_position, float4* out_dir, unsigned int* random_seed, const unsigned int type, const float spawn_rate_ts, const float living_time, const unsigned int particle_count, const float velocity, const float4 angle, const float4 extents, const float4 direction, const float4 position_offset) {
  ...
  switch (type) {
    case 0: {
      float4 rand = (float4)(sfrand_m1_1(random_seed), sfrand_m1_1(random_seed), sfrand_m1_1(random_seed), 0.0f);
      position.xyz = 0.5f * rand.xyz * extents.xyz;
    } break;
    case 1: {
      float theta = 2.0f * 3.1415926535897932384626433832795f * sfrand_0_1(random_seed);
      float u = sfrand_m1_1(random_seed);

      position = (float4)(cos(theta) * u, sin(theta) * u, u, 0.0f);

      float scale = sfrand_m1_1(random_seed);
  ...
  float4 rangle = (float4)(sfrand_m1_1(random_seed), sfrand_m1_1(random_seed), sfrand_m1_1(random_seed), 0.0f) * angle;
  ...
  *out_position = (float4)(position.x, position.y, position.z, ltime);
  *out_dir = (float4)(dir.x, dir.y, dir.z, 0.0f);
}
kernel void particle_respawn
  ...
  unsigned int random_seed = seed ^ mul24(seed - particle_num, 16807u);
