...
kernel void compute_data_cost_1(global const float* selected_disp_pyr, global float* data_cost_, global uchar* cleft, global uchar* cright, int h, int w, int level, int nr_plane, int channels, int cmsg_step1, int cmsg_step2, int cdisp_step1, int cdisp_step2, float cdata_weight, float cmax_data_term, int cimg_step, int cth) {
  ...
  if (y < h && x < w) {
    ...
    global const float* selected_disparity = selected_disp_pyr + y / 2 * cmsg_step2 + x / 2;
    global float* data_cost = data_cost_ + y * cmsg_step1 + x;
    ...
    for (int d = 0; d < nr_plane; d++) {
      ...
      data_cost[hook(20, cdisp_step1 * d)] = val;
    }
  }
}