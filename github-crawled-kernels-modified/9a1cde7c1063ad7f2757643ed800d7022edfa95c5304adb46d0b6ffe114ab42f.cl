...
kernel void warpPerspectiveCubic_C1_D5(global float* src, global float* dst, int src_cols, int src_rows, int dst_cols, int dst_rows, int srcStep, int dstStep, int src_offset, int dst_offset, constant F* M, int threadCols) {
  ...
  if (dx < threadCols && dy < dst_rows) {
    ...
    for (i = 0; i < 16; i++)
      v[hook(13, i)] = (sx + (i & 3) >= 0 && sx + (i & 3) < src_cols && sy + (i >> 2) >= 0 && sy + (i >> 2) < src_rows) ? src[hook(0, src_offset + (sy + (i >> 2)) * srcStep + (sx + (i & 3)))] : (float)0;
