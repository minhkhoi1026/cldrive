...
__attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) kernel void dgemm_kernel(unsigned int M, unsigned int N, unsigned int K, double ALPHA, global double* A, global double* B, global double* C) {
  double rC[8][4] = {{(double)0}};
  double rA[8];
  double rB[4];

  local double lA[2064];
  local double lB[1040];
  ...
  for (unsigned int block_k = 0; block_k < K; block_k += 16) {
    local double* plA = lA + idyT * 129 + idxT;
    local double* plB = lB + idyT * 65 + idxT;
