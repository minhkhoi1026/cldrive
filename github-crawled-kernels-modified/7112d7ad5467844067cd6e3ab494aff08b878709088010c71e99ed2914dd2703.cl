...
kernel void ckDMAS(global float* dSource, global float* dDest, global unsigned short* usedLines, global unsigned short* AddSamples, constant float* apodArray, unsigned short apodArraySize, unsigned int inputL, unsigned int inputS, unsigned int Slices, unsigned int outputL, unsigned int outputS) {
  ...
  if (globalPosX < outputL && globalPosY < outputS && globalPosZ < Slices) {
    ...
    dDest[hook(1, globalPosZ * outputL * outputS + globalPosY * outputL + globalPosX)] = output / (float)(curUsedLines * curUsedLines - (curUsedLines - 1));
  }
}