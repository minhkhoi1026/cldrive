...
kernel void downsweep_subarrays(global int* data, global int* part, global int* flag, global int* data2, global int* part2, global int* flag2, local int* x, local int* p, local int* f, int n) {
  ...
  x[hook(6, local_lane0)] = data[hook(0, lane0)];
  x[hook(6, local_lane1)] = data[hook(0, lane1)];
  p[hook(7, local_lane0)] = part[hook(1, lane0)];
  p[hook(7, local_lane1)] = part[hook(1, lane1)];
  f[hook(8, local_lane0)] = flag[hook(2, lane0)];
  f[hook(8, local_lane1)] = flag[hook(2, lane1)];

  if (lid == (wx - 1)) {
    x[hook(6, local_lane1)] = data2[hook(3, grpid)];
    p[hook(7, local_lane1)] = part2[hook(4, grpid)];
  }
