...
kernel void compute_xt_times_vector(const int observations, const int snps, const int subject_chunks, const int packedstride_snpmajor, const int max_work_group_size, global const char* packedgeno_snpmajor, global float* Xt_vec_chunks, global const float* vec, global const float* means, global const float* precisions, global const long* mask_n, local float* local_floatgeno) {
  local float mapping[4];
  ...
  float mean = means[hook(8, snp)];
  float precision = precisions[hook(9, snp)];
