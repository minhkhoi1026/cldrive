...
kernel void transform(global float* image, global float* output, global float4* matrix, global float2* offset, int image_width, int image_height, int output_width, int output_height, float fill, int mode) {
  ...
  if (0.0f <= tx && tx < image_width && 0.0f <= ty && ty < image_height) {
    if (mode == 1) {
      float image_p = image[hook(0, ty_prev * image_width + tx_prev)], image_x = image[hook(0, ty_prev * image_width + tx_next)], image_y = image[hook(0, ty_next * image_width + tx_prev)], image_n = image[hook(0, ty_next * image_width + tx_next)];
      ...
      interp = image[hook(0, ((int)ty) * image_width + ((int)tx))];
    }
  }
  ...
  output[hook(1, gid1 * output_width + gid0)] = interp;
}