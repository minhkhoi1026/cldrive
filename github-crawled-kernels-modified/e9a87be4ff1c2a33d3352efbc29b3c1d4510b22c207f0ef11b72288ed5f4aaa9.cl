...
void FHTransformLoop3(local float* data, global const float* gsincos) {
}

kernel void amdFHT32768FromSXFade(global const float* in, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int frame_ln, int current_bin, int xfade_span) {
}