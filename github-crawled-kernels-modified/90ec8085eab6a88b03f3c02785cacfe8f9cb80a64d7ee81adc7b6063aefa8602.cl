//{"c2":4,"n":3,"o":0,"prefactor":5,"q":2,"qlocal":8,"s":1,"slocal":7,"vlocal":6}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void coulomb1(global double* o, global int* s, global int* q, int n, int c2, double prefactor) {
  ...
  for (int load_number = 0; load_number < num_loads; load_number++) {
    ...
    if (load_id < n) {
      slocal[hook(7, j)] = s[hook(1, load_id)];
      qlocal[hook(8, j)] = q[hook(2, load_id)];
    } else {
      slocal[hook(7, j)] = -1;
      qlocal[hook(8, j)] = -1;
    }
    ...
    if (s >= 0) {
      ...
      if (r > 0 && r < c2) {
        vlocal[hook(6, j)] = vlocal[hook(6, j)] + qlocal[hook(8, j)] * rsqrt(r);
      }
    }
    barrier(0x01);
  }
