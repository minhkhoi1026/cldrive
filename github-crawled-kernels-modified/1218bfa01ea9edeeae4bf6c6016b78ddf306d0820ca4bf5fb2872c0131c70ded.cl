//{"height":2,"maxIterations":6,"offsetX":4,"offsetY":5,"pixels":0,"scale":3,"width":1}
...
kernel void mandelGPU(global unsigned int* pixels, const int width, const int height, const float scale, const float offsetX, const float offsetY, const int maxIterations) {
  ...
  if ((screenY.s0 >= height) || (screenY.s1 >= height) || (screenY.s2 >= height) || (screenY.s3 >= height))
    return;
  ...
  pixels[hook(0, gid)] = s0 | (s1 << 8) | (s2 << 16) | (s3 << 24);
}