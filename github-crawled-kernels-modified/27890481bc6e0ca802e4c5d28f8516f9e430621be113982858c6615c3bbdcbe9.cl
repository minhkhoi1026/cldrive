//{"Gauss76Wt":17,"Gauss76Z":16,"Ncap_radius":8,"Nlength":9,"Nphi":11,"Nq":3,"Nradius":7,"Ntheta":10,"background":13,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":12,"sld":14,"solvent_sld":15}
...
float Iq(float q, float sld, float solvent_sld, float radius, float cap_radius, float length);
float Iqxy(float qx, float qy, float sld, float solvent_sld, float radius, float cap_radius, float length, float theta, float phi);
float _cap_kernel(float q, float h, float cap_radius, float length, float sin_alpha, float cos_alpha);
float _cap_kernel(float q, float h, float cap_radius, float length, float sin_alpha, float cos_alpha) {
  const float upper = 1.0f;
  const float lower = h / cap_radius;

  const float m = q * cap_radius * cos_alpha;
  const float b = q * (0.5f * length - h) * cos_alpha;
  const float qrst = q * cap_radius * sin_alpha;
  float total = 0.0f;
  for (int i = 0; i < 76; i++) {
    const float t = 0.5f * (Gauss76Z[hook(16, i)] * (upper - lower) + upper + lower);
    const float radical = 1.0f - t * t;
    const float arg = qrst * sqrt(radical);
    const float be = (arg == 0.0f ? 0.5f : J1(arg) / arg);
    const float Fq = cos(m * t + b) * radical * be;
    total += Gauss76Wt[hook(17, i)] * Fq;
  }

  const float integral = 0.5f * (upper - lower) * total;
  return 4.0f * 3.14159265358979323846f * cap_radius * cap_radius * cap_radius * integral;
}

float form_volume(float radius, float cap_radius, float length) {
  const float hc = cap_radius - sqrt(cap_radius * cap_radius - radius * radius);
  return 3.14159265358979323846f * (radius * radius * (length + hc) + 0.333333333333333f * hc * hc * hc);
}

float Iq(float q, float sld, float solvent_sld, float radius, float cap_radius, float length) {
  float sn, cn;

  if (cap_radius < radius)
    return __builtin_astype((2147483647), float);

  const float lower = 0.0f;
  const float upper = 0x1.921fb54442d18p+0;
  const float h = sqrt(cap_radius * cap_radius - radius * radius);
  float total = 0.0f;
  for (int i = 0; i < 76; i++) {
    const float alpha = 0.5f * (Gauss76Z[hook(16, i)] * (upper - lower) + upper + lower);
    do {
    ...
    total += Gauss76Wt[hook(17, i)] * Aq * Aq * sn;
  }
...
kernel void capped_cylinder_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                                 global float* loops_g,

                                 local float* loops, const float cutoff, const int Nradius, const int Ncap_radius, const int Nlength, const int Ntheta, const int Nphi,

                                 const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nradius + Ncap_radius + Nlength + Ntheta + Nphi) * 2, 0);
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int cap_radius_i = 0; cap_radius_i < Ncap_radius; cap_radius_i++) {
        const float cap_radius = loops[hook(5, 2 * (cap_radius_i + Nradius))];
        const float cap_radius_w = loops[hook(5, 2 * (cap_radius_i + Nradius) + 1)];
        for (int length_i = 0; length_i < Nlength; length_i++) {
          const float length = loops[hook(5, 2 * (length_i + Nradius + Ncap_radius))];
          const float length_w = loops[hook(5, 2 * (length_i + Nradius + Ncap_radius) + 1)];
          for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
            const float theta = loops[hook(5, 2 * (theta_i + Nradius + Ncap_radius + Nlength))];
            const float theta_w = loops[hook(5, 2 * (theta_i + Nradius + Ncap_radius + Nlength) + 1)];
            for (int phi_i = 0; phi_i < Nphi; phi_i++) {
              const float phi = loops[hook(5, 2 * (phi_i + Nradius + Ncap_radius + Nlength + Ntheta))];
              const float phi_w = loops[hook(5, 2 * (phi_i + Nradius + Ncap_radius + Nlength + Ntheta) + 1)];

              const float weight = radius_w * cap_radius_w * length_w * theta_w * phi_w;
              if (weight > cutoff) {
                const float scattering = Iqxy(qxi, qyi, sld, solvent_sld, radius, cap_radius, length, theta, phi);
                if (!isnan(scattering)) {
                  ...
                  const float vol_weight = radius_w * cap_radius_w * length_w;
                  vol += vol_weight * form_volume(radius, cap_radius, length);
