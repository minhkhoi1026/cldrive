//{"displacement":2,"position":0,"velocity":1}
...
kernel void naive_update_positions(global double2* position, global double2* velocity, global double2* displacement) {
  int id = get_global_id(0);
  double2 base = position[hook(0, id)];
  double2 base_vel = velocity[hook(1, id)];
  base += base_vel * 0.001;
  displacement[hook(2, id)] += base_vel * 0.001;

  base = wrap(base);

  position[hook(0, id)] = base;
}