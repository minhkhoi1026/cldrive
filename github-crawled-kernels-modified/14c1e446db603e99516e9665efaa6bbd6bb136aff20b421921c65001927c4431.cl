//{"T->vertices":8,"array":6,"dim":2,"flip":7,"ids_behind":16,"num":22,"nums":4,"passback":21,"points":11,"pr":19,"raw":14,"raw_data":0,"ret":12,"ret[0]":17,"ret[1]":18,"rotated":15,"size":23,"sizes":5,"stride":1,"tex_id":3,"triangle->vertices":13,"tris":20,"x":9,"y":10}
...
void set_tri_vertex(global struct triangle* T, int i, float3 pos) {
  T->vertices[hook(8, i)].pos.xyz = pos;
}
...
float calc_rconstant(float x[3], float y[3]) {
  return native_recip(x[hook(9, 1)] * y[hook(10, 2)] + x[hook(9, 0)] * (y[hook(10, 1)] - y[hook(10, 2)]) - x[hook(9, 2)] * y[hook(10, 1)] + (x[hook(9, 2)] - x[hook(9, 1)]) * y[hook(10, 0)]);
}
...
void calc_min_max(float3 points[3], float width, float height, float ret[4]) {
  ...
  for (int i = 0; i < 3; i++) {
    x[hook(9, i)] = round(points[hook(11, i)].x);
    y[hook(10, i)] = round(points[hook(11, i)].y);
  }

  ret[hook(12, 0)] = min3(x[hook(9, 0)], x[hook(9, 1)], x[hook(9, 2)]) - 1;
  ret[hook(12, 1)] = max3(x[hook(9, 0)], x[hook(9, 1)], x[hook(9, 2)]);
  ret[hook(12, 2)] = min3(y[hook(10, 0)], y[hook(10, 1)], y[hook(10, 2)]) - 1;
  ret[hook(12, 3)] = max3(y[hook(10, 0)], y[hook(10, 1)], y[hook(10, 2)]);

  ret[hook(12, 0)] = clamp(ret[hook(12, 0)], 0.0f, width - 1);
  ret[hook(12, 1)] = clamp(ret[hook(12, 1)], 0.0f, width - 1);
  ret[hook(12, 2)] = clamp(ret[hook(12, 2)], 0.0f, height - 1);
  ret[hook(12, 3)] = clamp(ret[hook(12, 3)], 0.0f, height - 1);
}
...
void calc_min_max_oc(float3 points[3], float mx, float my, float width, float height, float ret[4]) {
  ...
  ret[hook(12, 0)] = clamp(ret[hook(12, 0)], mx, width - 1);
  ret[hook(12, 1)] = clamp(ret[hook(12, 1)], mx, width - 1);
  ret[hook(12, 2)] = clamp(ret[hook(12, 2)], my, height - 1);
  ret[hook(12, 3)] = clamp(ret[hook(12, 3)], my, height - 1);
}
...
void rot_3(global struct triangle* triangle, const float3 c_pos, const float3 c_rot, const float3 offset, const float3 rotation_offset, float3 ret[3]) {
  ret[hook(12, 0)] = rot(vertex_pos(&triangle->vertices[hook(13, 0)]), 0, rotation_offset);
  ret[hook(12, 1)] = rot(vertex_pos(&triangle->vertices[hook(13, 1)]), 0, rotation_offset);
  ret[hook(12, 2)] = rot(vertex_pos(&triangle->vertices[hook(13, 2)]), 0, rotation_offset);

  ret[hook(12, 0)] = rot(ret[hook(12, 0)] + offset, c_pos, c_rot);
  ret[hook(12, 1)] = rot(ret[hook(12, 1)] + offset, c_pos, c_rot);
  ret[hook(12, 2)] = rot(ret[hook(12, 2)] + offset, c_pos, c_rot);
}

void rot_3_raw(const float3 raw[3], const float3 rotation, float3 ret[3]) {
  ret[hook(12, 0)] = rot(raw[hook(14, 0)], 0, rotation);
  ret[hook(12, 1)] = rot(raw[hook(14, 1)], 0, rotation);
  ret[hook(12, 2)] = rot(raw[hook(14, 2)], 0, rotation);
}

void rot_3_pos(const float3 raw[3], const float3 pos, const float3 rotation, float3 ret[3]) {
  ret[hook(12, 0)] = rot(raw[hook(14, 0)], pos, rotation);
  ret[hook(12, 1)] = rot(raw[hook(14, 1)], pos, rotation);
  ret[hook(12, 2)] = rot(raw[hook(14, 2)], pos, rotation);
}

void depth_project(float3 rotated[3], float width, float height, float fovc, float3 ret[3]) {
  for (int i = 0; i < 3; i++) {
    float2 rxy = mad(rotated[hook(15, i)].xy, fovc / rotated[hook(15, i)].z, (float2){width / 2.f, height / 2.f});

    ret[hook(12, i)].xy = rxy;
    ret[hook(12, i)].z = rotated[hook(15, i)].z;
  }
}
...
void generate_new_triangles(float3 points[3], int* num, float3 ret[2][3]) {
  ...
  for (int i = 0; i < 3; i++) {
    if (points[hook(11, i)].z <= 20 || points[hook(11, i)].z > 350000.0f) {
      ids_behind[hook(16, n_behind)] = i;
      n_behind++;
    } else {
      id_valid = i;
    }
  ...
  if (n_behind == 0) {
    ret[hook(12, 0)][hook(17, 0)] = points[hook(11, 0)];
    ret[hook(12, 0)][hook(17, 1)] = points[hook(11, 1)];
    ret[hook(12, 0)][hook(17, 2)] = points[hook(11, 2)];
  ...
  if (n_behind == 2) {
    g2 = ids_behind[hook(16, 0)];
    g3 = ids_behind[hook(16, 1)];
    g1 = id_valid;
  }

  p1 = points[hook(11, g2)] + native_divide((20 - points[hook(11, g2)].z) * (points[hook(11, g1)] - points[hook(11, g2)]), points[hook(11, g1)].z - points[hook(11, g2)].z);
  p2 = points[hook(11, g3)] + native_divide((20 - points[hook(11, g3)].z) * (points[hook(11, g1)] - points[hook(11, g3)]), points[hook(11, g1)].z - points[hook(11, g3)].z);

  if (n_behind == 1) {
    c1 = points[hook(11, g2)];
    c2 = points[hook(11, g3)];

    ret[hook(12, 0)][hook(17, 0)] = p1;
    ret[hook(12, 0)][hook(17, 1)] = c1;
    ret[hook(12, 0)][hook(17, 2)] = c2;

    ret[hook(12, 1)][hook(18, 0)] = p1;
    ret[hook(12, 1)][hook(18, 1)] = c2;
    ret[hook(12, 1)][hook(18, 2)] = p2;
    *num = 2;
  } else {
    c1 = points[hook(11, g1)];
    ret[hook(12, 0)][hook(17, ids_behind[0hook(16, 0))] = p1;
    ret[hook(12, 0)][hook(17, ids_behind[1hook(16, 1))] = p2;
    ret[hook(12, 0)][hook(17, id_valid)] = c1;
    *num = 1;
  }
}
...
void full_rotate_n_extra(float3 v1, float3 v2, float3 v3, float3 passback[2][3], int* num, const float3 c_pos, const float3 c_rot, const float3 offset, const float3 rotation_offset, const float fovc, const float width, const float height) {
  ...
  pr[hook(19, 0)] = rot_with_offset(v1, c_pos, c_rot, offset, rotation_offset);
  pr[hook(19, 1)] = rot_with_offset(v2, c_pos, c_rot, offset, rotation_offset);
  pr[hook(19, 2)] = rot_with_offset(v3, c_pos, c_rot, offset, rotation_offset);
  ...
  *num = n;
  ...
  depth_project(tris[hook(20, 0)], width, height, fovc, passback[hook(21, 0)]);

  if (n == 2) {
    depth_project(tris[hook(20, 1)], width, height, fovc, passback[hook(21, 1)]);
  }
}
...
void full_rotate_quat(float3 v1, float3 v2, float3 v3, float3 passback[2][3], int* num, float3 c_pos, float3 c_rot, float3 offset, float4 rotation_offset, float scale, float fovc, float width, float height) {
  ...
  pr[hook(19, 0)] = rot_quat_with_offset(v1 * scale, c_pos, c_rot, offset, rotation_offset);
  pr[hook(19, 1)] = rot_quat_with_offset(v2 * scale, c_pos, c_rot, offset, rotation_offset);
  pr[hook(19, 2)] = rot_quat_with_offset(v3 * scale, c_pos, c_rot, offset, rotation_offset);
...
{
  array[hook(6, coord.z * width * height + mul24(coord.y, width) + coord.x)] = convert_uchar4(to_write);
}

uint4 read_image_3d_hardware(int4 coord, global uchar4* array, int width, int height)

{
  return convert_uint4(array[hook(6, coord.z * width * height + mul24(coord.y, width) + coord.x)]);
}
...
kernel void generate_from_raw(global uchar* raw_data, int stride, int2 dim, unsigned int tex_id, global unsigned int* nums, global unsigned int* sizes, image_3d_write array, int flip) {
  ...
  int slice = nums[hook(4, tex_id)] >> 16;
  int width = sizes[hook(5, slice)];

  if (x >= width || y >= width)
    return;

  uint4 val = (unsigned int)raw_data[hook(0, y * stride + x)];
