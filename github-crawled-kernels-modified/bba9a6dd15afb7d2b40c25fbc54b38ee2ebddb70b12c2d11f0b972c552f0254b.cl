...
kernel void prescanLocal2(global unsigned int* restrict sum) {
  size_t localID = get_local_id(0);
  ...
  uint4 val = sum4[hook(6, localID)];
  lsum[hook(5, localID)] = val.x + val.y + val.z + val.w;

  barrier(0x01);

  unsigned int d2;
  unsigned int dstop = localWork >> 1;
  for (unsigned int d = 1; d < dstop; d = d2) {
    d2 = 2 * d;
    if ((localID + 1) % d2 == 0) {
      lsum[hook(5, localID)] += lsum[hook(5, localID - d)];
    }
    barrier(0x01);
  }

  if (localID == localWork - 1) {
    unsigned int indexLeft = localID - dstop;

    lsum[hook(5, localID)] = lsum[hook(5, indexLeft)];
    lsum[hook(5, indexLeft)] = 0;
  }

  barrier(0x01);

  for (unsigned int d = dstop; d >= 2; d = d2) {
    d2 = d / 2;
    if ((localID + 1) % d == 0) {
      unsigned int indexLeft = localID - d2;

      unsigned int t = lsum[hook(5, indexLeft)];
      lsum[hook(5, indexLeft)] = lsum[hook(5, localID)];
      lsum[hook(5, localID)] += t;
    }
    barrier(0x01);
  }

  unsigned int p = lsum[hook(5, localID)];
  uint4 result;
  result.x = p;
  p += val.x;
  result.y = p;
