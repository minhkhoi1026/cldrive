//{"DATA_D":5,"DATA_H":4,"DATA_W":3,"Phase_Gradients":0,"q11":1,"q21":2}
...
kernel void CalculatePhaseGradientsX(global float* Phase_Gradients, global const float2* q11, global const float2* q21, private int DATA_W, private int DATA_H, private int DATA_D) {
  ...
  a.x = c.x;
  a.y = c.y;
  c = q11[hook(1, idx_minus_1)];

  total_complex_product_real += a.x * c.x + a.y * c.y;
  total_complex_product_imag += a.y * c.x - a.x * c.y;

  a = q21[hook(2, idx_plus_1)];
  c = q21[hook(2, idx)];
