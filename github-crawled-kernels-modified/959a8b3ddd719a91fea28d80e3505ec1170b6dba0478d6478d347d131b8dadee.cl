//{"FE":4,"REAs":2,"atoms":1,"lATOMs":5,"lREAs":6,"nAtoms":0,"poss":3}
...
kernel void evalMorse(const int nAtoms, global float4* atoms, global float4* REAs, global float4* poss, global float4* FE) {
  local float4 lATOMs[32];
  local float4 lREAs[32];
  ...
  for (int i0 = 0; i0 < nAtoms; i0 += nL) {
    ...
    lREAs[hook(6, iL)] = REAs[hook(2, i)];
    barrier(0x01);
    for (int j = 0; j < nL; j++) {
      if ((j + i0) < nAtoms)
        fe += getMorse(pos - lATOMs[hook(5, j)].xyz, lREAs[hook(6, j)].xyz);
    }
    barrier(0x01);
  }
