...
kernel void vglClCumSumNorm(global int* cumsum, int size, int norm_total) {
  ...
  for (int d = global_size >> 1; d > 0; d >>= 1) {
    if (index < d) {
      ...
      if (d == global_size >> 1) {
        cumsum[hook(0, bi)] = cumsum[hook(0, bi)] * 255 / norm_total;
        cumsum[hook(0, ai)] = cumsum[hook(0, ai)] * 255 / norm_total;
      }

      atomic_add(&cumsum[hook(0, bi)], cumsum[hook(0, ai)]);
    }
    offset *= 2;
    barrier(0x02);
  }
