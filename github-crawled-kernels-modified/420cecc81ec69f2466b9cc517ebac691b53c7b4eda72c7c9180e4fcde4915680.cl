//{"field":4,"neg_div":2,"v_x":0,"v_y":1,"vals":3}
...
kernel void velocity_divergence(global float* v_x, global float* v_y, global float* neg_div) {
  ...
  float divergence = v_x[hook(0, hi)] - v_x[hook(0, low)];

  low = elem_index(id.x, id.y, dim.y + 1, dim.x);
  hi = elem_index(id.x, id.y + 1, dim.y + 1, dim.x);
  divergence += v_y[hook(1, hi)] - v_y[hook(1, low)];
  neg_div[hook(2, id.x + id.y * dim.x)] = -divergence;
}