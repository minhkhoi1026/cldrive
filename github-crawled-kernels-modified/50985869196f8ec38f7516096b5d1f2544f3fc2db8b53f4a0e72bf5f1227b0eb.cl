...
kernel void mtgp32_single_kernel(global struct mtgp32_kernel_status_t* d_status, global float* d_data, local unsigned int* status, constant struct mtgp32_param_t* params, int size) {
  ...
  for (int i = 0; i < size; i += (256 * 3)) {
    ...
    status[hook(2, tid)] = r;
    o = temper_single(r, status[hook(2, (256 * 3) - 351 + tid + pos - 1)], bid, params);
    ...
    o = temper_single(r, status[hook(2, (4 * 256 - 351 + tid + pos - 1) % (256 * 3))], bid, params);
    ...
    o = temper_single(r, status[hook(2, tid + pos - 1 + 2 * 256 - 351)], bid, params);
