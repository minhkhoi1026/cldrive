//{"coupling":1,"deriv":3,"params":2,"state":0}
...
kernel void dfun(global float* state, global float* coupling, global float* params, global float* deriv) {
  ...
  float3 xi = vload3(i, state), eta = vload3(i, state + 3 * n), tau = vload3(i, state + 6 * n), alpha = vload3(i, state + 9 * n), beta = vload3(i, state + 12 * n), gamma = vload3(i, state + 15 * n);

  float c_0 = coupling[hook(1, i)];

  float r = params[hook(2, i)];
  float s = params[hook(2, n + i)];
  float K11 = params[hook(2, 2 * n + i)];
  float K12 = params[hook(2, 3 * n + i)];
  float K21 = params[hook(2, 4 * n + i)];
  ...
  float3 deriv1 = (eta - a_i * pow(xi, 3) + b_i * pow(xi, 2) - tau + K11 * ((float3)(dot(xi, Aik_0), dot(xi, Aik_1), dot(xi, Aik_2)) - xi) - K12 * ((float3)(dot(alpha, Bik_0), dot(alpha, Bik_1), dot(alpha, Bik_2)) - xi) + IE_i + c_0 + local_coupling * xi);

  float3 deriv2 = c_i - d_i * pow(xi, 2) - eta;

  float3 deriv3 = r * s * xi - r * tau - m_i;

  float3 deriv4 = beta - e_i * pow(alpha, 3) + f_i * pow(alpha, 2) - gamma + K21 * ((float3)(dot(xi, Cik_0), dot(xi, Cik_1), dot(xi, Cik_2)) - alpha) + II_i + c_0 + local_coupling * xi;
