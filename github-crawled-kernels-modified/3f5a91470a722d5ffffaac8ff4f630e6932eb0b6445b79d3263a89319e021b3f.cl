...
kernel void shuffle_state2(global unsigned int* state, local unsigned int* cache) {
  ...
  state[hook(0, id * 4 + 0)] = cache[hook(1, id + 0)];
  state[hook(0, id * 4 + 1)] = cache[hook(1, id + 32)];
  state[hook(0, id * 4 + 2)] = cache[hook(1, id + 64)];
  state[hook(0, id * 4 + 3)] = cache[hook(1, id + 96)];
}