...
kernel void integrateBodies_noMT(global float4* newPos, global float4* newVel, global float4* oldPos, global float4* oldVel, float deltaTime, float damping, float softeningSquared, int numBodies, local float4* sharedPos) {
  unsigned int threadIdxx = get_local_id(0);
  unsigned int threadIdxy = get_local_id(1);
  unsigned int blockIdxx = get_group_id(0);
  unsigned int blockIdxy = get_group_id(1);
