...
kernel void merge_vector_C4_D5(global float* mat_dst, int dst_step, int dst_offset, global float* mat_src0, int src0_step, int src0_offset, global float* mat_src1, int src1_step, int src1_offset, global float* mat_src2, int src2_step, int src2_offset, global float* mat_src3, int src3_step, int src3_offset, int rows, int cols, int dst_step1) {
  ...
  if ((x < cols) && (y < rows)) {
    ...
    float src2 = *((global float*)((global uchar*)mat_src2 + src2_index + (x << 2)));
    float src3 = *((global float*)((global uchar*)mat_src3 + src3_index + (x << 2)));

    *((global float4*)((global uchar*)mat_dst + dst_index + (x << 4))) = (float4)(src0, src1, src2, src3);
  }
}