...
kernel void PreScanKernel(global int* output_data, global const int* input_data, local int* shared_data, const unsigned int group_index, const unsigned int base_index, const unsigned int n, const unsigned int addr_offset) {
  const unsigned int group_id = get_global_id(0) / get_local_size(0);
  const unsigned int group_size = get_local_size(0);
