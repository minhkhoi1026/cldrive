...
kernel void reduce_max_ui(global uint4* in, global unsigned int* out, local unsigned int* data, unsigned int n) {
  ...
  uint4 a = select((uint4)(0), in[hook(0, gY * n + 2 * gX)], flag);
  flag = (uint4)(2 * gX + 1) < (uint4)(n);
  uint4 b = select((uint4)(0), in[hook(0, gY * n + 2 * gX + 1)], flag);

  uint2 ta = select(a.hi, a.lo, a.lo > a.hi);
  uint2 tb = select(b.hi, b.lo, b.lo > b.hi);

  data[hook(2, 2 * lX)] = select(ta.y, ta.x, ta.x > ta.y);
  data[hook(2, 2 * lX + 1)] = select(tb.y, tb.x, tb.x > tb.y);
