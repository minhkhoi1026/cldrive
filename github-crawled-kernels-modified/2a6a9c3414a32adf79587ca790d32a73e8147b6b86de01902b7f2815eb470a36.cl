//{"baryCoords":10,"bindMatrices":11,"drivenInverseMatrix":17,"drivenMatrices":12,"drivenMatrix":16,"driverNormals":3,"driverPoints":2,"envelope":13,"finalPos":0,"initialPos":1,"paintWeights":4,"positionCount":14,"sampleCounts":5,"sampleIds":7,"sampleOffsets":6,"sampleWeights":8,"scaleMatrix":15,"triangleVerts":9}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void cvwrap(global float* finalPos, global const float* initialPos, global const float* driverPoints, global const float* driverNormals, global const float* paintWeights, global const int* sampleCounts, global const int* sampleOffsets, global const int* sampleIds, global const float* sampleWeights, global const int* triangleVerts, global const float* baryCoords, global const float4* bindMatrices, global const float4* drivenMatrices, const float envelope, const unsigned int positionCount) {
  ...
  global const float4* scaleMatrix = &(drivenMatrices[hook(12, 8)]);
  float4 scaleMatrix0 = (float4)(dot(scaleMatrix[hook(15, 0)], matrix0), dot(scaleMatrix[hook(15, 0)], matrix1), dot(scaleMatrix[hook(15, 0)], matrix2), dot(scaleMatrix[hook(15, 0)], matrix3));
  float4 scaleMatrix1 = (float4)(dot(scaleMatrix[hook(15, 1)], matrix0), dot(scaleMatrix[hook(15, 1)], matrix1), dot(scaleMatrix[hook(15, 1)], matrix2), dot(scaleMatrix[hook(15, 1)], matrix3));
  float4 scaleMatrix2 = (float4)(dot(scaleMatrix[hook(15, 2)], matrix0), dot(scaleMatrix[hook(15, 2)], matrix1), dot(scaleMatrix[hook(15, 2)], matrix2), dot(scaleMatrix[hook(15, 2)], matrix3));
  float4 scaleMatrix3 = (float4)(dot(scaleMatrix[hook(15, 3)], matrix0), dot(scaleMatrix[hook(15, 3)], matrix1), dot(scaleMatrix[hook(15, 3)], matrix2), dot(scaleMatrix[hook(15, 3)], matrix3));
  ...
  global const float4* drivenInverseMatrix = &(drivenMatrices[hook(12, 4)]);
  global const float4* drivenMatrix = drivenMatrices;
  float4 worldPt = (float4)(dot(initialPosition, drivenMatrix[hook(16, 0)]), dot(initialPosition, drivenMatrix[hook(16, 1)]), dot(initialPosition, drivenMatrix[hook(16, 2)]), dot(initialPosition, drivenMatrix[hook(16, 3)]));
  ...
  float3 newPt = (float3)(dot(worldPt, drivenInverseMatrix[hook(17, 0)]), dot(worldPt, drivenInverseMatrix[hook(17, 1)]), dot(worldPt, drivenInverseMatrix[hook(17, 2)]));
