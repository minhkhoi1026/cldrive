...
kernel void gemm(const global float* A, const global float* B, global float* C, const int M, const int N, const int K) {
 ...
 private
  ...
  for (int i = 0; i < K / NUM_UNROLL; ++i) {
    ...
    tileB[hook(8, local_thread_id + 0 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 0)];
    tileB[hook(8, local_thread_id + 1 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 1)];
    tileB[hook(8, local_thread_id + 2 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 2)];
    tileB[hook(8, local_thread_id + 3 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 3)];
    tileB[hook(8, local_thread_id + 4 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 4)];
    tileB[hook(8, local_thread_id + 5 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 5)];
    tileB[hook(8, local_thread_id + 6 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 6)];
    tileB[hook(8, local_thread_id + 7 * tile_dim_y)] = B[hook(1, (block_id_y * tile_dim_y + local_thread_id) * K + i * NUM_UNROLL + 7)];
