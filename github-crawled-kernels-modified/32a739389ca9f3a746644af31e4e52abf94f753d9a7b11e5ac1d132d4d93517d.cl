  ...
  if (is_division) {
    for (unsigned int col = col_gid; col < A_size2; col += get_num_groups(0))
      for (unsigned int row = row_gid; row < A_size1; row += get_local_size(0))
        A[hook(0, (row * A_inc1 + A_start1) + (col * A_inc2 + A_start2) * A_internal_size1)] = B[hook(9, (row * B_inc1 + B_start1) + (col * B_inc2 + B_start2) * B_internal_size1)] / C[hook(16, (row * C_inc1 + C_start1) + (col * C_inc2 + C_start2) * C_internal_size1)];
  } else {
    for (unsigned int col = col_gid; col < A_size2; col += get_num_groups(0))
      for (unsigned int row = row_gid; row < A_size1; row += get_local_size(0))
        A[hook(0, (row * A_inc1 + A_start1) + (col * A_inc2 + A_start2) * A_internal_size1)] = B[hook(9, (row * B_inc1 + B_start1) + (col * B_inc2 + B_start2) * B_internal_size1)] * C[hook(16, (row * C_inc1 + C_start1) + (col * C_inc2 + C_start2) * C_internal_size1)];
