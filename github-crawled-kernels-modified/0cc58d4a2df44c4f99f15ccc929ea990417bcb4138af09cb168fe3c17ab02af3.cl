//{"d_prefixoffsets":4,"indice":1,"input":0,"l_offsets":5,"output":2,"s_offset":6,"size":3}
...
kernel void bucketsort(global float* input, global int* indice, global float* output, const int size, global unsigned int* d_prefixoffsets, global unsigned int* l_offsets) {
  volatile local unsigned int s_offset[((1 << 10) * (1))];

  int prefixBase = get_group_id(0) * ((1 << 10) * (1));
  ...
  for (int i = get_local_id(0); i < ((1 << 10) * (1)); i += get_local_size(0)) {
    s_offset[hook(6, i)] = l_offsets[hook(5, i & ((1 << 10) - 1))] + d_prefixoffsets[hook(4, prefixBase + i)];
  }
  ...
  for (int tid = get_global_id(0); tid < size; tid += numThreads) {
    ...
    output[hook(2, s_offset[whook(6, warpBase + (id & ((1 << 10) - 1))) + (id >> (10)))] = elem;
    int test = s_offset[hook(6, warpBase + (id & ((1 << 10) - 1)))] + (id >> (10));
  }
}