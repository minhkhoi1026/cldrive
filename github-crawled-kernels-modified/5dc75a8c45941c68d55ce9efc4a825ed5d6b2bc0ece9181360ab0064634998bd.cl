...
kernel void init(global char* trit_hash, global long* mid_low, global long* mid_high, global long* state_low, global long* state_high, constant size_t* min_weight_magnitude, global volatile char* found, global long* nonce_probe, constant size_t* loop_count) {
 private
  size_t i, j, id, gid, gr_id, gl_off, l_size, n_trits;
  ...
  for (i = 0; i < n_trits; i++) {
    j = id + i * l_size;
    mid_low[hook(1, gid + j)] = mid_low[hook(1, j)];
    mid_high[hook(2, gid + j)] = mid_high[hook(2, j)];
  }

  if (id == 0) {
    for (i = 0; i < gr_id + gl_off; i++) {
      increment(&(mid_low[hook(1, gid)]), &(mid_high[hook(2, gid)]), 243 - 243 / 3 + 4, 243 - 243 / 3 + 4 + 243 / 3 / 3);
    }
  }
}