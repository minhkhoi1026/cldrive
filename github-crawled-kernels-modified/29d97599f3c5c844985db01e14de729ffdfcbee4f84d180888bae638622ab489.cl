...
kernel void arithm_flip_rows(global T* src, int src_step, int src_offset, global T* dst, int dst_step, int dst_offset, int rows, int cols, int thread_rows, int thread_cols) {
  ...
  if (x < cols && y < thread_rows) {
    ...
    dst[hook(10, dst_index_0)] = data1;
    dst[hook(10, dst_index_1)] = data0;
  }
}