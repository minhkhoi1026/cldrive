...
kernel void reduce_min_f(global float4* in, global float* out, local float* data, unsigned int n) {
  ...
  float4 a = select((float4)((__builtin_inff())), in[hook(0, gY * n + 2 * gX)], flag);
  flag = (uint4)(2 * gX + 1) < (uint4)(n);
  float4 b = select((float4)((__builtin_inff())), in[hook(0, gY * n + 2 * gX + 1)], flag);

  float2 ta = fmin(a.lo, a.hi);
  float2 tb = fmin(b.lo, b.hi);

  data[hook(2, 2 * lX)] = fmin(ta.x, ta.y);
  data[hook(2, 2 * lX + 1)] = fmin(tb.x, tb.y);
