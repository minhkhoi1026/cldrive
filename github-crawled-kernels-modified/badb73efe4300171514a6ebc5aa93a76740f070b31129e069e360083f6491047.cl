//{"g_qs":4,"g_rho_i":5,"g_rhs":7,"g_square":6,"g_u":0,"g_us":1,"g_vs":2,"g_ws":3,"nx2":8,"ny2":9,"nz2":10,"qs":44,"qs[k]":43,"qs[k][j + 1]":42,"qs[k][j - 1]":46,"qs[k][j]":45,"rho_i":49,"rho_i[k]":48,"rho_i[k][j + 1]":47,"rho_i[k][j - 1]":51,"rho_i[k][j]":50,"rhs":19,"rhs[k]":18,"rhs[k][j]":17,"rhs[k][j][i]":16,"square":35,"square[k]":34,"square[k][j + 1]":33,"square[k][j - 1]":36,"u":23,"u[k]":22,"u[k][j + 1]":21,"u[k][j + 1][i]":20,"u[k][j + 2]":53,"u[k][j + 2][i]":52,"u[k][j - 1]":27,"u[k][j - 1][i]":26,"u[k][j - 2]":55,"u[k][j - 2][i]":54,"u[k][j]":25,"u[k][j][i]":24,"us":30,"us[k]":29,"us[k][j + 1]":28,"us[k][j - 1]":32,"us[k][j]":31,"vs":13,"vs[k]":12,"vs[k][j + 1]":14,"vs[k][j - 1]":15,"vs[k][j]":11,"ws":39,"ws[k]":38,"ws[k][j + 1]":37,"ws[k][j - 1]":41,"ws[k][j]":40}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void compute_rhs4(global const double* g_u, global const double* g_us, global const double* g_vs, global const double* g_ws, global const double* g_qs, global const double* g_rho_i, global const double* g_square, global double* g_rhs, int nx2, int ny2, int nz2) {
  int i, j, k, m;
  double vijk, vp1, vm1;
  ...
  j = ny2;
  for (m = 0; m < 5; m++) {
    rhs[hook(19, k)][hook(18, j)][hook(17, i)][hook(16, m)] = rhs[hook(19, k)][hook(18, j)][hook(17, i)][hook(16, m)] - (0.25 * ((0.75) > (((0.75) > (1.0) ? (0.75) : (1.0))) ? (0.75) : (((0.75) > (1.0) ? (0.75) : (1.0))))) * (u[hook(23, k)][hook(22, j - 2)][hook(55, i)][hook(54, m)] - 4.0 * u[hook(23, k)][hook(22, j - 1)][hook(27, i)][hook(26, m)] + 5.0 * u[hook(23, k)][hook(22, j)][hook(25, i)][hook(24, m)]);
  }
}