//{"Nq":2,"background":4,"edge_sep":6,"num_pearls":7,"pearl_sld":8,"q":0,"radius":5,"result":1,"scale":3,"solvent_sld":9}
...
kernel void linear_pearls_Iq(global const float* q, global float* result, const int Nq, const float scale, const float background, const float radius, const float edge_sep, const float num_pearls, const float pearl_sld, const float solvent_sld) {
  int i = get_global_id(0);
  if (i < Nq)

  {
    const float qi = q[hook(0, i)];
    result[hook(1, i)] = scale * Iq(qi, radius, edge_sep, num_pearls, pearl_sld, solvent_sld) + background;
  }
}