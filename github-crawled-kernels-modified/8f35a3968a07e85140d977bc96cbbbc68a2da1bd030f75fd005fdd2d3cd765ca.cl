//{"AR1_Estimates":2,"AR2_Estimates":3,"AR3_Estimates":4,"AR4_Estimates":5,"Cxx":13,"Cxx[0]":12,"Cxx[1]":14,"Cxx[2]":15,"Cxx[3]":16,"DATA_D":9,"DATA_H":8,"DATA_T":10,"DATA_W":7,"Mask":6,"Whitened_fMRI_Volumes":0,"fMRI_Volumes":1,"inv_Cxx":18,"inv_Cxx[0]":17,"inv_Cxx[1]":19,"inv_Cxx[2]":20,"inv_Cxx[3]":21,"slice":11}
...
kernel void ApplyWhiteningAR4Slice(global float* Whitened_fMRI_Volumes, global float* fMRI_Volumes, global const float* AR1_Estimates, global const float* AR2_Estimates, global const float* AR3_Estimates, global const float* AR4_Estimates, global const float* Mask, private int DATA_W, private int DATA_H, private int DATA_D, private int DATA_T, private int slice) {
  ...
  float4 alphas;
  alphas.x = AR1_Estimates[hook(2, Calculate3DIndex(x, y, slice, DATA_W, DATA_H))];
  alphas.y = AR2_Estimates[hook(3, Calculate3DIndex(x, y, slice, DATA_W, DATA_H))];
  alphas.z = AR3_Estimates[hook(4, Calculate3DIndex(x, y, slice, DATA_W, DATA_H))];
  alphas.w = AR4_Estimates[hook(5, Calculate3DIndex(x, y, slice, DATA_W, DATA_H))];

  old_value_1 = fMRI_Volumes[hook(1, Calculate3DIndex(x, y, 0, DATA_W, DATA_H))];
  Whitened_fMRI_Volumes[hook(0, Calculate3DIndex(x, y, 0, DATA_W, DATA_H))] = old_value_1;
  old_value_2 = fMRI_Volumes[hook(1, Calculate3DIndex(x, y, 1, DATA_W, DATA_H))];
  Whitened_fMRI_Volumes[hook(0, Calculate3DIndex(x, y, 1, DATA_W, DATA_H))] = old_value_2 - alphas.x * old_value_1;
  old_value_3 = fMRI_Volumes[hook(1, Calculate3DIndex(x, y, 2, DATA_W, DATA_H))];
  Whitened_fMRI_Volumes[hook(0, Calculate3DIndex(x, y, 2, DATA_W, DATA_H))] = old_value_3 - alphas.x * old_value_2 - alphas.y * old_value_1;
  old_value_4 = fMRI_Volumes[hook(1, Calculate3DIndex(x, y, 3, DATA_W, DATA_H))];
  Whitened_fMRI_Volumes[hook(0, Calculate3DIndex(x, y, 3, DATA_W, DATA_H))] = old_value_4 - alphas.x * old_value_3 - alphas.y * old_value_2 - alphas.z * old_value_1;

  for (t = 4; t < DATA_T; t++) {
    old_value_5 = fMRI_Volumes[hook(1, Calculate3DIndex(x, y, t, DATA_W, DATA_H))];
    Whitened_fMRI_Volumes[hook(0, Calculate3DIndex(x, y, t, DATA_W, DATA_H))] = old_value_5 - alphas.x * old_value_4 - alphas.y * old_value_3 - alphas.z * old_value_2 - alphas.w * old_value_1;
