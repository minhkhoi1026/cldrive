//{"Gauss76Wt":18,"Gauss76Z":17,"Nlength":9,"Nphi":11,"Nq":3,"Nradius":7,"Ntheta":10,"Nthickness":8,"background":13,"core_sld":14,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":12,"shell_sld":15,"solvent_sld":16}
...
float Iq(float q, float core_sld, float shell_sld, float solvent_sld, float radius, float thickness, float length) {
  ...
  for (int i = 0; i < 76; i++) {
    float sn, cn;
    const float alpha = 0.5f * (Gauss76Z[hook(17, i)] * 0x1.921fb54442d18p+0 + 0x1.921fb54442d18p+0);
    do {
    ...
    total += Gauss76Wt[hook(18, i)] * fq * fq * sn;
  }
...
kernel void core_shell_cylinder_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                                     global float* loops_g,

                                     local float* loops, const float cutoff, const int Nradius, const int Nthickness, const int Nlength, const int Ntheta, const int Nphi,

                                     const float scale, const float background, const float core_sld, const float shell_sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nradius + Nthickness + Nlength + Ntheta + Nphi) * 2, 0);
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int thickness_i = 0; thickness_i < Nthickness; thickness_i++) {
        const float thickness = loops[hook(5, 2 * (thickness_i + Nradius))];
        const float thickness_w = loops[hook(5, 2 * (thickness_i + Nradius) + 1)];
        for (int length_i = 0; length_i < Nlength; length_i++) {
          const float length = loops[hook(5, 2 * (length_i + Nradius + Nthickness))];
          const float length_w = loops[hook(5, 2 * (length_i + Nradius + Nthickness) + 1)];
          for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
            const float theta = loops[hook(5, 2 * (theta_i + Nradius + Nthickness + Nlength))];
            const float theta_w = loops[hook(5, 2 * (theta_i + Nradius + Nthickness + Nlength) + 1)];
            for (int phi_i = 0; phi_i < Nphi; phi_i++) {
              const float phi = loops[hook(5, 2 * (phi_i + Nradius + Nthickness + Nlength + Ntheta))];
              const float phi_w = loops[hook(5, 2 * (phi_i + Nradius + Nthickness + Nlength + Ntheta) + 1)];

              const float weight = radius_w * thickness_w * length_w * theta_w * phi_w;
              if (weight > cutoff) {
                const float scattering = Iqxy(qxi, qyi, core_sld, shell_sld, solvent_sld, radius, thickness, length, theta, phi);
                if (!isnan(scattering)) {
