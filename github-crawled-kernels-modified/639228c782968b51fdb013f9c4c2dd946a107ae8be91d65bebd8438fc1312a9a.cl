...
kernel void _XACC_pack_vector_16(global short* dst, unsigned long dst_off_e, global const short* src, unsigned long src_off_e, unsigned long blocklength_e, unsigned long stride_e, unsigned long count) {
  ...
  for (c = c_init; c < count; c += c_step) {
    for (b = b_init; b < blocklength_e; b += b_step) {
      dst[hook(0, dst_off_e + blocklength_e * c + b)] = src[hook(2, src_off_e + stride_e * c + b)];
    }
  }
}