//{"delta":8,"in":9,"inHeight":2,"inPixelPitch":4,"inRowPitch":6,"inWidth":0,"out":10,"outHeight":3,"outPixelPitch":5,"outRowPitch":7,"outWidth":1}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void affineTrans(const int inWidth, const int outWidth, const int inHeight, const int outHeight, const int inPixelPitch, const int outPixelPitch, const int inRowPitch, const int outRowPitch, const float delta, global const unsigned char* in, global unsigned char* out) {
  ...
  if (inFixY >= 0 && inFixY < (abs(inHeight) - 1) && inFixX >= 0 && inFixX < (inWidth - 1)) {
    ...
    for (int rgb = 0; rgb < 3; rgb++)
      out[hook(10, dst + rgb)] = convert_uchar_sat((int)((1.0f - q) * ((1.0f - p) * (float)in[hook(9, src00 + rgb)] + p * (float)in[hook(9, src01 + rgb)]) + q * ((1.0f - p) * (float)in[hook(9, src10 + rgb)] + p * (float)in[hook(9, src11 + rgb)])));
  }
}