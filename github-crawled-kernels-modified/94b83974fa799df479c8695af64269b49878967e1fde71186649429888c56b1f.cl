//{"dest":1,"dst_step":3,"levels":5,"source":0,"src_step":2,"values":6,"width":4}
...
kernel void lut_256(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, constant const uchar* levels) {
  ...
  if (4 > 1 && gx + 4 > width) {
    for (int x = gx; x < width; x++)
      dest[hook(1, gy * dst_step + x)] = levels[hook(5, source[ghook(0, gy * src_step + x))];
