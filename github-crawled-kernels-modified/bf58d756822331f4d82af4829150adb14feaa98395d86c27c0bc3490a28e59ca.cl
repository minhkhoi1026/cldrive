...
kernel void init_data_cost_1(global float* ctemp, global uchar* cleft, global uchar* cright, int h, int w, int level, int channels, int cmsg_step1, float cdata_weight, float cmax_data_term, int cdisp_step1, int cth, int cimg_step, int cndisp) {
  ...
  if (y < h && x < w) {
    ...
    for (int d = 0; d < cndisp; ++d) {
      ...
      data_cost[hook(16, cdisp_step1 * d)] = val;
    }
  }
}