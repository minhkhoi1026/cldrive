...
kernel void sobel3(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height) {
  constant const float matrixH[9] = {-1, -2, -1, 0, 0, 0, 1, 2, 1};

  constant const float matrixV[9] = {1, 0, -1, 2, 0, -2, 1, 0, -1};
  ...
  float Result = Combine(sumH, sumV);
