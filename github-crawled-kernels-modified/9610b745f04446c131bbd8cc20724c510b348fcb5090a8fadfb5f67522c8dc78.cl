...
kernel void lud_internal(global float* m, local float* peri_row, local float* peri_col, int matrix_dim, int offset) {
  ...
  peri_row[hook(1, ty * 16 + tx)] = m[hook(0, (offset + ty) * matrix_dim + global_col_id + tx)];
  peri_col[hook(2, ty * 16 + tx)] = m[hook(0, (global_row_id + ty) * matrix_dim + offset + tx)];

  barrier(0x01);

  sum = 0;
  for (i = 0; i < 16; i++)
    sum += peri_col[hook(2, ty * 16 + i)] * peri_row[hook(1, i * 16 + tx)];
