...
kernel void clBlockPrefix(global uint4* blockScan, global unsigned int* blockSum, unsigned int size, local unsigned int* prefixSum) {
  ...
  if (blockid > 0 && globalId < size) {
    blockScan[hook(0, globalId)] += prefixSum[hook(3, blockid - 1)];
  }
}