...
__attribute__((reqd_work_group_size(7, 7, 2))) __attribute__((reqd_work_group_size(14, 14, 2))) __attribute__((reqd_work_group_size(2, 2, 4))) kernel void conv3(
          ...
          for (int i = 0; i < 1; ++i) {
            ...
            for (int x = 0; x < 5; ++x) {
              for (int y = 0; y < 5; ++y, ++weightIdx) {
                outPrivate[hook(6, oPrivateIdx)] += inLocal[hook(2, ((i + iTile) * 14 + r + rTile + x) * 14 + c + cTile + y)] * weightLocal[hook(4, ((o + oTile) * 6 + i + iTile) * 25 + weightIdx)];
              }
            }
          }
        }
  ...
  for (int r = 0; r < 5; ++r) {
    for (int c = 0; c < 5; ++c) {
      for (int o = 0; o < 4; ++o, ++oPrivateIdx) {
        buf3[hook(7, ((o + oTile) * 10 + r + rTile) * 10 + c + cTile)] = sigmod(outPrivate[hook(6, oPrivateIdx)] + offsetLocal[hook(5, o + oTile)]);
      }
    }
  }
}