...
kernel void compute_message_0(global short* u_, global short* d_, global short* l_, global short* r_, global const short* data_cost_selected, global const short* selected_disp_pyr_cur, global short* ctemp, int h, int w, int nr_plane, int i, float cmax_disc_term, int cdisp_step1, int cmsg_step1, float cdisc_single_jump) {
  ...
  if (y > 0 && y < h - 1 && x > 0 && x < w - 1) {
    ...
    global short* u = u_ + y * cmsg_step1 + x;
    global short* d = d_ + y * cmsg_step1 + x;
    global short* l = l_ + y * cmsg_step1 + x;
    global short* r = r_ + y * cmsg_step1 + x;

    global const short* disp = selected_disp_pyr_cur + y * cmsg_step1 + x;

    global short* temp = ctemp + y * cmsg_step1 + x;

    message_per_pixel_0(data, u, r - 1, u + cmsg_step1, l + 1, disp, disp - cmsg_step1, nr_plane, temp, cmax_disc_term, cdisp_step1, cdisc_single_jump);
    message_per_pixel_0(data, d, d - cmsg_step1, r - 1, l + 1, disp, disp + cmsg_step1, nr_plane, temp, cmax_disc_term, cdisp_step1, cdisc_single_jump);
    message_per_pixel_0(data, l, u + cmsg_step1, d - cmsg_step1, l + 1, disp, disp - 1, nr_plane, temp, cmax_disc_term, cdisp_step1, cdisc_single_jump);
    message_per_pixel_0(data, r, u + cmsg_step1, d - cmsg_step1, r - 1, disp, disp + 1, nr_plane, temp, cmax_disc_term, cdisp_step1, cdisc_single_jump);
  }
}