...
kernel void f_dz_dt(global struct state* states_old, global struct state* states_new, const float dt) {
  ...
  states_new[hook(1, idx)].z = state_0.z + dt * (f1 + 2.0f * f2 + 2.0f * f3 + f4) / 6.0f;
}