//{"colid":1,"data":2,"midrow":6,"result":4,"rowallsize":5,"rowinfo":7,"rowptr":0,"start":8,"vec":3}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void cpu_csr_ve_slm_pm_fs(global int* rowptr, global int* colid, global float* data, global float* vec, global float* result, int rowallsize, int midrow, global int* rowinfo, int start) {
