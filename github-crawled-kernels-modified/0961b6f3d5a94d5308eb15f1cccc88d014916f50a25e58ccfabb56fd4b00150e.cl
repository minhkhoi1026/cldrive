...
kernel void amdFHT2048FromSXFade(global const float* in1, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int frame_ln, int current_bin, global const float* in2, global const float* out2, int index2, int chnl_stride2) {
  int lcl_id = get_local_id(0);
  int chnl = channels_map[hook(4, get_group_id(1))];
  int chnl_off = chnl_stride * chnl;
  local float data1[2048];
  local float data2[2048];
  ...
  for (int i = lcl_id; i < 2048 / 2; i += 256) {
    float fadeInCoeff = (float)i / (float)(2048 / 2 - 1);
