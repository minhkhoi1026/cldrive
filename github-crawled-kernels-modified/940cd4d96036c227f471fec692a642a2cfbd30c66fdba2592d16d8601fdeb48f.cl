...
void dsum_reduce(int n, int i, local float* x);
void dsum_reduce(int n, int i, local float* x) {
  barrier(0x01);
  if (n > 1024) {
    if (i < 1024 && i + 1024 < n) {
...
kernel void magmablas_snrm2_kernel(int m, global float* da, int da_offset, int ldda, global float* dxnorm, int dxnorm_offset) {
  da += da_offset;
