//{"depth":6,"gInput":0,"gInputImage":7,"height":5,"index":3,"indexBuffer":2,"outputArr":1,"width":4}
...
float sampleZeroPaddedFloat(const global float* gInputImage, int offset, int x, int y, int width, int height) {
  if (x < 0 || x >= width || y < 0 || y >= height)
    return 0.0f;
  return gInputImage[hook(7, offset + (y * width + x))];
}
...
float getClampedFloat3d(const global float* gInputImage, int x, int y, int z, int width, int height, int depth) {
  ...
  if (z >= depth)
    z = depth - 1;
  return gInputImage[hook(7, z * width * height + y * width + x)];
}
...
kernel void resolveRoIIndexBuffer(const global float* gInput, global float* outputArr, global float* indexBuffer, int index, int width, int height, int depth) {
  float originX = indexBuffer[hook(2, index)];
  float originY = indexBuffer[hook(2, index + 1)];
