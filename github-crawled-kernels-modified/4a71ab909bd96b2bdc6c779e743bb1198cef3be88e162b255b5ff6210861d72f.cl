...
__attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) kernel void distribute_part_sum_int4(global int* in_sums, global int4* inout_array, unsigned int numElems) {
  int globalId = get_global_id(0);
  int groupId = get_group_id(0);
  int4 v1 = safe_load_int4(inout_array, globalId, numElems);
  int sum = in_sums[hook(0, groupId >> 1)];
  v1.xyzw += sum;
  safe_store_int4(v1, inout_array, globalId, numElems);
}