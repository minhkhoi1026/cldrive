...
kernel void integral_sum_rows_D5(global float4* srcsum, global float* sum, int rows, int cols, int src_step, int sum_step, int sum_offset) {
  ...
  for (int i = 0; i < rows; i = i + 255) {
    src_t[hook(7, 0)] = i + lid < rows ? srcsum[hook(0, (lid + i) * src_step + gid * 2)] : (float4)0;
    src_t[hook(7, 1)] = i + lid < rows ? srcsum[hook(0, (lid + i) * src_step + gid * 2 + 1)] : (float4)0;

    sum_t[hook(8, 0)] = (i == 0 ? (float4)0 : lm_sum[hook(10, 0)][hook(9, 254 + 8)]);
    sum_t[hook(8, 1)] = (i == 0 ? (float4)0 : lm_sum[hook(10, 1)][hook(11, 254 + 8)]);
    barrier(0x01);
    ...
    if (lid > 0 && (i + lid) <= rows) {
      ...
      sum_p = (local float*)(&(lm_sum[hook(10, 0)][hook(9, bf_loc)]));
      for (int k = 0; k < 4; k++) {
        if (gid * 8 + k >= cols)
          break;
        sum[hook(1, loc_s0 + k * sum_step / 4)] = sum_p[hook(14, k)];
      }
      sum_p = (local float*)(&(lm_sum[hook(10, 1)][hook(11, bf_loc)]));
      for (int k = 0; k < 4; k++) {
        if (gid * 8 + 4 + k >= cols)
          break;
