...
kernel void normalizeKernel_C1_D0(global const float* img_sqsums, global float* res, ulong tpl_sqsum, int res_rows, int res_cols, int tpl_rows, int tpl_cols, int img_sqsums_offset, int img_sqsums_step, int res_offset, int res_step) {
  ...
  if (gidx < res_cols && gidy < res_rows) {
    ...
    res[hook(1, res_idx)] = normAcc(res[hook(1, res_idx)], sqrt(image_sqsum_ * tpl_sqsum));
  }
}