//{"class0offset":2,"class1offset":3,"class2offset":4,"gInput":0,"gInputImage":5,"outputArr":1}
...
float sampleZeroPaddedFloat(const global float* gInputImage, int offset, int x, int y, int width, int height) {
  if (x < 0 || x >= width || y < 0 || y >= height)
    return 0.0f;
  return gInputImage[hook(5, offset + (y * width + x))];
}
...
float getClampedFloat3d(const global float* gInputImage, int x, int y, int z, int width, int height, int depth) {
  ...
  if (z >= depth)
    z = depth - 1;
  return gInputImage[hook(5, z * width * height + y * width + x)];
}
...
kernel void toLabelImage3to2ClassesConf(const global float* gInput, global float* outputArr, float class0offset, float class1offset, float class2offset) {
  ...
  float prob2 = gInput[hook(0, widthheight * 2 + index)] + class2offset;
  if (prob1 > prob2)
    outputArr[hook(1, index)] = prob1;
  else
    outputArr[hook(1, index)] = prob2;
}