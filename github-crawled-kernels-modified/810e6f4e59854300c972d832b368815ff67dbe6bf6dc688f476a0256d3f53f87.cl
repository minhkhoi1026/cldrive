...
kernel void gpu_bcsr_red_18(global int* rowptr, global int* colid, global float4* data, global float4* vec, global float* result, int data_align) {
...

  {
    ...
    for (int i = start + localid; i < end; i += 128) {
      ...
      vecelem = vec[hook(3, col + 1)];
      matelem = data[hook(2, i + data_align)];
      sum = mad(matelem, vecelem, sum);
    }
