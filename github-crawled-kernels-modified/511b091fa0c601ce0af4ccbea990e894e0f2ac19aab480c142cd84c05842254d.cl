//{"gaussianBlur":4,"output":0,"pixels":3,"xRes":1,"yRes":2}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void gaussianBlurKernel2(global float* restrict output, const int xRes, const int yRes, global const float* restrict pixels, const int gaussianBlur) {
  ...
  output[hook(0, y * xRes * 3 + x * 3 + 0)] = r;
  output[hook(0, y * xRes * 3 + x * 3 + 1)] = g;
  output[hook(0, y * xRes * 3 + x * 3 + 2)] = b;
}