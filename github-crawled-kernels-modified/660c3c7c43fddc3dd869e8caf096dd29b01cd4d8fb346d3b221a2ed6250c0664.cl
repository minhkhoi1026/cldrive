//{"buffer":4,"gridResolution":0,"inputVelocityBuffer":1,"outputVelocityBuffer":3,"pressureBuffer":2}
...
kernel void projection(const int gridResolution, global float2* inputVelocityBuffer, global float* pressureBuffer, global float2* outputVelocityBuffer) {
  int2 id = (int2)(get_global_id(0), get_global_id(1));

  if (id.x > 0 && id.x < gridResolution - 1 && id.y > 0 && id.y < gridResolution - 1) {
    float pL = pressureBuffer[hook(2, id.x - 1 + id.y * gridResolution)];
    float pR = pressureBuffer[hook(2, id.x + 1 + id.y * gridResolution)];
    float pB = pressureBuffer[hook(2, id.x + (id.y - 1) * gridResolution)];
    float pT = pressureBuffer[hook(2, id.x + (id.y + 1) * gridResolution)];

    float2 velocity = inputVelocityBuffer[hook(1, id.x + id.y * gridResolution)];

    outputVelocityBuffer[hook(3, id.x + id.y * gridResolution)] = velocity - (float2)(pR - pL, pT - pB);
  } else {
    if (id.x == 0)
      outputVelocityBuffer[hook(3, id.x + id.y * gridResolution)] = -inputVelocityBuffer[hook(1, id.x + 1 + id.y * gridResolution)];
    if (id.x == gridResolution - 1)
      outputVelocityBuffer[hook(3, id.x + id.y * gridResolution)] = -inputVelocityBuffer[hook(1, id.x - 1 + id.y * gridResolution)];
    if (id.y == 0)
      outputVelocityBuffer[hook(3, id.x + id.y * gridResolution)] = -inputVelocityBuffer[hook(1, id.x + 1 + (id.y + 1) * gridResolution)];
