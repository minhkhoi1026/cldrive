//{"((__local unsigned int *)smem)":11,"((__local unsigned int *)src)":12,"global_histo":7,"global_overflow":8,"global_subhisto":6,"histo_height":4,"histo_width":5,"num_elements":1,"sm_mappings":0,"sm_range_max":3,"sm_range_min":2,"smem":10,"smem[indexhi]":9}
...
void copyMemory(global unsigned int* dst, local unsigned int src[2][256]) {
  for (int i = get_local_id(0), blockDimx = get_local_size(0); i < 8 / 4; i += blockDimx) {
    atom_add(dst + i * 4, (((local unsigned int*)src)[hook(12, i)] >> 0) & 0xFF);
    atom_add(dst + i * 4 + 1, (((local unsigned int*)src)[hook(12, i)] >> 8) & 0xFF);
    atom_add(dst + i * 4 + 2, (((local unsigned int*)src)[hook(12, i)] >> 16) & 0xFF);
    atom_add(dst + i * 4 + 3, (((local unsigned int*)src)[hook(12, i)] >> 24) & 0xFF);
  }
}
