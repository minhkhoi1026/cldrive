...
void segmented_parallel_reduction(unsigned int row, float val, local unsigned int* shared_rows, local float* inter_results) {
  shared_rows[hook(5, get_local_id(0))] = row;
  inter_results[hook(6, get_local_id(0))] = val;
  float left = 0;

  barrier(0x01);
  if (get_local_id(0) >= 1 && row == shared_rows[hook(5, get_local_id(0) - 1)]) {
    left = inter_results[hook(6, get_local_id(0) - 1)];
  }
  barrier(0x01);
  ...
  if (get_local_id(0) >= 2 && row == shared_rows[hook(5, get_local_id(0) - 2)]) {
    left = inter_results[hook(6, get_local_id(0) - 2)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 4 && row == shared_rows[hook(5, get_local_id(0) - 4)]) {
    left = inter_results[hook(6, get_local_id(0) - 4)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 8 && row == shared_rows[hook(5, get_local_id(0) - 8)]) {
    left = inter_results[hook(6, get_local_id(0) - 8)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 16 && row == shared_rows[hook(5, get_local_id(0) - 16)]) {
    left = inter_results[hook(6, get_local_id(0) - 16)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 32 && row == shared_rows[hook(5, get_local_id(0) - 32)]) {
    left = inter_results[hook(6, get_local_id(0) - 32)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 64 && row == shared_rows[hook(5, get_local_id(0) - 64)]) {
    left = inter_results[hook(6, get_local_id(0) - 64)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
  ...
  if (get_local_id(0) >= 128 && row == shared_rows[hook(5, get_local_id(0) - 128)]) {
    left = inter_results[hook(6, get_local_id(0) - 128)];
  }
  barrier(0x01);
  inter_results[hook(6, get_local_id(0))] += left;
  left = 0;
...
kernel void vec_mul(global const uint2* coords, global const float* elements, global const unsigned int* group_boundaries, global const float* vector, global float* result, local unsigned int* shared_rows, local float* inter_results) {
  ...
  unsigned int k_end = 1 + (group_end - group_start - 1) / get_local_size(0);
  ...
  for (unsigned int k = 0; k < k_end; ++k) {
    ...
    if (local_index < group_end) {
      tmp = coords[hook(0, local_index)];
      val = elements[hook(1, local_index)] * vector[hook(3, tmp.y)];
    } else {
      tmp.x = 0;
      tmp.y = 0;
      val = 0;
