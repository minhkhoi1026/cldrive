...
kernel void matchTemplate_Naive_SQDIFF_C1_D5(global const float* img, global const float* tpl, global float* res, int img_rows, int img_cols, int tpl_rows, int tpl_cols, int res_rows, int res_cols, int img_offset, int tpl_offset, int res_offset, int img_step, int tpl_step, int res_step) {
  ...
  if (gidx < res_cols && gidy < res_rows) {
    for (i = 0; i < tpl_rows; i++) {
      ...
      for (j = 0; j < tpl_cols; j++) {
        delta = img_ptr[hook(15, j)] - tpl_ptr[hook(16, j)];
