...
kernel void hotspot(int iteration, global float* restrict power, global float* restrict temp_src, global float* restrict temp_dst, int grid_cols, int grid_rows, int border_cols, int border_rows, float Cap, float Rx, float Ry, float Rz, float step) {
  ...
  int small_block_rows = 512 - iteration * 2;
  int small_block_cols = 512 - iteration * 2;
  ...
  for (int i = 0; i < iteration; i++) {
    ...
    if (((tx) >= (i + 1) && (tx) <= (512 - i - 2)) && ((ty) >= (i + 1) && (ty) <= (512 - i - 2)) && ((tx) >= (validXmin) && (tx) <= (validXmax)) && ((ty) >= (validYmin) && (ty) <= (validYmax))) {
      computed = true;
      temp_t[hook(18, ty)][hook(17, tx)] = temp_on_cuda[hook(14, ty)][hook(13, tx)] + step_div_Cap * (power_on_cuda[hook(16, ty)][hook(15, tx)] + (temp_on_cuda[hook(14, S)][hook(19, tx)] + temp_on_cuda[hook(14, N)][hook(20, tx)] - 2.0f * temp_on_cuda[hook(14, ty)][hook(13, tx)]) * Ry_1 + (temp_on_cuda[hook(14, ty)][hook(13, E)] + temp_on_cuda[hook(14, ty)][hook(13, W)] - 2.0f * temp_on_cuda[hook(14, ty)][hook(13, tx)]) * Rx_1 + ((80.0f) - temp_on_cuda[hook(14, ty)][hook(13, tx)]) * Rz_1);
    }
    barrier(0x01);
