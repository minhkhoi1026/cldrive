...
kernel void amdFHT1024FromSXFade(global const float* in1, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int frame_ln, int current_bin, global const float* in2, global const float* out2, int index2, int chnl_stride2) {
  int lcl_id = get_local_id(0);
  int chnl = channels_map[hook(4, get_group_id(1))];
  int chnl_off = chnl_stride * chnl;

  local float data1[1024];
  local float data2[1024];
  for (int i = lcl_id; i < 1024; i += 256) {
  ...
  int n = 1;
  int n2 = 1024 / 2;
  for (int log2_n = 0, log2_n2 = 10 - 1; log2_n < 10; log2_n++, log2_n2--) {
    n = (1 << log2_n);
    n2 = (1 << log2_n2);

    for (int k = lcl_id; k < 1024; k += 256) {
      FHTIteration(data1, gsincos, n, n2, k);
      FHTIteration(data2, gsincos, n, n2, k);
    }
    barrier(0x01);
  }
  int out2_off = mul24(chnl_stride2, chnl) + mul24(index2, 1024 / 2);
  for (int i = lcl_id; i < 1024 / 2; i += 256) {
    float fadeInCoeff = (float)i / (float)(1024 / 2 - 1);
