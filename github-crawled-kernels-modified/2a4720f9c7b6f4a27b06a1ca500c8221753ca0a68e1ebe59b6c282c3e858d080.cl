...
kernel void vectorSquaredTotalByReduce(global double* input, global double* output, local double* sdata, int firstLocalZeroIndex) {
  ...
  if (bid + 1 == bnum) {
    ...
    if (localInputIndex >= firstLocalZeroIndex)
      ...
      sdata[hook(2, tid)] = pown(input[hook(0, globalInputIndex)], 2);
    else
      sdata[hook(2, tid)] = pown(input[hook(0, globalInputIndex)], 2) + pown(input[hook(0, globalInputIndex + 1)], 2);
  } else {
    sdata[hook(2, tid)] = pown(input[hook(0, globalInputIndex)], 2) + pown(input[hook(0, globalInputIndex + 1)], 2);
  }
