...
float weight(int i, int j, int x, int y, float invdx2) {
  return exp(-invdx2 * ((i - x) * (i - x) + (j - y) * (j - y)));
}

kernel void gaussian_blur(global const float* in_half, global float* out_half, const int n, const int width, const int height) {
