...
kernel void computeWeightedTVGradient(global float* grid, global float* Wmatrix, global float* TVgradient, const int sizeX, const int sizeY, const int sizeZ) {
  ...
  for (int z = 0; z < sizeZ; z++) {
    ...
    vxyz = Wmatrix[hook(1, idx)] * (3 * fxyz - fl - fu - ft) / sqrt(eps + (fxyz - fl) * (fxyz - fl) + (fxyz - fu) * (fxyz - fu) + (fxyz - ft) * (fxyz - ft)) - wr * (fr - fxyz) / sqrt(eps + (fr - fxyz) * (fr - fxyz) + (fr - fru) * (fr - fru) + (fr - frt) * (fr - frt)) - wd * (fd - fxyz) / sqrt(eps + (fd - fxyz) * (fd - fxyz) + (fd - fld) * (fd - fld) + (fd - fdt) * (fd - fdt)) - wb * (fb - fxyz) / sqrt(eps + (fb - flb) * (fb - flb) + (fb - fub) * (fb - fub) + (fb - fxyz) * (fb - fxyz));
