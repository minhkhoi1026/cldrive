...
kernel void sobel5(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height) {
  constant const float matrixH[25] = {-1, -4, -6, -4, -1, -2, -8, -12, -8, -2, 0, 0, 0, 0, 0, 2, 8, 12, 8, 2, 1, 4, 6, 4, 1};

  constant const float matrixV[25] = {1, 2, 0, -2, -1, 4, 8, 0, -8, -4, 6, 12, 0, -12, -6, 4, 8, 0, -8, -4, 1, 2, 0, -2, -1};

  float sumH = Convolution(source, src_step, dst_step, width, height, matrixH, 5);
  float sumV = Convolution(source, src_step, dst_step, width, height, matrixV, 5);

  dest[hook(1, get_global_id(1) * dst_step / sizeof(uchar) + get_global_id(0))] = convert_uchar_sat(Combine(sumH, sumV));
}