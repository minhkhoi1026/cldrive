//{"d_iv":2,"data":0,"idea_constant_schedule":1,"idea_schedule":4,"out":3}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void IDEAdecKernel_cbc(global unsigned long* data, constant unsigned long* idea_constant_schedule, global unsigned long* d_iv, global unsigned long* out) {
  local unsigned long idea_schedule[27];
  if (get_local_id(0) < 27)
    idea_schedule[hook(4, get_local_id(0))] = idea_constant_schedule[hook(1, get_local_id(0))];
