...
kernel void givens_prev(global float* matr, global float* cs, global float* ss, unsigned int size, unsigned int stride, unsigned int start_i, unsigned int end_i) {
  ...
  float x = (j < size) ? matr[hook(0, (start_i - 1) * stride + j)] : 0;

  unsigned int elems_num = end_i - start_i;
  ...
  for (unsigned int block_id = 0; block_id < block_num; block_id++) {
    ...
    if (lcl_id < to) {
      cs_lcl[hook(7, lcl_id)] = cs[hook(1, lcl_id + start_i + block_id * lcl_sz)];
      ss_lcl[hook(8, lcl_id)] = ss[hook(2, lcl_id + start_i + block_id * lcl_sz)];
    }
    ...
    if (j < size) {
      for (unsigned int ind = 0; ind < to; ind++) {
        unsigned int i = ind + start_i + block_id * lcl_sz;
  ...
  if (j < size)
    matr[hook(0, (end_i - 1) * stride + j)] = x;
}