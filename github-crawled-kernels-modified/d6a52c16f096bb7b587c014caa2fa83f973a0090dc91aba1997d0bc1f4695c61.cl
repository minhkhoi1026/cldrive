...
kernel void reorder_labels2(global unsigned int* labelImg, global unsigned int* tmpBuffer, unsigned int label_step, unsigned int table_step, global struct SBlobInfo* info) {
  ...
  if (Label != 0xffffffff) {
    int2 Pos = GetPosFromLabel(Label);
    labelImg[hook(0, label_index)] = tmpBuffer[hook(1, Pos.y * table_step + Pos.x)];
  }
}