//{"a_backBuffer":1,"a_bvh_childs":4,"a_bvh_nodes":3,"a_cam":12,"a_dx":14,"a_dy":15,"a_idbuffer":6,"a_m0":16,"a_mx":17,"a_my":18,"a_mz":19,"a_octree":2,"a_origin":13,"a_screenBuffer":0,"a_stack":5,"fovx":20,"fovy":21,"frame":10,"idbuf_size":11,"octree":22,"octree_root":7,"res_x":8,"res_y":9,"stack":23}
...
inline unsigned int fetchChildOffset(global unsigned int* octree, unsigned int const a_node, unsigned int const a_node_before, unsigned int* local_root, unsigned int a_child, unsigned int child_test,
  ...
  if (a_node & 256) {
    ...
    if (rekursion == 2) {
      return ((octree[hook(22, n + (nadd >> 2))] >> ((nadd & 3) << 3)) & 255) + 256 + (nadd << 9);
    }
  }
  return octree[hook(22, n + nadd)];
}
...
inline unsigned int fetchColor(global unsigned int* octree, unsigned int const a_node, unsigned int const a_node_before, unsigned int const a_node_before2, unsigned int* local_root, unsigned int rekursion, unsigned int child_test
  ...
  if (rekursion == 1) {
    ...
    for (int i = 1; i < n; ++i) {
      nadd += get_bitcount((octree[hook(22, ofs + (i >> 2))] >> ((i & 3) << 3)) & 255);
    }
    nadd += get_bitcount(a_node_before & ((child_test << 1) - 1));

    return octree[hook(22, ofs + (nadd >> 2))] >> ((nadd & 3) << 3);
  }
...
kernel void raycast_holes(global unsigned int* a_screenBuffer, global float* a_backBuffer, global unsigned int* a_octree, global struct BVHNode* a_bvh_nodes, global struct BVHChild* a_bvh_childs, global unsigned int* a_stack, global unsigned int* a_idbuffer, unsigned int octree_root, int res_x, int res_y, int frame, int idbuf_size, float4 a_cam, float4 a_origin, float4 a_dx, float4 a_dy,
  ...
  unsigned int idxy = a_idbuffer[hook(6, id + idsize * 2)];
  ...
  do {
    ...
    if (node_test) {
      ...
      if (rekursion <= lod)
        break;
      rekursion--;
      stack[hook(23, stack_add + rekursion)] = nodeid;
      continue;
    }
    ...
    nodeid = stack[hook(23, stack_add + rekursion)];
    nodeid_before = stack[hook(23, stack_add + rekursion + 1)];
    if (rekursion == 11)
  ...
  {
    col = fetchColor(a_octree, nodeid, nodeid_before, stack[hook(23, stack_add + rekursion + 1)], &local_root, rekursion, node_test);
