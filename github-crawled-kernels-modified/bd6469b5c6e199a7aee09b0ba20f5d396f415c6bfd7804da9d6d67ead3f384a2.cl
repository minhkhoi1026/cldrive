...
kernel void normal_1d_logpost(global unsigned int* labels, global float* data, global unsigned int* uniq_label, global float* mu, global float* ss, global unsigned int* n, int cluster_num, global float* hyper_param, global float* rand, global float* logpost) {
  ...
  float Lambda, sigma;
  float new_size = n[hook(5, c)], new_mu = mu[hook(3, c)];
  float logpost_temp;

  k_n = gaussian_k0 + new_size;
  mu_n = (gaussian_k0 * gaussian_mu0 + new_size * new_mu) / k_n;
  alpha_n = gamma_alpha0 + new_size / 2.0f;
  beta_n = gamma_beta0 + 0.5f * ss[hook(4, c)] + gaussian_k0 * new_size * pow((new_mu - gaussian_mu0), 2.0f) / (2.0f * k_n);

  Lambda = alpha_n * k_n / (beta_n * (k_n + 1.0f));
  sigma = pow(1.0f / Lambda, 0.5f);
  logpost_temp = t_logpdf(data[hook(1, i)], 2.0f * alpha_n, mu_n, sigma);
  logpost_temp += (new_size > 0) ? log(new_size / (alpha + data_size)) : log(alpha / (alpha + data_size));
  logpost[hook(9, i * cluster_num + c)] = logpost_temp;
}