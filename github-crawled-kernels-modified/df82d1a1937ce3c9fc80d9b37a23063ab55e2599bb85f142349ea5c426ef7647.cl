...
kernel void conv2d3x3(const int input_channels, const int input_size, const int pad, const int stride, const int start_channel, const int output_size, global float* input_im, global const float* filter_weight, global const float* filter_bias, global float* restrict output_im) {
        ...
        for (int l = 0; l < 3; l++) {
          ...
          for (int m = 0; m < 3; m++) {
            ...
            if ((h >= 0) && (h < input_size) && (w >= 0) && (w < input_size)) {
              tmp += input_im[hook(6, k * input_size * input_size + h * input_size + w)] * filter_weight[hook(7, 9 * k + 3 * l + m)];
            }
          }
        }
      }
