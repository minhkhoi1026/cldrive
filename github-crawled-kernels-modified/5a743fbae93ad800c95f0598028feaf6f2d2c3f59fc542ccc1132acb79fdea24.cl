...
kernel void warpAffineNN_C1_D5(global float* src, global float* dst, int src_cols, int src_rows, int dst_cols, int dst_rows, int srcStep, int dstStep, int src_offset, int dst_offset, constant F* M, int threadCols) {
  ...
  if (dx < threadCols && dy < dst_rows) {
    ...
    if (dx >= 0 && dx < dst_cols && dy >= 0 && dy < dst_rows)
      dst[hook(1, (dst_offset >> 2) + dy * dstStep + dx)] = (sx0 >= 0 && sx0 < src_cols && sy0 >= 0 && sy0 < src_rows) ? src[hook(0, (src_offset >> 2) + sy0 * srcStep + sx0)] : 0;
  }
}