//{"a":0,"a4":6,"gcount":0,"lsum":7,"m":2,"n":3,"psum":1,"sum":1,"sum4":8}
...
kernel void prescanWithOffset(global unsigned int* restrict gcount, global unsigned int const* restrict psum, unsigned int m, unsigned int n) {
  size_t globalID = get_global_id(0);

  int startIndex = globalID * m;
  int stopIndex = min(startIndex + m, n);

  unsigned int sum = psum[hook(1, globalID)];
  for (int i = startIndex; i < stopIndex; i++) {
    unsigned int x = gcount[hook(0, i)];
    gcount[hook(0, i)] = sum;
    sum += x;
  }
}