//{"ablock":6,"ablock[0]":5,"ablock[1]":8,"ablock[2]":9,"ablock[3]":10,"ablock[4]":11,"avec":7,"bblock":15,"bblock[0]":14,"bblock[1]":17,"bblock[2]":19,"bblock[3]":21,"bblock[4]":23,"bvec":4,"c":31,"c[0]":30,"c[1]":32,"c[2]":33,"c[3]":34,"c[4]":35,"cblock":13,"cblock[0]":12,"cblock[1]":16,"cblock[2]":18,"cblock[3]":20,"cblock[4]":22,"g_lhs":0,"gp0":1,"gp1":2,"gp2":3,"lhs":25,"lhs[0]":24,"lhs[1]":26,"lhs[2]":27,"lhs[3]":28,"lhs[4]":29,"lhs[i]":39,"lhs[i][0]":38,"lhs[i][0][n]":37,"lhs[i][1]":41,"lhs[i][1][n]":40,"lhs[i][2]":43,"lhs[i][2][n]":42,"r":36}
...
kernel void x_solve2(global double* g_lhs, int gp0, int gp1, int gp2) {
  int i, j, k, n, m;
  ...
  for (n = 0; n < 5; n++) {
    for (m = 0; m < 5; m++) {
      lhs[hook(25, i)][hook(39, 0)][hook(38, n)][hook(37, m)] = 0.0;
      lhs[hook(25, i)][hook(39, 1)][hook(41, n)][hook(40, m)] = 0.0;
      lhs[hook(25, i)][hook(39, 2)][hook(43, n)][hook(42, m)] = 0.0;
    }
    lhs[hook(25, i)][hook(39, 1)][hook(41, n)][hook(40, n)] = 1.0;
  }
}