...
float4 LABtoRGB(float4 LAB) {
  ...
  float4 XYZ = (float4)((fx > delta) ? D65.x * (fx * fx * fx) : (fx - 16.0f / 116.0f) * 3 * (delta * delta) * D65.x, (fy > delta) ? D65.y * (fy * fy * fy) : (fy - 16.0f / 116.0f) * 3 * (delta * delta) * D65.y, (fz > delta) ? D65.z * (fz * fz * fz) : (fz - 16.0f / 116.0f) * 3 * (delta * delta) * D65.z, LAB.w);

  return XYZtoRGB(XYZ);
}

kernel void clRGBtoLAB(read_only image2d_t srce, write_only image2d_t dest) {
