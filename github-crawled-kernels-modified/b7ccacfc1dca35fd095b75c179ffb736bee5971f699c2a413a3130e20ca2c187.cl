//{"convolution_f_imag":1,"convolution_f_real":0,"distances_f_imag":3,"distances_f_real":2,"sVals_f_imag":5,"sVals_f_real":4,"scaleFFT":6}
...
kernel void convolution(global float* convolution_f_real, global float* convolution_f_imag, global const float* distances_f_real, global const float* distances_f_imag, global const float* sVals_f_real, global const float* sVals_f_imag, const float scaleFFT) {
  const unsigned int idx = get_global_id(0);

  convolution_f_real[hook(0, idx)] = (distances_f_real[hook(2, idx)] * sVals_f_real[hook(4, idx)] - distances_f_imag[hook(3, idx)] * sVals_f_imag[hook(5, idx)]) * scaleFFT;
  convolution_f_imag[hook(1, idx)] = (distances_f_real[hook(2, idx)] * sVals_f_imag[hook(5, idx)] + distances_f_imag[hook(3, idx)] * sVals_f_real[hook(4, idx)]) * scaleFFT;
}