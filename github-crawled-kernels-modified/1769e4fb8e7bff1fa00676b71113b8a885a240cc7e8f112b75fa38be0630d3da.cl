//{"global_state":0,"input":4,"key":1,"output":5,"s":3,"state":2}
...
kernel void encrypt2(global uint4* global_state, const global uint4* key, local uint4* state) {
  const int rounds = 10;
  ...
  key += 32;
  for (unsigned int i = 1; i < rounds; i++, key += 32) {
    subBytes(state);
    shiftRows(state);
    mixColumns(state);
    for (int j = 0; j < 32; j += 8) {
      state[hook(2, j)] ^= key[hook(1, j)];
      state[hook(2, j + 1)] ^= key[hook(1, j + 1)];
      state[hook(2, j + 2)] ^= key[hook(1, j + 2)];
      state[hook(2, j + 3)] ^= key[hook(1, j + 3)];
      state[hook(2, j + 4)] ^= key[hook(1, j + 4)];
      state[hook(2, j + 5)] ^= key[hook(1, j + 5)];
      state[hook(2, j + 6)] ^= key[hook(1, j + 6)];
      state[hook(2, j + 7)] ^= key[hook(1, j + 7)];
    }
  }
  subBytes(state);
  shiftRows(state);
  ...
  for (int i = 0; i < 32; i++)
    global_state[hook(0, i)] = state[hook(2, i)] ^ key[hook(1, i)];
}