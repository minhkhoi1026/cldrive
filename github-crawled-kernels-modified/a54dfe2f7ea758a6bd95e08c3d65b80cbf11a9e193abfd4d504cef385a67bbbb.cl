...
kernel void processNineCellWindow(global float* scanLine1, global float* scanLine2, global float* scanLine3, global float* resultLine, global float* rasterParams) {
  ...
  if (scanLine2[hook(1, i + 1)] == rasterParams[hook(4, 0)]) {
    resultLine[hook(3, i)] = rasterParams[hook(4, 1)];
  } else {
    float derX = calcFirstDer(scanLine1[hook(0, i)], scanLine1[hook(0, i + 1)], scanLine1[hook(0, i + 2)], scanLine2[hook(1, i)], scanLine2[hook(1, i + 1)], scanLine2[hook(1, i + 2)], scanLine3[hook(2, i)], scanLine3[hook(2, i + 1)], scanLine3[hook(2, i + 2)], rasterParams[hook(4, 0)], rasterParams[hook(4, 1)], rasterParams[hook(4, 2)], rasterParams[hook(4, 3)]);

    float derY = calcFirstDer(scanLine3[hook(2, i)], scanLine2[hook(1, i)], scanLine1[hook(0, i)], scanLine3[hook(2, i + 1)], scanLine2[hook(1, i + 1)], scanLine1[hook(0, i + 1)], scanLine3[hook(2, i + 2)], scanLine2[hook(1, i + 2)], scanLine1[hook(0, i + 2)], rasterParams[hook(4, 0)], rasterParams[hook(4, 1)], rasterParams[hook(4, 2)], rasterParams[hook(4, 4)]);
    ...
    if (derX == rasterParams[hook(4, 1)] || derY == rasterParams[hook(4, 1)]) {
      ...
      if (derX == 0.0f && derY == 0.0f)
        aspect_rad = rasterParams[hook(4, 7)] / 2.0f;
      else
        aspect_rad = 3.14159265358979323846f + atan2(derX, derY);

      resultLine[hook(3, i)] = max(0.0f, 255.0f * ((rasterParams[hook(4, 5)] * cos(slope_rad)) + (rasterParams[hook(4, 6)] * sin(slope_rad) * cos(rasterParams[hook(4, 7)] - aspect_rad))));
    }
  }
}