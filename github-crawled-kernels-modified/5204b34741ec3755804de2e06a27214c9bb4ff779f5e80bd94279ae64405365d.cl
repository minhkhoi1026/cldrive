...
kernel void init_message_1(global float* u_new_, global float* d_new_, global float* l_new_, global float* r_new_, global const float* u_cur_, global const float* d_cur_, global const float* l_cur_, global const float* r_cur_, global float* ctemp, global float* selected_disp_pyr_new, global const float* selected_disp_pyr_cur, global float* data_cost_selected_, global const float* data_cost_, int h, int w, int nr_plane, int h2, int w2, int nr_plane2, int cdisp_step1, int cdisp_step2, int cmsg_step1, int cmsg_step2) {
  int x = get_global_id(0);
  int y = get_global_id(1);

  global const float* u_cur = u_cur_ + min(h2 - 1, y / 2 + 1) * cmsg_step2 + x / 2;
  global const float* d_cur = d_cur_ + max(0, y / 2 - 1) * cmsg_step2 + x / 2;
  global const float* l_cur = l_cur_ + y / 2 * cmsg_step2 + min(w2 - 1, x / 2 + 1);
  global const float* r_cur = r_cur_ + y / 2 * cmsg_step2 + max(0, x / 2 - 1);

  global float* data_cost_new = ctemp + y * cmsg_step1 + x;

  global const float* disparity_selected_cur = selected_disp_pyr_cur + y / 2 * cmsg_step2 + x / 2;
  global const float* data_cost = data_cost_ + y * cmsg_step1 + x;

  if (y < h && x < w) {
    for (int d = 0; d < nr_plane2; d++) {
      int idx2 = d * cdisp_step2;

      float val = data_cost[hook(38, d * cdisp_step1)] + u_cur[hook(31, idx2)] + d_cur[hook(33, idx2)] + l_cur[hook(35, idx2)] + r_cur[hook(37, idx2)];
      data_cost_new[hook(25, d * cdisp_step1)] = val;
    }
  }

  global float* data_cost_selected = data_cost_selected_ + y * cmsg_step1 + x;
  global float* disparity_selected_new = selected_disp_pyr_new + y * cmsg_step1 + x;

  global float* u_new = u_new_ + y * cmsg_step1 + x;
  global float* d_new = d_new_ + y * cmsg_step1 + x;
  global float* l_new = l_new_ + y * cmsg_step1 + x;
  global float* r_new = r_new_ + y * cmsg_step1 + x;
  ...
  if (y < h && x < w) {
    ...
    for (int i = 0; i < nr_plane; i++) {
      ...
      for (int j = 0; j < nr_plane2; j++) {
        float cur = data_cost_new[hook(25, j * cdisp_step1)];
        if (cur < minimum) {
          minimum = cur;
          id = j;
        }
      }
      data_cost_selected[hook(26, i * cdisp_step1)] = data_cost[hook(38, id * cdisp_step1)];
      disparity_selected_new[hook(28, i * cdisp_step1)] = disparity_selected_cur[hook(29, id * cdisp_step2)];
      u_new[hook(30, i * cdisp_step1)] = u_cur[hook(31, id * cdisp_step2)];
      d_new[hook(32, i * cdisp_step1)] = d_cur[hook(33, id * cdisp_step2)];
      l_new[hook(34, i * cdisp_step1)] = l_cur[hook(35, id * cdisp_step2)];
      r_new[hook(36, i * cdisp_step1)] = r_cur[hook(37, id * cdisp_step2)];
      data_cost_new[hook(25, id * cdisp_step1)] = 0x1.fffffep127f;
    }
  }
}