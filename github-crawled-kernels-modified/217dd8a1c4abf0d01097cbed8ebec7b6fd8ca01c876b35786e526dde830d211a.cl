...
kernel void merge(global struct float4* p, int n, global struct float4* aux, int nprocINI) {
  int base = get_global_id(0);
  int tam = get_global_size(0);
  int start, med, stop, mod, elem, mult;
  elem = n / nprocINI;
  mod = n % nprocINI;
  mult = nprocINI / tam;
  if (mult * base <= mod)
    start = mult * base * (elem + 1);
  else
    start = mult * base * elem + mod;
  if (mult * base + (mult >> 1) <= mod)
    med = (mult * base + (mult >> 1)) * (elem + 1);
  else
    med = (mult * base + (mult >> 1)) * elem + mod;
  if (mult * (base + 1) <= mod)
    stop = mult * (base + 1) * (elem + 1) - 1;
  else
    stop = mult * (base + 1) * elem + mod - 1;

  mergeA(p, start, med, stop + 1, aux);
}