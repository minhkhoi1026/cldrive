...
kernel void reduce_float(global float2* input, const unsigned int offset, const unsigned int count, global float2* output, const unsigned int output_offset) {
  const unsigned int block_offset = get_group_id(0) * 8 * 12;
  global const float2* block = input + offset + block_offset;
  const unsigned int lid = get_local_id(0);
  local float2 scratch[12];
  float2 sum = 0;
