//{"addr_offset":7,"base_index":5,"group_index":4,"input_data":1,"n":6,"output_data":0,"partial_sums":2,"shared_data":3}
...
void LoadLocalFromGlobal(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  shared_data[hook(3, local_index_a + bank_offset_a)] = input_data[hook(1, global_index_a)];
  shared_data[hook(3, local_index_b + bank_offset_b)] = input_data[hook(1, global_index_b)];
}
...
void LoadLocalFromGlobalFirstLevel(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  shared_data[hook(3, local_index_a + bank_offset_a)] = input_data_a > -1 ? 1 : 0;
  shared_data[hook(3, local_index_b + bank_offset_b)] = input_data_b > -1 ? 1 : 0;
}
...
void LoadLocalFromGlobalNonPowerOfTwo(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  shared_data[hook(3, local_index_a + bank_offset_a)] = input_data[hook(1, global_index_a)];
  shared_data[hook(3, local_index_b + bank_offset_b)] = (local_index_b < n) ? input_data[hook(1, global_index_b)] : 0;
...
void LoadLocalFromGlobalNonPowerOfTwoFirstLevel(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  if (local_index_b < n) {
    ...
    shared_data[hook(3, local_index_b + bank_offset_b)] = 0;
...
void StoreLocalToGlobal(global int* output_data, local const int* shared_data, const uint4 address_pair, const unsigned int n) {
  ...
  output_data[hook(0, global_index_a)] = shared_data[hook(3, local_index_a + bank_offset_a)];
  output_data[hook(0, global_index_b)] = shared_data[hook(3, local_index_b + bank_offset_b)];
}
...
void ClearLastElementStoreSum(local int* shared_data, global int* partial_sums, int group_index) {
  ...
  if (local_id == 0) {
    int index = (group_size << 1) - 1;
    index += ((index) >> (4));
    partial_sums[hook(2, group_index)] = shared_data[hook(3, index)];
    shared_data[hook(3, index)] = 0;
  }
}
...
unsigned int BuildPartialSum(local int* shared_data) {
  ...
  for (unsigned int j = group_size; j > 0; j >>= 1) {
    ...
    if (local_id < j) {
      ...
      shared_data[hook(3, local_index_b)] += shared_data[hook(3, local_index_a)];
    }
...
void ScanRootToLeaves(local int* shared_data, unsigned int stride) {
  ...
  for (unsigned int j = 1; j <= group_size; j *= two) {
    ...
    if (local_id < j) {
      ...
      int t = shared_data[hook(3, local_index_a)];
      shared_data[hook(3, local_index_a)] = shared_data[hook(3, local_index_b)];
      shared_data[hook(3, local_index_b)] += t;
    }
  }
}
...
kernel void PreScanStoreSumNonPowerOfTwoFirstLevelKernel(global int* output_data, global const int* input_data, global int* partial_sums, local int* shared_data, const unsigned int group_index, const unsigned int base_index, const unsigned int n, const unsigned int addr_offset) {
  const unsigned int local_id = get_local_id(0);
  const unsigned int group_id = get_global_id(0) / get_local_size(0);
  const unsigned int group_size = get_local_size(0);
