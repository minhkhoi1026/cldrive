//{"buffer":5,"gridResolution":3,"height":1,"velocityBuffer":4,"visualizationBuffer":2,"width":0}
...
kernel void visualizationVelocity(const int width, const int height, global float4* visualizationBuffer, const int gridResolution, global float2* velocityBuffer) {
  int2 id = (int2)(get_global_id(0), get_global_id(1));

  if (id.x < width && id.y < height) {
    float2 velocity = velocityBuffer[hook(4, id.x + id.y * width)];
    visualizationBuffer[hook(2, id.x + id.y * width)] = (float4)((1.0f + velocity) / 2.0f, 0.0f, 0.0f);
  }
}