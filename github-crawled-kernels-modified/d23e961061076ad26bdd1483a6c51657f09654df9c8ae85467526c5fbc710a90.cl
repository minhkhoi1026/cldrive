//{"T->vertices":9,"array":8,"col":3,"dir":2,"ids_behind":17,"mipmap_start":5,"num":0,"nums":6,"passback":22,"points":12,"pos":1,"pr":20,"raw":15,"ret":13,"ret[0]":18,"ret[1]":19,"rotated":16,"size":23,"sizes":7,"tex_id":4,"triangle->vertices":14,"tris":21,"x":10,"y":11}
...
void set_tri_vertex(global struct triangle* T, int i, float3 pos) {
  T->vertices[hook(9, i)].pos.xyz = pos;
}
...
float calc_rconstant(float x[3], float y[3]) {
  return native_recip(x[hook(10, 1)] * y[hook(11, 2)] + x[hook(10, 0)] * (y[hook(11, 1)] - y[hook(11, 2)]) - x[hook(10, 2)] * y[hook(11, 1)] + (x[hook(10, 2)] - x[hook(10, 1)]) * y[hook(11, 0)]);
}
...
void calc_min_max(float3 points[3], float width, float height, float ret[4]) {
  ...
  for (int i = 0; i < 3; i++) {
    x[hook(10, i)] = round(points[hook(12, i)].x);
    y[hook(11, i)] = round(points[hook(12, i)].y);
  }

  ret[hook(13, 0)] = min3(x[hook(10, 0)], x[hook(10, 1)], x[hook(10, 2)]) - 1;
  ret[hook(13, 1)] = max3(x[hook(10, 0)], x[hook(10, 1)], x[hook(10, 2)]);
  ret[hook(13, 2)] = min3(y[hook(11, 0)], y[hook(11, 1)], y[hook(11, 2)]) - 1;
  ret[hook(13, 3)] = max3(y[hook(11, 0)], y[hook(11, 1)], y[hook(11, 2)]);

  ret[hook(13, 0)] = clamp(ret[hook(13, 0)], 0.0f, width - 1);
  ret[hook(13, 1)] = clamp(ret[hook(13, 1)], 0.0f, width - 1);
  ret[hook(13, 2)] = clamp(ret[hook(13, 2)], 0.0f, height - 1);
  ret[hook(13, 3)] = clamp(ret[hook(13, 3)], 0.0f, height - 1);
}
...
void calc_min_max_oc(float3 points[3], float mx, float my, float width, float height, float ret[4]) {
  ...
  ret[hook(13, 0)] = clamp(ret[hook(13, 0)], mx, width - 1);
  ret[hook(13, 1)] = clamp(ret[hook(13, 1)], mx, width - 1);
  ret[hook(13, 2)] = clamp(ret[hook(13, 2)], my, height - 1);
  ret[hook(13, 3)] = clamp(ret[hook(13, 3)], my, height - 1);
}
...
void rot_3(global struct triangle* triangle, const float3 c_pos, const float3 c_rot, const float3 offset, const float3 rotation_offset, float3 ret[3]) {
  ret[hook(13, 0)] = rot(vertex_pos(&triangle->vertices[hook(14, 0)]), 0, rotation_offset);
  ret[hook(13, 1)] = rot(vertex_pos(&triangle->vertices[hook(14, 1)]), 0, rotation_offset);
  ret[hook(13, 2)] = rot(vertex_pos(&triangle->vertices[hook(14, 2)]), 0, rotation_offset);

  ret[hook(13, 0)] = rot(ret[hook(13, 0)] + offset, c_pos, c_rot);
  ret[hook(13, 1)] = rot(ret[hook(13, 1)] + offset, c_pos, c_rot);
  ret[hook(13, 2)] = rot(ret[hook(13, 2)] + offset, c_pos, c_rot);
}

void rot_3_raw(const float3 raw[3], const float3 rotation, float3 ret[3]) {
  ret[hook(13, 0)] = rot(raw[hook(15, 0)], 0, rotation);
  ret[hook(13, 1)] = rot(raw[hook(15, 1)], 0, rotation);
  ret[hook(13, 2)] = rot(raw[hook(15, 2)], 0, rotation);
}

void rot_3_pos(const float3 raw[3], const float3 pos, const float3 rotation, float3 ret[3]) {
  ret[hook(13, 0)] = rot(raw[hook(15, 0)], pos, rotation);
  ret[hook(13, 1)] = rot(raw[hook(15, 1)], pos, rotation);
  ret[hook(13, 2)] = rot(raw[hook(15, 2)], pos, rotation);
}

void depth_project(float3 rotated[3], float width, float height, float fovc, float3 ret[3]) {
  for (int i = 0; i < 3; i++) {
    float2 rxy = mad(rotated[hook(16, i)].xy, fovc / rotated[hook(16, i)].z, (float2){width / 2.f, height / 2.f});

    ret[hook(13, i)].xy = rxy;
    ret[hook(13, i)].z = rotated[hook(16, i)].z;
  }
}
...
void generate_new_triangles(float3 points[3], int* num, float3 ret[2][3]) {
  ...
  for (int i = 0; i < 3; i++) {
    if (points[hook(12, i)].z <= 20 || points[hook(12, i)].z > 350000.0f) {
      ids_behind[hook(17, n_behind)] = i;
      n_behind++;
    } else {
      id_valid = i;
    }
  ...
  if (n_behind == 0) {
    ret[hook(13, 0)][hook(18, 0)] = points[hook(12, 0)];
    ret[hook(13, 0)][hook(18, 1)] = points[hook(12, 1)];
    ret[hook(13, 0)][hook(18, 2)] = points[hook(12, 2)];
  ...
  if (n_behind == 2) {
    g2 = ids_behind[hook(17, 0)];
    g3 = ids_behind[hook(17, 1)];
    g1 = id_valid;
  }

  p1 = points[hook(12, g2)] + native_divide((20 - points[hook(12, g2)].z) * (points[hook(12, g1)] - points[hook(12, g2)]), points[hook(12, g1)].z - points[hook(12, g2)].z);
  p2 = points[hook(12, g3)] + native_divide((20 - points[hook(12, g3)].z) * (points[hook(12, g1)] - points[hook(12, g3)]), points[hook(12, g1)].z - points[hook(12, g3)].z);

  if (n_behind == 1) {
    c1 = points[hook(12, g2)];
    c2 = points[hook(12, g3)];

    ret[hook(13, 0)][hook(18, 0)] = p1;
    ret[hook(13, 0)][hook(18, 1)] = c1;
    ret[hook(13, 0)][hook(18, 2)] = c2;

    ret[hook(13, 1)][hook(19, 0)] = p1;
    ret[hook(13, 1)][hook(19, 1)] = c2;
    ret[hook(13, 1)][hook(19, 2)] = p2;
    *num = 2;
  } else {
    c1 = points[hook(12, g1)];
    ret[hook(13, 0)][hook(18, ids_behind[0hook(17, 0))] = p1;
    ret[hook(13, 0)][hook(18, ids_behind[1hook(17, 1))] = p2;
    ret[hook(13, 0)][hook(18, id_valid)] = c1;
    *num = 1;
  }
}
...
void full_rotate_n_extra(float3 v1, float3 v2, float3 v3, float3 passback[2][3], int* num, const float3 c_pos, const float3 c_rot, const float3 offset, const float3 rotation_offset, const float fovc, const float width, const float height) {
  ...
  pr[hook(20, 0)] = rot_with_offset(v1, c_pos, c_rot, offset, rotation_offset);
  pr[hook(20, 1)] = rot_with_offset(v2, c_pos, c_rot, offset, rotation_offset);
  pr[hook(20, 2)] = rot_with_offset(v3, c_pos, c_rot, offset, rotation_offset);
  ...
  *num = n;
  ...
  depth_project(tris[hook(21, 0)], width, height, fovc, passback[hook(22, 0)]);

  if (n == 2) {
    depth_project(tris[hook(21, 1)], width, height, fovc, passback[hook(22, 1)]);
  }
}
...
void full_rotate_quat(float3 v1, float3 v2, float3 v3, float3 passback[2][3], int* num, float3 c_pos, float3 c_rot, float3 offset, float4 rotation_offset, float scale, float fovc, float width, float height) {
  ...
  pr[hook(20, 0)] = rot_quat_with_offset(v1 * scale, c_pos, c_rot, offset, rotation_offset);
  pr[hook(20, 1)] = rot_quat_with_offset(v2 * scale, c_pos, c_rot, offset, rotation_offset);
  pr[hook(20, 2)] = rot_quat_with_offset(v3 * scale, c_pos, c_rot, offset, rotation_offset);
...
{
  array[hook(8, coord.z * width * height + mul24(coord.y, width) + coord.x)] = convert_uchar4(to_write);
}

uint4 read_image_3d_hardware(int4 coord, global uchar4* array, int width, int height)

{
  return convert_uint4(array[hook(8, coord.z * width * height + mul24(coord.y, width) + coord.x)]);
}
...
kernel void procedural_crack(int num, float2 pos, float2 dir, float4 col, unsigned int tex_id, unsigned int mipmap_start, global unsigned int* nums, global unsigned int* sizes, image_3d_write array) {
  int line_id = get_global_id(0);

  if (line_id >= num)
    return;

  int slice = nums[hook(6, tex_id)] >> 16;
  float width = sizes[hook(7, slice)];

  int length = width / 3.f;
  ...
  for (int i = 0; i < length; i++) {
    float change_angle = 0.174533f * (noise(per_texture + i * 10.f + nnoise)) * 0.5f;
