//{"Caille_parameter":13,"Nhead_length":8,"Nlayers":12,"Nq":3,"Nspacing":9,"Ntail_length":7,"background":11,"cutoff":6,"head_sld":15,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":10,"sld":14,"solvent_sld":16}
...
kernel void lamellarCailleHG_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,
    ...
    for (int tail_length_i = 0; tail_length_i < Ntail_length; tail_length_i++) {
      ...
      for (int head_length_i = 0; head_length_i < Nhead_length; head_length_i++) {
        ...
        for (int spacing_i = 0; spacing_i < Nspacing; spacing_i++) {
          const float spacing = loops[hook(5, 2 * (spacing_i + Ntail_length + Nhead_length))];
          const float spacing_w = loops[hook(5, 2 * (spacing_i + Ntail_length + Nhead_length) + 1)];

          const float weight = tail_length_w * head_length_w * spacing_w;
          if (weight > cutoff) {
            const float scattering = Iqxy(qxi, qyi, tail_length, head_length, Nlayers, spacing, Caille_parameter, sld, head_sld, solvent_sld);
            if (!isnan(scattering)) {
