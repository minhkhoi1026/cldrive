//{"Gauss76Wt":16,"Gauss76Z":15,"Nphi":10,"Nq":3,"Nrequatorial":8,"Nrpolar":7,"Ntheta":9,"background":12,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":11,"sld":13,"solvent_sld":14}
...
float Iq(float q, float sld, float solvent_sld, float rpolar, float requatorial) {
  float total = 0.0f;
  for (int i = 0; i < 76; i++) {
    const float sin_alpha = 0.5f * (Gauss76Z[hook(15, i)] + 1.0f);
    total += Gauss76Wt[hook(16, i)] * _ellipsoid_kernel(q, rpolar, requatorial, sin_alpha);
  }
...
kernel void ellipsoid_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                           global float* loops_g,

                           local float* loops, const float cutoff, const int Nrpolar, const int Nrequatorial, const int Ntheta, const int Nphi,

                           const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nrpolar + Nrequatorial + Ntheta + Nphi) * 2, 0);
...

  {
    ...
    for (int rpolar_i = 0; rpolar_i < Nrpolar; rpolar_i++) {
      const float rpolar = loops[hook(5, 2 * (rpolar_i))];
      const float rpolar_w = loops[hook(5, 2 * (rpolar_i) + 1)];
      for (int requatorial_i = 0; requatorial_i < Nrequatorial; requatorial_i++) {
        const float requatorial = loops[hook(5, 2 * (requatorial_i + Nrpolar))];
        const float requatorial_w = loops[hook(5, 2 * (requatorial_i + Nrpolar) + 1)];
        for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
          const float theta = loops[hook(5, 2 * (theta_i + Nrpolar + Nrequatorial))];
          const float theta_w = loops[hook(5, 2 * (theta_i + Nrpolar + Nrequatorial) + 1)];
          for (int phi_i = 0; phi_i < Nphi; phi_i++) {
            const float phi = loops[hook(5, 2 * (phi_i + Nrpolar + Nrequatorial + Ntheta))];
            const float phi_w = loops[hook(5, 2 * (phi_i + Nrpolar + Nrequatorial + Ntheta) + 1)];

            const float weight = rpolar_w * requatorial_w * theta_w * phi_w;
            if (weight > cutoff) {
              const float scattering = Iqxy(qxi, qyi, sld, solvent_sld, rpolar, requatorial, theta, phi);
              if (!isnan(scattering)) {
