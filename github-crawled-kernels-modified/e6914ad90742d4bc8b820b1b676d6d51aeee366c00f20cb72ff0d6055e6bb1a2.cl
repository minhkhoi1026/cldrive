//{"cache":8,"dest":1,"dst_step":3,"height":5,"matrix":6,"source":0,"src_step":2,"values":7,"width":4}
...
__attribute__((reqd_work_group_size(16, 16, 1))) kernel void median3_cached(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height) {
  const int gx = get_global_id(0);
  const int gy = get_global_id(1);
  const int2 pos = {gx, gy};
  src_step /= sizeof(uchar);
  ...
  local float cache[16 * 16];
  cache[hook(8, lid)] = convert_float(source[hook(0, (pos).y * src_step + (pos).x)]);
  barrier(0x01);
    ...
    if (py < y_cache_begin || py >= y_cache_end) {
      ...
      for (int x = -mask_size; x <= mask_size; x++) {
        ...
        if (px < x_cache_begin || px >= x_cache_end)
          values[hook(7, Index++)] = convert_float(source[hook(0, ((int2){px, py}).y * src_step + ((int2){px, py}).x)]);
        else {
          int cache_y = py - y_cache_begin;
          int cache_x = px - x_cache_begin;
          values[hook(7, Index++)] = cache[hook(8, cache_y * 16 + cache_x)];
        }
      }
    }
  }
