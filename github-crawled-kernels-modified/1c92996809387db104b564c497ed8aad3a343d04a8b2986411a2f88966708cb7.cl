...
kernel void ConvertToN8UC4(private const int global_size_dim0, private const int global_size_dim1, read_only image2d_t input_ptr, global uchar* output, private const int height, private const int width, private const float4 scale, private const float4 bias) {
  ...
  int buffer_offset = ((batch_idx * height + height_idx) * width + width_idx + channel_block_idx) * 4;
