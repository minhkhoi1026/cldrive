//{"divisionSize":2,"input":0,"inputSize":3,"mean":4,"output":1}
...
kernel void varianceKernel(global float* input, global float* output, unsigned int const divisionSize, unsigned int const inputSize, float const mean) {
  unsigned int n = get_global_id(0);
  float outputInternal = 0;
  if ((n + 1) * divisionSize <= inputSize) {
    for (unsigned int i = 0; i < divisionSize; i++) {
      outputInternal += (input[hook(0, n * divisionSize + i)] - mean) * (input[hook(0, n * divisionSize + i)] - mean);
    }
  } else {
