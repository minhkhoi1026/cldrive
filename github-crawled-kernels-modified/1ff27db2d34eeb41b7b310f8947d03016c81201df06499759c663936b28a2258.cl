...
kernel void nw_kernel1_gpu(global int* reference_d, global int* input_itemsets_d, global int* output_itemsets_d,
  ...
  if (tx == 0) {
    input_itemsets_l[hook(11, 0 + tx * (32 + 1))] = input_itemsets_d[hook(1, index_nw + tx)];
  }
  ...
  for (int ty = 0; ty < 32; ty++)
    input_itemsets_d[hook(1, index + cols * ty)] = input_itemsets_l[hook(11, (tx + 1) + (ty + 1) * (32 + 1))];
