//{"base_index":4,"group_offset":3,"input_data":1,"n":5,"output_data":0,"partial_sums":6,"shared_data":2}
...
kernel void UniformAddKernel(global float* output_data, global float* input_data, local float* shared_data, const unsigned int group_offset, const unsigned int base_index, const unsigned int n) {
  const unsigned int local_id = get_local_id(0);
  const unsigned int group_id = get_global_id(0) / get_local_size(0);
  const unsigned int group_size = get_local_size(0);
  ...
  unsigned int address = mul24(group_id, (group_size << 1)) + base_index + local_id;
