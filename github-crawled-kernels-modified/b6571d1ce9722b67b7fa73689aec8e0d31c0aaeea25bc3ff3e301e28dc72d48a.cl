...
__attribute__((reqd_work_group_size(7, 7, 2))) __attribute__((reqd_work_group_size(14, 14, 2))) __attribute__((reqd_work_group_size(2, 2, 4))) __attribute__((reqd_work_group_size(16, 1, 1))) __attribute__((reqd_work_group_size(1, 1, 10))) kernel void conv5(
  ...
  for (int iTile = 0; iTile < 16; iTile += 4) {
    ...
    for (int r = 0; r < 1; ++r) {
      for (int c = 0; c < 1; ++c) {
        for (int o = 0; o < 12; ++o, ++oPrivateIdx) {
          for (int i = 0; i < 4; ++i) {
            int weightIdx = 0;
            for (int x = 0; x < 5; ++x) {
              for (int y = 0; y < 5; ++y, ++weightIdx) {
                outPrivate[hook(6, oPrivateIdx)] += inLocal[hook(2, ((i + iTile) * 5 + r + rTile + x) * 5 + c + cTile + y)] * weightLocal[hook(4, ((o + oTile) * 16 + i + iTile) * 25 + weightIdx)];
              }
            }
          }
        }
  ...
  for (int r = 0; r < 1; ++r) {
    for (int c = 0; c < 1; ++c) {
      for (int o = 0; o < 12; ++o, ++oPrivateIdx) {
        buf5[hook(7, ((o + oTile) * 1 + r + rTile) * 1 + c + cTile)] = sigmod(outPrivate[hook(6, oPrivateIdx)] + offsetLocal[hook(5, o + oTile)]);
      }
    }
  }
}