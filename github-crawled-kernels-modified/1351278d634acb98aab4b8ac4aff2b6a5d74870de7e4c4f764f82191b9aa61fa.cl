//{"cache":4,"grid":0,"mean":2,"num_elements":3,"result":1}
...
kernel void stddev(global float* grid, global float* result, float const mean, int const num_elements, local float* cache) {
  ...
  cache[hook(4, local_id)] = (global_id < num_elements) ? pow((grid[hook(0, global_id)] - mean), 2) : 0.0f;
  barrier(0x01);
  ...
  if (local_id == 0)
    result[hook(1, group_id)] = cache[hook(4, 0)];
}