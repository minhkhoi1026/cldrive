//{"g_atomic_input":1,"g_special_values":2,"p_3->g_2":6,"p_3->g_2[2]":5,"p_3->g_2[2][6]":4,"p_3->g_2[i]":8,"p_3->g_2[i][j]":7,"result":0,"sequence_input":3}
...
unsigned int func_1(struct S3* p_3);
unsigned int func_1(struct S3* p_3) {
  return p_3->g_2[hook(6, 2)][hook(5, 6)][hook(4, 4)];
}
...
kernel void entry(global ulong* result, global volatile unsigned int* g_atomic_input, global volatile unsigned int* g_special_values, global int* sequence_input) {
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 10; j++) {
      for (k = 0; k < 8; k++) {
        transparent_crc_no_string(&crc64_context, p_3->g_2[hook(6, i)][hook(8, j)][hook(7, k)]);
      }
    }
  }
