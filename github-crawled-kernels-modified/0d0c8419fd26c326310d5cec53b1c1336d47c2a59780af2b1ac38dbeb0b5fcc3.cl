//{"buffer":5,"gridResolution":3,"height":1,"pressureBuffer":4,"visualizationBuffer":2,"width":0}
...
kernel void visualizationPressure(const int width, const int height, global float4* visualizationBuffer, const int gridResolution, global float* pressureBuffer) {
  int2 id = (int2)(get_global_id(0), get_global_id(1));

  if (id.x < width && id.y < height) {
    float pressure = pressureBuffer[hook(4, id.x + id.y * width)];
    visualizationBuffer[hook(2, id.x + id.y * width)] = (float4)((1.0f + pressure) / 2.0f);
  }
}