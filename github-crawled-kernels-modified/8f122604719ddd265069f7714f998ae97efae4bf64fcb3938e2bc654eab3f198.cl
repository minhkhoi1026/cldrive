//{"Mul2_OUT":11,"Mul4_OUT":12,"Mul6_OUT":13,"Mul8_OUT":14,"MulA_OUT":15,"_pt":16,"_rk":17,"clefia_s0":7,"clefia_s1":9,"ct":0,"out":4,"out2":5,"outCipher":18,"pt":1,"r":3,"rk":2,"x":8,"y":10,"z":6}
...
kernel void clefiaCipher(global uchar* ct, global uchar* pt, global uchar* rk, global int* r) {
 private
  int gid;
  gid = get_global_id(0);
 ...
 private
  ...
  for (int i = 0; i < 16; i++) {
    int offset = gid * 16 + i;
    _pt[hook(16, i)] = pt[hook(1, offset)];
  }

 private
  uchar _rk[8 * 26 + 16];
  for (int i = 0; i < (8 * 26 + 16); i++) {
    _rk[hook(17, i)] = rk[hook(2, i)];
  }
 ...
 private
  ...
  for (int i = 0; i < 16; i++) {
    int offset = gid * 16 + i;
    ct[hook(0, offset)] = outCipher[hook(18, i)];
  }
}