...
kernel void yBlend(global uchar4* in, global uchar4* out) {
  ...
  const float4 shift = convert_float4(in[hook(0, (((get_global_id(1) + get_global_size(1) / 2) % get_global_size(1)) * get_global_size(0) + (get_global_id(0))))]);
  const float4 result = Blend(original, shift, clamp(fabs(fma(-2.0f * get_global_id(1), 1.0f / (get_global_size(1) - 1.0f), 1.0f)), 0.0f, 1.0f));
