//{"doubleBuff":2,"iterationsGlobal":0,"iterationsInternal":1}
...
kernel void trigonometryLoopDP(const int iterationsGlobal, const int iterationsInternal, global double* doubleBuff) {
  double r = 0;
  for (int i = 0; i < iterationsGlobal; i++) {
    r = trigonometryDP(iterationsInternal);
  }

  size_t gid = get_global_id(0);
  doubleBuff[hook(2, gid)] = r;
}