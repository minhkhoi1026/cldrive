...
kernel void compObjWave(global float3* pc, global float2* of, unsigned int of_rows, unsigned int of_cols, float hologram_z, float k, float pitch, float corner_x, float corner_y) {
  int row = get_global_id(1);
  int col = get_global_id(2);

  float3 ps = pc[hook(0, get_global_id(0))];
