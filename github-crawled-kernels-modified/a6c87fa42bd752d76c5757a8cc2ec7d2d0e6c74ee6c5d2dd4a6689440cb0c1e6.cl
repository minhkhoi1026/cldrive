...
void group_scan_exclusive_uint(int localId, int groupSize, local unsigned int* shmem) {
  ...
  for (int stride = (groupSize >> 1); stride > 0; stride >>= 1) {
    if (localId < groupSize / (2 * stride)) {
      unsigned int temp = shmem[hook(4, (2 * localId + 1) * stride - 1)];
      shmem[hook(4, (2 * localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)];
      shmem[hook(4, 2 * (localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)] + temp;
    }
    barrier(0x01);
...
void group_scan_exclusive_float(int localId, int groupSize, local float* shmem) {
  ...
  for (int stride = (groupSize >> 1); stride > 0; stride >>= 1) {
    if (localId < groupSize / (2 * stride)) {
      float temp = shmem[hook(4, (2 * localId + 1) * stride - 1)];
      shmem[hook(4, (2 * localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)];
      shmem[hook(4, 2 * (localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)] + temp;
    }
    barrier(0x01);
...
void group_scan_exclusive_short(int localId, int groupSize, local short* shmem) {
  ...
  for (int stride = (groupSize >> 1); stride > 0; stride >>= 1) {
    if (localId < groupSize / (2 * stride)) {
      short temp = shmem[hook(4, (2 * localId + 1) * stride - 1)];
      shmem[hook(4, (2 * localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)];
      shmem[hook(4, 2 * (localId + 1) * stride - 1)] = shmem[hook(4, 2 * (localId + 1) * stride - 1)] + temp;
    }
    barrier(0x01);
...
__attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) __attribute__((reqd_work_group_size(64, 1, 1))) kernel void scan_exclusive_part_float4(global float4 const* in_array, global float4* out_array, unsigned int numElems, global float* out_sums, local float* shmem) {
  int globalId = get_global_id(0);
  int localId = get_local_id(0);
  int groupId = get_group_id(0);
  int groupSize = get_local_size(0);
  float4 v1 = safe_load_float4(in_array, 2 * globalId, numElems);
  float4 v2 = safe_load_float4(in_array, 2 * globalId + 1, numElems);
  v1.y += v1.x;
  v1.w += v1.z;
  v1.w += v1.y;
  v2.y += v2.x;
  ...
  barrier(0x01);
  float sum = group_scan_exclusive_part_float(localId, groupSize, shmem);
  if (localId == 0)
  ...
  v2.z = v2.w;
  v2.w += t;
  safe_store_float4(v2, out_array, 2 * globalId + 1, numElems);
  safe_store_float4(v1, out_array, 2 * globalId, numElems);
}