...
kernel void initialize2(global double* g_u, constant double* g_ce, int gp0, int gp1, int gp2) {
  ...
  for (ix = 0; ix < 2; ix++) {
    exact_solution((double)ix, eta, zeta, &Pface[hook(10, ix)][hook(9, 0)][hook(8, 0)], g_ce);
  }

  for (iy = 0; iy < 2; iy++) {
    exact_solution(xi, (double)iy, zeta, &Pface[hook(10, iy)][hook(12, 1)][hook(11, 0)], g_ce);
  }

  for (iz = 0; iz < 2; iz++) {
    exact_solution(xi, eta, (double)iz, &Pface[hook(10, iz)][hook(14, 2)][hook(13, 0)], g_ce);
  }
