//{"addr_offset":6,"base_index":4,"group_offset":3,"input_data":1,"n":5,"output_data":0,"partial_sums":7,"shared_data":2}
...
kernel void UniformAddKernel(global int* output_data, global int* input_data, local int* shared_data, const unsigned int group_offset, const unsigned int base_index, const unsigned int n, const unsigned int addr_offset) {
  const unsigned int local_id = get_local_id(0);
  const unsigned int group_id = get_global_id(0) / get_local_size(0);
  const unsigned int group_size = get_local_size(0);

  if (local_id == 0)
    shared_data[hook(2, 0)] = input_data[hook(1, group_id + group_offset)];

  barrier(0x01);

  unsigned int address = mul24(group_id, (group_size << 1)) + base_index + local_id + addr_offset;

  output_data[hook(0, address)] += shared_data[hook(2, 0)];
  if ((local_id + group_size) < n)
    output_data[hook(0, address + group_size)] += shared_data[hook(2, 0)];
}