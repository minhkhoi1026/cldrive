...
kernel void lu_forward(global const unsigned int* row_indices, global const unsigned int* column_indices, global const float* elements, global float* vector, unsigned int size) {
  ...
  for (unsigned int i = get_local_id(0); i < loop_end; i += get_local_size(0)) {
    ...
    if (get_local_id(0) == 0) {
      for (unsigned int k = 0; k < get_local_size(0); ++k) {
        if (current_row < size && i + k == next_row) {
          vector[hook(3, current_row)] = current_vector_entry / diagonal_entry;
          ++current_row;
          if (current_row < size) {
        ...
        if (current_row < size && col_index_buffer[hook(6, k)] < current_row) {
          ...
          diagonal_entry = element_buffer[hook(5, k)];
      }
