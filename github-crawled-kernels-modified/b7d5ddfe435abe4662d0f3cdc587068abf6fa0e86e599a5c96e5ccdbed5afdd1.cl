...
kernel void gemm(const global float* A, const global float* B, global float* C, const int M, const int N, const int K) {
  ...
  for (int i = 0; i < K / NUM_UNROLL; ++i) {
    if (i % 2 == 0) {
      ...
      for (int k = 0; k < NUM_UNROLL; ++k) {
        regA[hook(12, 0)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 0)];
        regA[hook(12, 1)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 1)];
        regA[hook(12, 2)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 2)];
        regA[hook(12, 3)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 3)];
        regA[hook(12, 4)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 4)];
        regA[hook(12, 5)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 5)];
        regA[hook(12, 6)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 6)];
        regA[hook(12, 7)] = tileA_bak[hook(10, k * tile_dim_x + local_thread_id_x * 8 + 7)];

        regB[hook(13, 0)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 0)];
        regB[hook(13, 1)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 1)];
        regB[hook(13, 2)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 2)];
        regB[hook(13, 3)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 3)];
        regB[hook(13, 4)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 4)];
        regB[hook(13, 5)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 5)];
        regB[hook(13, 6)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 6)];
        regB[hook(13, 7)] = tileB_bak[hook(11, k * tile_dim_y + local_thread_id_y * 8 + 7)];
