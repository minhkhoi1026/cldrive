...
kernel void ParallelCompact(global int* CompactIn_data, global int* CompactOut_data, global int* numUsefulNodes, global int* ScanResult_data, global unsigned int* LevelLength_data, const unsigned int max_level_length) {
  const unsigned int node_id = get_global_id(0);
  const unsigned int move_id = get_global_id(1);

  int n = LevelLength_data[hook(4, move_id)] * 2;
