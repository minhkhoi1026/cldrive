...
kernel void prod_AA(global const float* A, unsigned int A_rows, unsigned int A_cols, unsigned int A_internal_rows, unsigned int A_internal_cols, global const float* B, unsigned int B_rows, unsigned int B_cols, unsigned int B_internal_rows, unsigned int B_internal_cols, global float* C, unsigned int C_rows, unsigned int C_cols, unsigned int C_internal_rows, unsigned int C_internal_cols, local float* bufA, local float* bufB) {
  ...
  if (get_global_id(0) < A_rows && get_global_id(1) < B_cols)
    C[hook(10, get_global_id(0) + get_global_id(1) * C_internal_rows)] = Csub;
}