...
kernel void MatVecMulCoalesced2(const global float* M, const global float* V, unsigned int width, unsigned int height, global float* W, local float* partialDotProduct) {
  for (unsigned int y = get_group_id(0); y < height; y += get_num_groups(0)) {
    ...
    for (unsigned int stride = get_local_size(0) / 2; stride > 0; stride /= 2) {
      ...
      if (get_local_id(0) < stride) {
        partialDotProduct[hook(5, get_local_id(0))] += partialDotProduct[hook(5, get_local_id(0) + stride)];
      }
    }
