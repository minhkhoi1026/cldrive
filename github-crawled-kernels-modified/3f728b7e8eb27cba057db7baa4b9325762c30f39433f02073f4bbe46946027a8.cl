...
kernel void noise_reduction_cl(global float4* src_buf, int src_stride, global float4* dst_buf, int dst_stride) {
  ...
  for (int axis = 0; axis < (8 / 2); axis++) {
    ...
    metric_reference[hook(5, axis)] = (((*center_pix) * 2 - (before_pix) - (after_pix)) * ((*center_pix) * 2 - (before_pix) - (after_pix)));
  }
  ...
  for (int direction = 0; direction < 8; direction++) {
    ...
    for (axis = 0; axis < (8 / 2); axis++) {
      ...
      float4 metric_new = (((value)*2 - (before_pix) - (after_pix)) * ((value)*2 - (before_pix) - (after_pix)));
