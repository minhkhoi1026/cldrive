//{"dH":13,"dW":14,"gradInput_data":0,"gradInput_offset":1,"gradOutput_data":2,"gradOutput_offset":3,"indices_data":4,"indices_offset":5,"indices_x_offset":6,"indices_y_offset":7,"input_h":9,"input_n":8,"input_w":10,"kH":11,"kW":12,"ptr_gradInput":15}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void maxgradinput(global float* gradInput_data, int gradInput_offset, global const float* gradOutput_data, int gradOutput_offset, global const float* indices_data, int indices_offset, int indices_x_offset, int indices_y_offset, int input_n, int input_h, int input_w, int kH, int kW, int dH, int dW) {
  ...
  for (yy = yy_start; yy < yy_end; yy += yy_step) {
    for (xx = xx_start; xx < xx_end; xx += xx_step) {
      ...
      ptr_gradInput[hook(15, argmax_x + argmax_y * input_w)] += z;
    }
  }
}