...
kernel void advect_field(float dt, global float* in, global float* out, global float* v_x, global float* v_y) {
  ...
  out[hook(2, id.x + id.y * dim.x)] = bilinear_interpolate(pos, in, dim.y, dim.x);
}