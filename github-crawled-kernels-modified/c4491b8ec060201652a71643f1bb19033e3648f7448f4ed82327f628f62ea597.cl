...
kernel void scharr3(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height) {
  constant const float matrixH[9] = {-3, -10, -3, 0, 0, 0, 3, 10, 3};

  constant const float matrixV[9] = {-3, 0, 3, -10, 0, 10, -3, 0, 3};

  float sumH = Convolution(source, src_step, dst_step, width, height, matrixH, 3);
  float sumV = Convolution(source, src_step, dst_step, width, height, matrixV, 3);
