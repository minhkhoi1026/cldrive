...
kernel void matrixTranspose(const int heightA, const int widthA, global const float* a, global float* a_T) {
  ...
  if ((rowA < heightA) && (colA < widthA)) {
    global float* src_vec1x4_0 = a + (rowA + 0) * widthA + (colA + 0);
    global float* src_vec1x4_1 = a + (rowA + 1) * widthA + (colA + 0);
    global float* src_vec1x4_2 = a + (rowA + 2) * widthA + (colA + 0);
    global float* src_vec1x4_3 = a + (rowA + 3) * widthA + (colA + 0);

    float4 vec1x4_0 = *((global float4*)src_vec1x4_0);
    float4 vec1x4_1 = *((global float4*)src_vec1x4_1);
    float4 vec1x4_2 = *((global float4*)src_vec1x4_2);
    float4 vec1x4_3 = *((global float4*)src_vec1x4_3);
