//{"batch_size":11,"in":1,"in_depth":4,"in_height":2,"in_width":3,"out":0,"padding_height":9,"padding_width":10,"pool_height":5,"pool_width":6,"stride_height":7,"stride_width":8}
...
kernel void feed_forward_average_pooling(global float* out, const global float* in, const int in_height, const int in_width, const int in_depth, const int pool_height, const int pool_width, const int stride_height, const int stride_width, const int padding_height, const int padding_width, const int batch_size) {
  ...
  for (int pr = 0; pr < pool_height; pr++)
    for (int pc = 0; pc < pool_width; pc++) {
      ...
      sum += in[hook(1, in_index)];
    }
  out[hook(0, offset)] = sum / pool_height / pool_width;
}