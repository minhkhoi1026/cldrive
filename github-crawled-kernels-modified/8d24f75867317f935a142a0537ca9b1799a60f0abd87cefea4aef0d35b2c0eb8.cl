//{"global_mem":0,"input":4,"key":1,"output":5,"s":3,"state":2}
...
kernel void encrypt1(global uint4* global_mem, const global uint4* key, local uint4* state) {
  const int rounds = 10;
  global_mem += get_global_id(0) * 32;
  state += get_local_id(0) * 32;
  bitslice(global_mem, state);
  for (int i = 0; i < 32; i++)
    state[hook(2, i)] ^= key[hook(1, i)];
  for (unsigned int i = 1; i < rounds; i++) {
    subBytes(state);
    shiftRows(state);
    mixColumns(state);
    for (int j = 0; j < 32; j++)
      state[hook(2, j)] = state[hook(2, j)] ^ key[hook(1, i * 32 + j)];
  }
  subBytes(state);
  shiftRows(state);
  for (int i = 0; i < 32; i++)
    state[hook(2, i)] ^= key[hook(1, 10 * 32 + i)];
