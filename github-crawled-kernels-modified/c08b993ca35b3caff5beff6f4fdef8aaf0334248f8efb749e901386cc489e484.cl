...
kernel void raycast_writeids(global unsigned int* a_screenBuffer, global float* a_backBuffer, global unsigned int* a_idBuffer, int res_x, int res_y, int frame)

{
  ...
  int ofs = x + y * res_x;
  int dstofs = a_idBuffer[hook(2, idx + idy * (res_x / 16) + size)] + size * 2;
  ...
  for (int j = 0; j < dy; ++j)
    for (int i = 0; i < dx; ++i) {
      ...
      if (a >= 4) {
        a_idBuffer[hook(2, dstofs)] = val;
        dstofs++;
        a_idBuffer[hook(2, dstofs)] = val + 1;
        dstofs++;
        a_idBuffer[hook(2, dstofs)] = val + 1 + (1 << 16);
        dstofs++;
        a_idBuffer[hook(2, dstofs)] = val + (1 << 16);
        dstofs++;
      }
    }
}