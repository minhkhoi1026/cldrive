...
constant float GLOHRadii[3] = {6.f, 11.f, 15.f};
...
inline void normalizeGLOHDesc(local float* desc, local float* accum, const int histlen, int lid_x, int lid_y, int lsz_x) {
  for (int i = lid_x; i < histlen; i += lsz_x)
    accum[hook(15, i)] = desc[hook(16, lid_y * histlen + i)] * desc[hook(16, lid_y * histlen + i)];
  barrier(0x01);
  ...
  if (lid_x < 128)
    accum[hook(15, lid_x)] += accum[hook(15, lid_x + 128)];
  barrier(0x01);
  if (lid_x < 64)
    accum[hook(15, lid_x)] += accum[hook(15, lid_x + 64)];
  barrier(0x01);
  ...
  if (lid_x < 16)
    ...
    accum[hook(15, lid_x)] += accum[hook(15, lid_x + 16)] + accum[hook(15, lid_x + 256)];
  barrier(0x01);
  if (lid_x < 8)
    accum[hook(15, lid_x)] += accum[hook(15, lid_x + 8)];
  barrier(0x01);
