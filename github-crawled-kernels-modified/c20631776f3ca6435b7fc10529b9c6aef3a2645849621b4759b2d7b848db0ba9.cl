//{"dest":1,"dst_step":3,"height":5,"mask_size":7,"matrix":6,"source":0,"src_step":2,"width":4}
...
kernel void gaussian_blur(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height, constant const float* matrix, private int mask_size) {
  const int gx = get_global_id(0);
  const int gy = get_global_id(1);
  const int2 pos = {gx, gy};
  src_step /= sizeof(uchar);
