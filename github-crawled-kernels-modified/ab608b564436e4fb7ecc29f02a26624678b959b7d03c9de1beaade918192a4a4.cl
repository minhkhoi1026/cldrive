//{"M":12,"MD":13,"Minvx":6,"a":10,"b":11,"dirs":2,"gamma":1,"lens":3,"m":7,"muA":0,"r":9,"rads":4,"t":8,"x":5}
...
kernel void calculate_Minv_x(const float muA, const float gamma, global const float4* dirs, global const float* lens, global const float* rads, global const float8* x, global float8* Minvx) {
  int i = get_global_id(0);

  float8 xi = x[hook(5, i)];
  float8 v = 0.f;
  v.s012 = xi.s012 / (muA * (lens[hook(3, i)] + 2.f * rads[hook(4, i)]));

  float4 Iinv[4];
  cyl_inv_inertia_tensor(muA, lens[hook(3, i)] + 2.f * rads[hook(4, i)], dirs[hook(2, i)], Iinv);
  float4 L = 0.f;
  L.s012 = xi.s345;
