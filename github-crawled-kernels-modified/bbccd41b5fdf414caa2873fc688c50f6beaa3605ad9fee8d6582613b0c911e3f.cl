//{"l":1,"m":2,"n":0,"output":3,"resolution":4}
...
float P(int a, int b, float x) {
  if (a == 0) {
    ...
    if (b == 0)
      ...
      return -(3.0f / 2.0f) * sqrt((float)1.0f - (x * x)) * (-1.0f + (5.0f * x * x));
    else if (b == 4)
...
float2 Y(int m, int l, float theta, float phi) {
  ...
  float2 eiStuff = cmul(cexp(cnew(0.0f, m * phi)), cnewf(P(m, l, native_cos(theta))));
  return cmul(root, eiStuff);
}

kernel void density(int n, int l, int m, global float* output, int resolution) {
  int gid = get_global_id(0);
  ...
  for (float z = -10.0f; z < 10.0f; z += 0.01f) {
    ...
    psi = cmul(cmul(cexp(cnewf(-(r / n * a))), cnewf(pow((float)(2.0f * r) / (n * a), (float)l))), cnewf(L(2 * l + 1, n - l - 1, ((2.0f * r) / (n * a)))));
    psi = cmul(psi, Y(m, l, theta, phi));

    psiStarPsi = cmul(cconj(psi), psi).x;

    output[hook(3, imgpos.x + (imgpos.y * resolution))] += psiStarPsi;
  }
}