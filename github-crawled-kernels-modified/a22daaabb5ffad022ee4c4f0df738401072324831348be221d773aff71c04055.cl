...
kernel void warpPerspectiveNN_C4_D0(global uchar4 const* restrict src, global uchar4* dst, int src_cols, int src_rows, int dst_cols, int dst_rows, int srcStep, int dstStep, int src_offset, int dst_offset, constant F* M, int threadCols) {
  ...
  if (dx < threadCols && dy < dst_rows) {
    ...
    if (dx >= 0 && dx < dst_cols && dy >= 0 && dy < dst_rows)
      dst[hook(1, (dst_offset >> 2) + dy * (dstStep >> 2) + dx)] = (sx >= 0 && sx < src_cols && sy >= 0 && sy < src_rows) ? src[hook(0, (src_offset >> 2) + sy * (srcStep >> 2) + sx)] : (uchar4)0;
  }
}