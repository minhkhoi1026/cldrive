//{"CompactIn_data":1,"CompactOut_data":0,"ScanResult_data":2,"input_data":6,"n":4,"numValidData":3,"output_data":7,"partial_sums":8,"shared_data":5}
...
void LoadLocalFromGlobal(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  shared_data[hook(5, local_index_a + bank_offset_a)] = input_data[hook(6, global_index_a)];
  shared_data[hook(5, local_index_b + bank_offset_b)] = input_data[hook(6, global_index_b)];
}
...
void LoadLocalFromGlobalNonPowerOfTwo(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  shared_data[hook(5, local_index_a + bank_offset_a)] = input_data[hook(6, global_index_a)];
  shared_data[hook(5, local_index_b + bank_offset_b)] = (local_index_b < n) ? input_data[hook(6, global_index_b)] : 0;
...
void StoreLocalToGlobal(global int* output_data, local const int* shared_data, const uint4 address_pair, const unsigned int n) {
  ...
  output_data[hook(7, global_index_a)] = shared_data[hook(5, local_index_a + bank_offset_a)];
  output_data[hook(7, global_index_b)] = shared_data[hook(5, local_index_b + bank_offset_b)];
}
...
void ClearLastElementStoreSum(local int* shared_data, global int* partial_sums, int group_index) {
  ...
  if (local_id == 0) {
    int index = (group_size << 1) - 1;
    index += ((index) >> (4));
    partial_sums[hook(8, group_index)] = shared_data[hook(5, index)];
    shared_data[hook(5, index)] = 0;
  }
}
...
unsigned int BuildPartialSum(local int* shared_data) {
  ...
  for (unsigned int j = group_size; j > 0; j >>= 1) {
    ...
    if (local_id < j) {
      ...
      shared_data[hook(5, local_index_b)] += shared_data[hook(5, local_index_a)];
    }
...
void ScanRootToLeaves(local int* shared_data, unsigned int stride) {
  ...
  for (unsigned int j = 1; j <= group_size; j *= two) {
    ...
    if (local_id < j) {
      ...
      int t = shared_data[hook(5, local_index_a)];
      shared_data[hook(5, local_index_a)] = shared_data[hook(5, local_index_b)];
      shared_data[hook(5, local_index_b)] += t;
    }
  }
}
...
kernel void ParallelCompactIndicesKernel(global int* CompactOut_data, global int* CompactIn_data, global int* ScanResult_data, global int* numValidData, const unsigned int n) {
  ...
  if (n == 0 && i == 0)
    *numValidData = 0;
    ...
    unsigned int addr = ScanResult_data[hook(2, i)];
    if (data > 0) {
      CompactOut_data[hook(0, addr)] = i;
      addr++;
    }
    if (i == n - 1)
      *numValidData = addr;
