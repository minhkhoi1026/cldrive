...
kernel void compiler_async_stride_copy(global char4* dst, global char4* src, local char4* localBuffer, int copiesPerWorkItem, int stride) {
  ...
  int i;
  event = async_work_group_strided_copy((local char4*)localBuffer, (global const char4*)(src + copiesPerWorkgroup * stride * get_group_id(0)), (size_t)copiesPerWorkgroup, (size_t)stride, (event_t)0);
  ...
  event = async_work_group_strided_copy((global char4*)(dst + copiesPerWorkgroup * stride * get_group_id(0)), (local const char4*)localBuffer, (size_t)copiesPerWorkgroup, (size_t)stride, (event_t)0);
  wait_group_events(1, &event);
}