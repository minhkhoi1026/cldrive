...
kernel void vglClFuzzyHamacherDilate(read_only image2d_t img_input, write_only image2d_t img_output, constant float* convolution_window, int window_size_x, int window_size_y, float gama) {
  ...
  for (int j = -factory; j <= factory; j++) {
    for (int i = -factorx; i <= factorx; i++) {
      ...
      float4 S = (a * b) / (gama + ((1 - gama) * (a + b - a * b)));
      pmax = max(pmax, S);
      conv_controller++;
    }
  }
