//{"bounds":4,"changed":13,"chunk":9,"cones":3,"count":7,"dir":1,"height":10,"index":12,"o":2,"size":8,"stack":6,"tHit":5,"threadsCount":11,"vertex":0}
...
void intersectAllLeaves(const global float* dir, const global float* o, const global float* bounds, global int* index, global float* tHit, float3 v1, float3 v2, float3 v3, float3 e1, float3 e2, int chunk, int rindex
  ...
  for (int i = 0; i < chunk; i++) {
    ...
    index[hook(12, rindex + i)] = get_global_id(0);
  }
}
...
kernel void IntersectionP(const global float* vertex, const global float* dir, const global float* o, const global float* cones, const global float* bounds, global unsigned char* tHit, local int* stack, int count, int size, int chunk, int height, unsigned int threadsCount) {
  ...
  for (int j = 0; j < levelcount; j++) {
    ...
    if (acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
      stack[hook(6, iLID * height + SPindex++)] = begin - 8 * lastlevelnum + 16 * j;
      while (SPindex > 0) {
        --SPindex;
        i = stack[hook(6, iLID * height + SPindex)];
        ...
        if (len < 0.000002f || acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
          ...
          if (child < 0) {
            ...
            intersectPAllLeaves(dir, o, bounds, tHit, v1, v2, v3, e1, e2, cones[hook(3, 7)], rindex);
          } else {
            stack[hook(6, iLID * height + SPindex++)] = child;
          }
        }
        ...
        if (len < 0.000002f || acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
          ...
          if (child < 0) {
            ...
            intersectPAllLeaves(dir, o, bounds, tHit, v1, v2, v3, e1, e2, cones[hook(3, i + 15)], rindex);
          } else {
            stack[hook(6, iLID * height + SPindex++)] = child;
          }
        }
