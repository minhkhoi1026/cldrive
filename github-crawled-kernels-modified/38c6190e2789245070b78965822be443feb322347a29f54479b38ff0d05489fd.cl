...
kernel void mvm_non_kernel_naive(int m, int n, global float* A, int lda, global float* x, int offsetX, global float* y, int offsetY) {
  unsigned int i, j;
  ...
  for (i = offset + tid; i < m_size; i += totalThreads) {
    sum = 0.f;
    for (j = 0; j < n_size; j++) {
      sum += A[hook(2, i * n_size + j)] * x[hook(4, j + offsetX)];
    }
