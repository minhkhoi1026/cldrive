//{"Gauss76Wt":17,"Gauss76Z":16,"Nbell_radius":7,"Nlength":9,"Nphi":11,"Nq":3,"Nradius":8,"Ntheta":10,"background":13,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":12,"sld":14,"solvent_sld":15}
...
kernel void barbell_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                         global float* loops_g,

                         local float* loops, const float cutoff, const int Nbell_radius, const int Nradius, const int Nlength, const int Ntheta, const int Nphi,

                         const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nbell_radius + Nradius + Nlength + Ntheta + Nphi) * 2, 0);
...

  {
    ...
    for (int bell_radius_i = 0; bell_radius_i < Nbell_radius; bell_radius_i++) {
      const float bell_radius = loops[hook(5, 2 * (bell_radius_i))];
      const float bell_radius_w = loops[hook(5, 2 * (bell_radius_i) + 1)];
      for (int radius_i = 0; radius_i < Nradius; radius_i++) {
        const float radius = loops[hook(5, 2 * (radius_i + Nbell_radius))];
        const float radius_w = loops[hook(5, 2 * (radius_i + Nbell_radius) + 1)];
        for (int length_i = 0; length_i < Nlength; length_i++) {
          const float length = loops[hook(5, 2 * (length_i + Nbell_radius + Nradius))];
          const float length_w = loops[hook(5, 2 * (length_i + Nbell_radius + Nradius) + 1)];
          for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
            const float theta = loops[hook(5, 2 * (theta_i + Nbell_radius + Nradius + Nlength))];
            const float theta_w = loops[hook(5, 2 * (theta_i + Nbell_radius + Nradius + Nlength) + 1)];
            for (int phi_i = 0; phi_i < Nphi; phi_i++) {
              const float phi = loops[hook(5, 2 * (phi_i + Nbell_radius + Nradius + Nlength + Ntheta))];
              const float phi_w = loops[hook(5, 2 * (phi_i + Nbell_radius + Nradius + Nlength + Ntheta) + 1)];

              const float weight = bell_radius_w * radius_w * length_w * theta_w * phi_w;
              if (weight > cutoff) {
                const float scattering = Iqxy(qxi, qyi, sld, solvent_sld, bell_radius, radius, length, theta, phi);
                if (!isnan(scattering)) {
