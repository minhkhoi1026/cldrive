//{"FEs":2,"dinvA":3,"dinvB":4,"dinvC":5,"imgIn":0,"points":1}
...
kernel void getFEinPoints(read_only image3d_t imgIn, global float4* points, global float4* FEs, float4 dinvA, float4 dinvB, float4 dinvC) {
  FEs[hook(2, get_global_id(0))] = interpFE(points[hook(1, get_global_id(0))].xyz, dinvA.xyz, dinvB.xyz, dinvC.xyz, imgIn);
}