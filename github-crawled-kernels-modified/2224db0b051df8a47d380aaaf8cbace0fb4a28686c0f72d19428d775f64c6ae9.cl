  ...
  for (unsigned int rows_processed = 0; rows_processed < A_size1; ++rows_processed) {
    ...
    if (!unit_diagonal_flag) {
      ...
      if (get_global_id(0) == 0)
        v[hook(9, row * v_inc + v_start)] /= A[hook(0, (row * A_inc1 + A_start1) * A_internal_size2 + (row * A_inc2 + A_start2))];
    }
    ...
    for (int elim = (is_lower_solve ? (row + get_global_id(0) + 1) : get_global_id(0)); elim < (is_lower_solve ? A_size1 : row); elim += get_global_size(0))
      v[hook(9, elim * v_inc + v_start)] -= temp * A[hook(0, transposed_access_A ? ((row * A_inc1 + A_start1) * A_internal_size2 + (elim * A_inc2 + A_start2)) : ((elim * A_inc1 + A_start1) * A_internal_size2 + (row * A_inc2 + A_start2)))];
  }
}