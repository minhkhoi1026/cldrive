//{"dt":0,"field":5,"v_x":3,"v_y":1,"v_y_out":2,"vals":4}
...
kernel void advect_vy(float dt, global float* v_y, global float* v_y_out, global float* v_x) {
  ...
  float2 vel = (float2)(bilinear_interpolate(x_pos, v_x, dim.y - 1, dim.x + 1), bilinear_interpolate(pos, v_y, dim.y, dim.x));

  pos -= 0.5f * dt * vel;
  x_pos = (float2)(pos.x + 0.5f, pos.y - 0.5f);
  vel = (float2)(bilinear_interpolate(x_pos, v_x, dim.y - 1, dim.x + 1), bilinear_interpolate(pos, v_y, dim.y, dim.x));
  pos -= dt * vel;

  v_y_out[hook(2, id.x + id.y * dim.x)] = bilinear_interpolate(pos, v_y, dim.y, dim.x);
}