...

{
 ...
 private
  ...
  galoisCoeff[hook(9, 0)] = 14;
  galoisCoeff[hook(9, 1)] = 11;
  galoisCoeff[hook(9, 2)] = 13;
  galoisCoeff[hook(9, 3)] = 9;

  block0[hook(4, localIndex)] ^= roundKey[hook(2, 4 * rounds + localIdy)];

  for (unsigned int r = rounds - 1; r > 0; --r) {
    block0[hook(4, localIndex)] = shiftRowsInv(block0[hook(4, localIndex)], localIdy);

    block0[hook(4, localIndex)] = sbox(SBox, block0[hook(4, localIndex)]);
    barrier(0x01);

    block1[hook(5, localIndex)] = block0[hook(4, localIndex)] ^ roundKey[hook(2, r * 4 + localIdy)];
    barrier(0x01);

    block0[hook(4, localIndex)] = mixColumns(block1, galoisCoeff, localIdy, localIndex, localSizex);
  }
  ...
  output[hook(0, globalIndex)] = block0[hook(4, localIndex)] ^ roundKey[hook(2, localIdy)];
}