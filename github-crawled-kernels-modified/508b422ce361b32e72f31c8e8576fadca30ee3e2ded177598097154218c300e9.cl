...
kernel void raycast_fine(global unsigned int* a_screenBuffer, global float* a_backBuffer, global unsigned int* a_octree, unsigned int octree_root, int res_x, int res_y, int frame, int add_x, int add_y, float4 a_cam, float4 a_origin, float4 a_dx, float4 a_dy,
  ...
  if (a_screenBuffer[hook(0, idy * res_x + idx)] == 0xffffff00) {
    for (int i = 0; i < 4; ++i) {
    ...
    for (int i = 0; i < 4; ++i) {
      float3 p;
      p.x = a_backBuffer[hook(1, ofs * 4 + 0)];
      p.y = a_backBuffer[hook(1, ofs * 4 + 1)];
      p.z = a_backBuffer[hook(1, ofs * 4 + 2)];
      if (length(p - phit) > 3) {
        a_screenBuffer[hook(0, ofs)] = col;
        a_backBuffer[hook(1, ofs * 4 + 0)] = phit.x;
        a_backBuffer[hook(1, ofs * 4 + 1)] = phit.y;
