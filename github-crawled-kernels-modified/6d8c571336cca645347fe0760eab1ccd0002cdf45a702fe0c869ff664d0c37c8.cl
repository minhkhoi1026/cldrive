...
__attribute__((reqd_work_group_size(1, 1, 10))) kernel void conv5(global float* in, global float* out, constant float* weight, constant float* offset) {
          ...
          for (int i = 0; i < 4; ++i) {
            ...
            for (int x = 0; x < 5; ++x) {
              for (int y = 0; y < 5; ++y, ++weightIdx) {
                outPrivate[hook(7, oPrivateIdx)] += inLocal[hook(4, ((i + iTile) * 5 + r + rTile + x) * 5 + c + cTile + y)] * weightLocal[hook(5, ((o + oTile) * 16 + i + iTile) * 25 + weightIdx)];
              }
            }
          }
        }
  ...
  for (int r = 0; r < 1; ++r) {
    for (int c = 0; c < 1; ++c) {
      for (int o = 0; o < 12; ++o, ++oPrivateIdx) {
        out[hook(1, ((o + oTile) * 1 + r + rTile) * 1 + c + cTile)] = sigmod(outPrivate[hook(7, oPrivateIdx)] + offsetLocal[hook(6, o + oTile)]);
      }
    }
  }
}