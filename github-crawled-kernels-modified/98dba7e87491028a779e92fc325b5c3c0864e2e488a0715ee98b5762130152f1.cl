...
kernel void pcr_branch_free_kernel(global float* a_d, global float* b_d, global float* c_d, global float* d_d, global float* x_d, local float* shared, int system_size, int num_systems, int iterations) {
  ...
  for (int j = 0; j < iterations; j++) {
    ...
    float tmp1 = a[hook(9, i)] / b[hook(10, iLeft)];
    float tmp2 = c[hook(11, i)] / b[hook(10, iRight)];
  ...
  if (thid < delta) {
    ...
    x[hook(13, addr1)] = (b[hook(10, addr2)] * d[hook(12, addr1)] - c[hook(11, addr1)] * d[hook(12, addr2)]) / tmp3;
    x[hook(13, addr2)] = (d[hook(12, addr2)] * b[hook(10, addr1)] - d[hook(12, addr1)] * a[hook(9, addr2)]) / tmp3;
  }
