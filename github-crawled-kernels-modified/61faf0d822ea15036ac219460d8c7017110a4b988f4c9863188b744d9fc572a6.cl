//{"A":0,"B":7,"C":2,"a":10,"b":9,"c":8,"k":6,"lda":1,"ldc":3,"m":4,"n":5}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void sgemm_buf_only(global const float* A, const int lda, global float* C, const int ldc, const int m, const int n, const int k, global const float* B) {
  ...
  if ((gx < n) && (gy < m)) {
    ...
    for (int pos = 0; pos < k; pos += 4) {
      for (int i = 0; i < 4; i++) {
        b[hook(9, i)] = vload4(0, B + (pos + i) * ldc + gx);
      }
