//{"heights":3,"keep":5,"n":7,"probs":4,"threshold":6,"widths":2,"xmins":0,"ymins":1}
...
kernel void nms_uo(global float* xmins, global float* ymins, global float* widths, global float* heights, global int* probs, global int* keep, float threshold, int n) {
  ...
  if (j < n) {
    ...
    if (iou_result > threshold) {
      if (probs[hook(4, j)] < probs[hook(4, i)]) {
        keep[hook(5, j)] = 0;
      } else {
        keep[hook(5, i)] = 0;
      }
