//{"base_index":5,"group_index":4,"input_data":1,"n":6,"output_data":0,"partial_sums":2,"shared_data":3}
...
void ScanRootToLeaves(local float* shared_data, unsigned int stride) {
  ...
  for (unsigned int j = 1; j <= group_size; j *= two) {
    ...
    if (local_id < j) {
      ...
      float t = shared_data[hook(3, local_index_a)];
...
kernel void PreScanStoreSumNonPowerOfTwoKernel(global float* output_data, global const float* input_data, global float* partial_sums, local float* shared_data, const unsigned int group_index, const unsigned int base_index, const unsigned int n) {
  const unsigned int local_id = get_local_id(0);
  const unsigned int group_id = get_global_id(0) / get_local_size(0);
  const unsigned int group_size = get_local_size(0);
