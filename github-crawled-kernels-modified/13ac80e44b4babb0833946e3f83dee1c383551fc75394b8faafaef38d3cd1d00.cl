...
kernel void mtxtex_cpx2cpx(read_only image2d_t samples, global float2* result, int N, global float2* coeffs) {
  ...
  for (int j = 0; j < N; j++)
    sum += cmult(read_imagef(samples, sampler, (int2)(0, j)).xy, coeffs[hook(3, j * N + k)]);
