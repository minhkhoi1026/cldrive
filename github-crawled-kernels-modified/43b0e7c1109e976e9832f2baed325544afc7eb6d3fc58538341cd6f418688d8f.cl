...
kernel void gemm(const global float* A, const global float* B, global float* C, const int M, const int N, const int K) {
 ...
 private
  ...
  for (int i = 0; i < K / NUM_UNROLL; ++i) {
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 0 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 0 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 1 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 1 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 2 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 2 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 3 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 3 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 4 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 4 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 5 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 5 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 6 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 6 * tile_dim_y + local_thread_id_y * 8 + 7)];
    ...
    regA[hook(11, 0)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 0)];
    regA[hook(11, 1)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 1)];
    regA[hook(11, 2)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 2)];
    regA[hook(11, 3)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 3)];
    regA[hook(11, 4)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 4)];
    regA[hook(11, 5)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 5)];
    regA[hook(11, 6)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 6)];
    regA[hook(11, 7)] = tileA[hook(9, swap_index + 7 * tile_dim_x + local_thread_id_x * 8 + 7)];

    regB[hook(12, 0)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 0)];
    regB[hook(12, 1)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 1)];
    regB[hook(12, 2)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 2)];
    regB[hook(12, 3)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 3)];
    regB[hook(12, 4)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 4)];
    regB[hook(12, 5)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 5)];
    regB[hook(12, 6)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 6)];
    regB[hook(12, 7)] = tileB[hook(10, swap_index + 7 * tile_dim_y + local_thread_id_y * 8 + 7)];
