...
kernel void bpnn_layerforward_ocl(global float* input_cuda, global float* output_hidden_cuda, global float* input_hidden_cuda, global float* hidden_partial_sum, local float* input_node, local float* weight_matrix, int in, int hid) {
  ...
  for (int i = 1; i <= log2((float)16); i++) {
    int power_two = pow((float)2, i);
