...
kernel void Sspr2_naive_upper(int n, float alpha, global float* x, int incx, global float* y, int incy, global float* AP) {
  ...
  if (col_id >= row_id) {
    ...
    AP[hook(6, row_id + (col_id * (col_id + 1)) / 2)] = res_1 + res_2 + AP[hook(6, row_id + (col_id * (col_id + 1)) / 2)];
  }
}