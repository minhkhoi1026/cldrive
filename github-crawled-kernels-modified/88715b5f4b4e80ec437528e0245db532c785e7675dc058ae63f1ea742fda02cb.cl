//{"((__local unsigned int *)smem)":12,"((__local unsigned int *)src)":13,"blockx":9,"global_histo":7,"global_overflow":8,"global_subhisto":6,"histo_height":4,"histo_width":5,"num_elements":1,"sm_mappings":0,"sm_range_max":3,"sm_range_min":2,"smem":11,"smem[indexhi]":10}
...
void testIncrementLocal(global unsigned int* global_overflow, local unsigned int smem[2][256], const unsigned int myRange, const uchar4 sm) {
  ...
  if (range == myRange) {
    ...
    unsigned int prev = atom_add(&smem[hook(11, indexhi)][hook(10, indexlo)], add);
...
void clearMemory(local unsigned int smem[2][256]) {
  for (int i = get_local_id(0), blockDimx = get_local_size(0); i < 8 / 4; i += blockDimx) {
    ((local unsigned int*)smem)[hook(12, i)] = 0;
  }
}

void copyMemory(global unsigned int* dst, local unsigned int src[2][256]) {
  for (int i = get_local_id(0), blockDimx = get_local_size(0); i < 8 / 4; i += blockDimx) {
    atom_add(dst + i * 4, (((local unsigned int*)src)[hook(13, i)] >> 0) & 0xFF);
    atom_add(dst + i * 4 + 1, (((local unsigned int*)src)[hook(13, i)] >> 8) & 0xFF);
    atom_add(dst + i * 4 + 2, (((local unsigned int*)src)[hook(13, i)] >> 16) & 0xFF);
    atom_add(dst + i * 4 + 3, (((local unsigned int*)src)[hook(13, i)] >> 24) & 0xFF);
  }
}
...
kernel void histo_main_kernel(global uchar4* sm_mappings, unsigned int num_elements, unsigned int sm_range_min, unsigned int sm_range_max, unsigned int histo_height, unsigned int histo_width, global unsigned int* global_subhisto, global unsigned int* global_histo, global unsigned int* global_overflow,
  ...
  unsigned int store_index = (local_scan_range * 8);
