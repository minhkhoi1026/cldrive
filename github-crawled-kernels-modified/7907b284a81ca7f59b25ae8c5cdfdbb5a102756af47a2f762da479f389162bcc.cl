...
kernel void subtended_angle2_naive(global float* x1, global float* y1, float x2, float y2, float rx, float ry, float sw, float sh, float ez, float ex, float ey, global float* out) {
  unsigned int i = get_global_id(0);
  out[hook(11, i)] = _subtended_angle(x1[hook(0, i)], y1[hook(1, i)], x2, y2, rx, ry, sw, sh, ez, ex, ey);
}