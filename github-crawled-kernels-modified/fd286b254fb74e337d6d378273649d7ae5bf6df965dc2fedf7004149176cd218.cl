...
__attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(8, 8, 1))) kernel void sqr_F64(global const uchar* source, global double* dest, int src_step, int dst_step, int width, int height) {
  ...
  double Sum = 0;
  for (int i = 0; i < 8; i++) {
    double v = convert_double(source[hook(0, gy * src_step + gx + i)]);
    Sum += v * v;
