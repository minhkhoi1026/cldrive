//{"dt":0,"field":5,"v_x":1,"v_x_out":2,"v_y":3,"vals":4}
...
kernel void advect_vx(float dt, global float* v_x, global float* v_x_out, global float* v_y) {
  ...
  float2 vel = (float2)(bilinear_interpolate(pos, v_x, dim.y, dim.x), bilinear_interpolate(y_pos, v_y, dim.y + 1, dim.x - 1));

  pos -= 0.5f * dt * vel;
  y_pos = (float2)(pos.x - 0.5f, pos.y + 0.5f);
  vel = (float2)(bilinear_interpolate(pos, v_x, dim.y, dim.x), bilinear_interpolate(y_pos, v_y, dim.y + 1, dim.x - 1));
  pos -= dt * vel;

  v_x_out[hook(2, id.x + id.y * dim.x)] = bilinear_interpolate(pos, v_x, dim.y, dim.x);
}