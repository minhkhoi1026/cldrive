...
kernel void max_bw_kernel_256(const global float* px, const global float* py, const global float* pgy, const unsigned skip, const unsigned n, global float* pgx) {
  const unsigned bid = get_group_id(0);
  const unsigned tid = get_local_id(0);
  const float max_val = py[hook(1, bid)];
  local unsigned argmax_val[256];
