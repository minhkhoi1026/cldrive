...
kernel void integral_sum_cols_D4(global uchar4* src, global int* sum, int src_offset, int pre_invalid, int rows, int cols, int src_step, int dst_step) {
  ...
  for (int i = 0; i < rows; i = i + 255) {
    src_t[hook(8, 0)] = (i + lid < rows ? convert_int4(src[hook(0, src_offset + (lid + i) * src_step + gid)]) : 0);
    src_t[hook(8, 1)] = (i + lid < rows ? convert_int4(src[hook(0, src_offset + (lid + i) * src_step + gid + 1)]) : 0);

    sum_t[hook(9, 0)] = (i == 0 ? 0 : lm_sum[hook(11, 0)][hook(10, 254 + 8)]);
    sum_t[hook(9, 1)] = (i == 0 ? 0 : lm_sum[hook(11, 1)][hook(12, 254 + 8)]);
    barrier(0x01);
    ...
    if (lid > 0 && (i + lid) <= rows) {
      ...
      sum_p = (local int*)(&(lm_sum[hook(11, 0)][hook(10, bf_loc)]));
      for (int k = 0; k < 4; k++) {
        if (gid * 4 + k >= cols + pre_invalid || gid * 4 + k < pre_invalid)
          continue;
        sum[hook(1, loc_s0 + k * dst_step / 4)] = sum_p[hook(15, k)];
      }
      sum_p = (local int*)(&(lm_sum[hook(11, 1)][hook(12, bf_loc)]));
      for (int k = 0; k < 4; k++) {
