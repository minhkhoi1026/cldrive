//{"g_atomic_input":1,"g_special_values":2,"l_5":4,"p_13->g_10":6,"p_13->g_2":5,"result":0,"sequence_input":3}
...
int func_1(struct S0* p_13) {
  ...
  int i;
  for (i = 0; i < 2; i++)
    l_5[hook(4, i)] = &p_13->g_3;
  for (p_13->g_3 = 0; (p_13->g_3 <= 0); p_13->g_3 += 1) {
    int i;
    return p_13->g_2[hook(5, p_13->g_3)];
  }
  p_13->g_10[hook(6, 3)]++;
  return p_13->g_9;
}
...
kernel void entry(global ulong* result, global volatile unsigned int* g_atomic_input, global volatile unsigned int* g_special_values, global int* sequence_input) {
  ...
  for (i = 0; i < 1; i++) {
    transparent_crc_no_string(&crc64_context, p_13->g_2[hook(5, i)]);
  }
  ...
  for (i = 0; i < 9; i++) {
    transparent_crc_no_string(&crc64_context, p_13->g_10[hook(6, i)]);
  }
