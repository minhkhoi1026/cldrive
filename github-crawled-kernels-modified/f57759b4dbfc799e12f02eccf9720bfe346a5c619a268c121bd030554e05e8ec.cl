...
kernel void compute_stereo_vertical_dir_kernel_6(global const uchar4* d_matching_cost, global ushort* d_scost, int width, int height) {
  local ushort2 lcost_sh[128 * 8 / 2];
  local ushort minCostNext[32 * 8];

  init_lcost_sh_128(lcost_sh);
  int j = get_group_id(0) * 8 + get_local_id(1);
  int minCost = 0;
