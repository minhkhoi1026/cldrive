//{"aux":3,"p":4,"pesos":2,"start":0,"stop":1}
...
void mergeA(global struct float4* p, int start, int m, int stop, global struct float4* aux) {
  for (int i = start; i < m; i++)
    aux[hook(3, i)] = p[hook(4, i)];
  for (int j = m; j < stop; j++)
    aux[hook(3, j)] = p[hook(4, m + stop - j - 1)];
  int i = start, j = stop - 1;
  for (int k = start; k < stop; k++)
    if (aux[hook(3, j)].w > aux[hook(3, i)].w)
      p[hook(4, k)] = aux[hook(3, j--)];
    else
      p[hook(4, k)] = aux[hook(3, i++)];
}

kernel void calc_scan_total(int start, int stop, global int* pesos) {
  int base = get_global_id(0);
  int tam = get_global_size(0);
  int ini = start, fin = stop, sumador = pesos[hook(2, ini - 1)];

  for (int i = ini + base; i < stop; i += tam)
    pesos[hook(2, i)] += sumador;
}