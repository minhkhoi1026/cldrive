...
kernel void Sgemm_ntransAB(unsigned int k, float alpha, global float* A, unsigned int lda, global float* B, unsigned int ldb, float beta, global float* C, unsigned int ldc) {
  ...
  for (unsigned int i = 0; i < k; i++) {
    unsigned int indexA = i * lda + row;
    float valueA = A[hook(2, indexA)];

    unsigned int indexB = column * ldb + i;
