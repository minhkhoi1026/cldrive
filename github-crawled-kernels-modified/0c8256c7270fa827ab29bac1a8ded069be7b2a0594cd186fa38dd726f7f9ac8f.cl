...
kernel void mmmKernel_local(global double4* matrixA, global double4* matrixB, global double4* matrixC, int widthA, local double4* blockA) {
  ...
  for (int i = 0; i < (temp / get_local_size(0)); i++) {
    ...
    for (int j = 0; j < get_local_size(0) * 4; j = j + 4) {
      double4 tempA0 = blockA[hook(4, (j >> 2) + get_local_id(1) * 4 * get_local_size(0))];
      double4 tempA1 = blockA[hook(4, (j >> 2) + (get_local_id(1) * 4 + 1) * get_local_size(0))];
      double4 tempA2 = blockA[hook(4, (j >> 2) + (get_local_id(1) * 4 + 2) * get_local_size(0))];
      double4 tempA3 = blockA[hook(4, (j >> 2) + (get_local_id(1) * 4 + 3) * get_local_size(0))];

      double4 tempB0 = matrixB[hook(1, globalPosB + j * get_global_size(0))];
      double4 tempB1 = matrixB[hook(1, globalPosB + (j + 1) * get_global_size(0))];
      double4 tempB2 = matrixB[hook(1, globalPosB + (j + 2) * get_global_size(0))];
      double4 tempB3 = matrixB[hook(1, globalPosB + (j + 3) * get_global_size(0))];
