//{"N":0,"aT":4,"alpha":6,"b":5,"constMem":3,"current":1,"sm":2}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void FWD_update_alpha(const int N, const int current, local float* sm, constant float* constMem, global float* aT, global float* b, global float* alpha) {
  ...
  for (i = 0; i < iters; ++i) {
    int col = i * 16 + lx;
    data += aT[hook(4, gy * N + col)] * constMem[hook(3, col)];
  }
  ...
  if (gx == 0) {
    ...
    alpha[hook(6, current + gy)] = data * b[hook(5, current + gy)];
  }
}