...
kernel void integral_cols_D5(global uchar4* src, global float* sum, global float* sqsum, int src_offset, int pre_invalid, int rows, int cols, int src_step, int dst_step) {
  ...
  for (int i = 0; i < rows; i = i + 255) {
    src_t[hook(9, 0)] = (i + lid < rows ? convert_float4(src[hook(0, src_offset + (lid + i) * src_step + min(gid, cols - 1))]) : (float4)0);
    src_t[hook(9, 1)] = (i + lid < rows ? convert_float4(src[hook(0, src_offset + (lid + i) * src_step + min(gid + 1, cols - 1))]) : (float4)0);

    sum_t[hook(10, 0)] = (i == 0 ? (float4)0 : lm_sum[hook(12, 0)][hook(11, 254 + 8)]);
    sqsum_t[hook(13, 0)] = (i == 0 ? (float4)0 : lm_sqsum[hook(15, 0)][hook(14, 254 + 8)]);
    sum_t[hook(10, 1)] = (i == 0 ? (float4)0 : lm_sum[hook(12, 1)][hook(16, 254 + 8)]);
    ...
    if (lid > 0 && (i + lid) <= rows) {
      ...
      sum_p = (local float*)(&(lm_sum[hook(12, 0)][hook(11, bf_loc)]));
      ...
      sum_p = (local float*)(&(lm_sum[hook(12, 1)][hook(16, bf_loc)]));
