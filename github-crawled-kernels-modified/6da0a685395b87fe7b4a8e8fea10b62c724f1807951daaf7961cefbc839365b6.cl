...
kernel void finalize(global char* trit_hash, global long* mid_low, global long* mid_high, global long* state_low, global long* state_high, constant size_t* min_weight_magnitude, global volatile char* found, global long* nonce_probe, constant size_t* loop_count) {
 private
  size_t i, j, id, gid, gr_id, l_size, n_trits;
  setup_ids(&id, &gid, &gr_id, &l_size, &n_trits);

  if (gr_id == (size_t)(*found - 1) && nonce_probe[hook(7, gr_id)] != 0) {
    for (i = 0; i < n_trits; i++) {
      j = id + i * l_size;
      if (j < 243) {
        trit_hash[hook(0, j)] = (mid_low[hook(1, gid + j)] & nonce_probe[hook(7, gr_id)]) == 0 ? 1 : (mid_high[hook(2, gid + j)] & nonce_probe[hook(7, gr_id)]) == 0 ? -1 : 0;
      }
    }
  }
}