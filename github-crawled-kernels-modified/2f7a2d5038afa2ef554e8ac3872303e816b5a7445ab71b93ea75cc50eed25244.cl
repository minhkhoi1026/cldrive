//{"CompactIn_data":0,"CompactOut_data":1,"LevelLength_data":4,"ScanResult_data":3,"input_data":7,"max_level_length":5,"numUsefulNodes":2,"output_data":8,"partial_sums":9,"shared_data":6}
...
void LoadLocalFromGlobalFirstLevel(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  int input_data_a = input_data[hook(7, global_index_a)];
  int input_data_b = input_data[hook(7, global_index_b)];

  shared_data[hook(6, local_index_a + bank_offset_a)] = input_data_a > -1 ? 1 : 0;
  shared_data[hook(6, local_index_b + bank_offset_b)] = input_data_b > -1 ? 1 : 0;
}
...
void LoadLocalFromGlobalNonPowerOfTwoFirstLevel(local int* shared_data, global const int* input_data, const uint4 address_pair, const unsigned int n) {
  ...
  if (local_index_b < n) {
    ...
    shared_data[hook(6, local_index_b + bank_offset_b)] = 0;
...
kernel void ParallelCompactFinal(global int* CompactIn_data, global int* CompactOut_data, global int* numUsefulNodes, global int* ScanResult_data, global unsigned int* LevelLength_data, const unsigned int max_level_length) {
  const unsigned int node_id = get_global_id(0);
  const unsigned int move_id = get_global_id(1);

  int n = LevelLength_data[hook(4, move_id)];

  if (n == 0 && node_id == 0)
    numUsefulNodes[hook(2, move_id)] = 0;

  if (node_id < n) {
    const unsigned int offset = move_id * max_level_length;

    int data = CompactIn_data[hook(0, offset + node_id)];
    unsigned int addr = ScanResult_data[hook(3, offset + node_id)];
    if (data > -1) {
      CompactOut_data[hook(1, offset + addr)] = data;
      addr++;
    }
    if (node_id == n - 1)
      numUsefulNodes[hook(2, move_id)] = addr;
  }
}