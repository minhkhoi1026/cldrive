...
kernel void fft1D_512(global const float2* input, global float2* output, constant const float* win) {
  local float2 buf[512];

  float2 r[8];
  int lid = get_local_id(0);
  int i;

  input += 512 * get_global_id(1);
  output += 512 * get_global_id(1);

  for (i = lid; i < 512; i += (512 / 8))
    buf[hook(4, i)] = input[hook(0, i)] * win[hook(2, i)];

  fft_radix8(buf, r, 1, lid, (512 / 8), 0);

  fft_radix8(buf, r, 8, lid, (512 / 8), 1);

  fft_radix8(buf, r, 64, lid, (512 / 8), 1);

  for (i = 0; i < 8; i++)
    output[hook(1, i * (512 / 8) + lid)] = buf[hook(4, i * (512 / 8) + lid)];
}