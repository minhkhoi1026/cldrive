...
kernel void MatrixMultiplication_double(global double* matrixA, global double* matrixB, global double* output, int matrixSize, int matrixWidth, local int* localA, local int* localB, int localSize) {
  double sum;
  ...
  for (int i = 0; i < matrixWidth / localSize; i++) {
    ...
    for (int j = 0; j < localSize; j++) {
      sum += localA[hook(5, lx * localSize + j)] * localB[hook(6, j * localSize + ly)];
    }
    barrier(0x01);
  }
