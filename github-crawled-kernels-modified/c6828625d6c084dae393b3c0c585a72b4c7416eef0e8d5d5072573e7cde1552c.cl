...
kernel void amdFHT128FromQ(global const float* in, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scaling, int chnl_stride, int que_ln, int frame_ln, int current_bin) {
  ...
  for (int i = lcl_id; i < 128 / 2; i += 64) {
    data[hook(10, gbitreverse[1hook(2, 128 / 2 + i))] = in[hook(0, i + chnl_off + bin_off)];
  }
