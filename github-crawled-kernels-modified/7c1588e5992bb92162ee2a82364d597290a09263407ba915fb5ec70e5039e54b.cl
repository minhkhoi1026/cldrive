//{"FE":3,"LATOMS":4,"atoms":1,"nAtoms":0,"poss":2}
...
kernel void evalCoulomb(int nAtoms, global float4* atoms, global float4* poss, global float4* FE) {
  local float4 LATOMS[32];
  const int iG = get_global_id(0);
  const int iL = get_local_id(0);
  const int nL = get_local_size(0);

  float3 pos = poss[hook(2, iG)].xyz;
  ...
  for (int i0 = 0; i0 < nAtoms; i0 += nL) {
    ...
    LATOMS[hook(4, iL)] = atoms[hook(1, i)];
    barrier(0x01);
    for (int j = 0; j < nL; j++) {
      fe += getCoulomb(LATOMS[hook(4, j)], pos);
    }
    barrier(0x01);
  }
  FE[hook(3, iG)] = fe;
