...
kernel void warpAffineCubic_C1_D0(global uchar* src, global uchar* dst, int src_cols, int src_rows, int dst_cols, int dst_rows, int srcStep, int dstStep, int src_offset, int dst_offset, constant F* M, int threadCols) {
  ...
  if (dx < threadCols && dy < dst_rows) {
    ...
    int X = X0 >> (max(10, (int)5) - 5);
    int Y = Y0 >> (max(10, (int)5) - 5);

    short sx = (short)(X >> 5) - 1;
    short sy = (short)(Y >> 5) - 1;
    ...
    for (i = 0; i < 4; i++)
      for (j = 0; j < 4; j++) {
        v[hook(13, i * 4 + j)] = (sx + j >= 0 && sx + j < src_cols && sy + i >= 0 && sy + i < src_rows) ? src[hook(0, src_offset + (sy + i) * srcStep + (sx + j))] : 0;
      }
