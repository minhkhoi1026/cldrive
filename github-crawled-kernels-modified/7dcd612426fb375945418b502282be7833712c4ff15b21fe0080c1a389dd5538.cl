...
__attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) kernel void bilateral_interpolate(global const float4* input, global const float2* blurz_r, global const float2* blurz_g, global const float2* blurz_b, global const float2* blurz_a, global float4* smoothed, int width, int sw, int sh, int depth, int s_sigma, float r_sigma) {
