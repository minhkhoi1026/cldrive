...
kernel void MeanFilterPad2_H(global const uchar* ImIn, global const uchar* ImMask, global int2* ImOut, int Height, int Width, int Radius) {
  ...
  LocalData[hook(7, LocalPosX)] = convert_int(ImIn[hook(0, Index(GlobalPosY, Col, Height, Width))]);
  ...
  for (; Col <= GlobalPosX + Radius; Col += LocalSizeX) {
    LocalData[hook(7, LocalPosX + LocalSizeX)] = convert_int(ImIn[hook(0, Index(GlobalPosY, Col + LocalSizeX, Height, Width))]);
    ...
    for (int j = 0; j < LocalSizeX && (Col + j <= GlobalPosX + Radius); ++j) {
      ...
      Counter = Counter + LocalMask[hook(8, LocalPosX + j)];
    }
