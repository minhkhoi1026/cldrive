...
__attribute__((reqd_work_group_size(8, 8, 1))) kernel void sqr_F32(global const uchar* source, global float* dest, int src_step, int dst_step, int width, int height) {
  ...
  float Sum = 0;
  for (int i = 0; i < 8; i++) {
    float v = convert_float(source[hook(0, gy * src_step + gx + i)]);
    Sum += v * v;
