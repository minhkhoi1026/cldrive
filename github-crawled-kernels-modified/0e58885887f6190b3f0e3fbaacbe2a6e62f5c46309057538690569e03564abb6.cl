...
kernel void bloom_vblur(read_only image2d_t in, write_only image2d_t out, const int rad, const int width, const int height, const int blocksize, local float* buffer) {
  ...
  for (int n = 0; n <= rad / lsz; n++) {
    ...
    buffer[hook(6, rad - l)] = read_imagef(in, sampleri, (int2)(x, yy)).x;
  }
  ...
  for (int n = 0; n <= rad / lsz; n++) {
    ...
    buffer[hook(6, rad + lsz - 1 + r)] = read_imagef(in, sampleri, (int2)(x, yy)).x;
  }
