//{"<recovery-expr>(v_data)":5,"v_data":1,"v_s0":3,"v_s2_diffuse":2,"v_seed":4,"v_sizes":0}
...
kernel void s2_diffuse_run(global int* v_sizes, global unit_data* v_data, global int* v_s2_diffuse, global int* v_s0, global long* v_seed) {
  int global_id = get_global_id(0);
  if (global_id >= v_sizes[hook(0, 4)])
    return;

  int this_id = v_s2_diffuse[hook(2, global_id)];
  unit_data s2 = v_data[this_id];

  bool dir = dot(s2.in_dir, s2.normal) < 0;

  long rand_seed = v_seed[hook(4, global_id)] + global_id;
  ...
  v_data[hook(5, this_id)].strength = strength;
  v_data[hook(5, this_id)].start_p = s2.intersect_p + 1e-5f * p;
  v_data[hook(5, this_id)].in_dir = p;
  v_seed[hook(4, global_id)] = rand_seed;
}