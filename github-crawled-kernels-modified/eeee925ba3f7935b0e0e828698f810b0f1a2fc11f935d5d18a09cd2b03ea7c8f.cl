//{"(input)":4,"(output)":13,"SB1":12,"SB2":8,"SB3":11,"SB4":7,"SB5":10,"SB6":6,"SB7":9,"SB8":5,"SK":1,"in":0,"input_size":3,"out":2,"state_in":14,"state_out":15}
...
__attribute__((reqd_work_group_size(1, 1, 1))) __attribute__((reqd_work_group_size(1, 1, 1))) kernel void des3Cipher(global uchar* restrict in, global unsigned int* restrict SK, global uchar* restrict out, unsigned int input_size) {
 private
  uchar state_in[8];
 private
  uchar state_out[8];
  for (size_t blockid = 0; blockid < input_size / 8; blockid++) {
    for (size_t i = 0; i < 8; ++i) {
      size_t offset = blockid * 8 + i;
      state_in[hook(14, i)] = in[hook(0, offset)];
    }
    des3_crypt(SK, state_in, state_out);
    for (size_t i = 0; i < 8; i++) {
      size_t offset = blockid * 8 + i;
      out[hook(2, offset)] = state_out[hook(15, i)];
    }
  }
}