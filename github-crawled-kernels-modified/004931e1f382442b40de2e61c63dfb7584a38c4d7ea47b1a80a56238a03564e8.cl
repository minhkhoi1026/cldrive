//{"count":6,"p00":0,"p01":1,"p02":2,"p10":3,"p11":4,"p12":5}
...
kernel void findTrianglesIntersections(global const float4* p00, global const float4* p01, global const float4* p02, global const float4* p10, global const float4* p11, global const float4* p12, global unsigned int* count) {
  ...
  bool a = doTrianglesIntersect(p00[hook(0, i)], p01[hook(1, i)], p02[hook(2, i)], p10[hook(3, j)], p11[hook(4, j)], p12[hook(5, j)]);
  if (a) {
    atomic_inc(count);
  }
}