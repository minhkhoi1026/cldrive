//{"M_g":0,"M_o":3,"P1_g":1,"P1_o":4,"P2_g":2,"P2_o":5,"X_size":8,"Y_size":9,"cos_val":6,"sin_val":7,"step":10}
...
kernel void JFA(global const unsigned int* M_g, global const unsigned int* P1_g, global const unsigned int* P2_g, global unsigned int* M_o, global unsigned int* P1_o, global unsigned int* P2_o, global const float* cos_val, global const float* sin_val, int X_size, int Y_size, int step) {
  int gid = get_global_id(0);
  int y = gid % Y_size;
  int x = (gid - y) / Y_size;
  ...
  for (int i = 0; i < 12; i++) {
    int A = (step * cos_val[hook(6, i)]);
    int B = (step * sin_val[hook(7, i)]);
    int nx = x + A;
    int ny = y + B;
