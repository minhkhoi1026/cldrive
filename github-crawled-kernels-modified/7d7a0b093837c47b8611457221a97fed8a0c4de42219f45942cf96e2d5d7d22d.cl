...
__attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) __attribute__((reqd_work_group_size(16, 16, 1))) kernel void reduce_minabs_flush(global const uchar* source, global float* result, int src_step, int img_width, int img_height) {
  ...
  barrier(0x01);
  if (lid < 128)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 128)]);
  barrier(0x01);
  if (lid < 64)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 64)]);
  barrier(0x01);
  if (lid < 32)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 32)]);
  barrier(0x01);
  if (lid < 16)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 16)]);
  barrier(0x01);
  if (lid < 8)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 8)]);
  barrier(0x01);
  if (lid < 4)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 4)]);
  barrier(0x01);
  if (lid < 2)
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 2)]);
  barrier(0x01);
  if (lid == 0) {
    buffer[hook(5, lid)] = min(buffer[hook(5, lid)], buffer[hook(5, lid + 1)]);
    atomic_minf_1(result, buffer[hook(5, 0)], 16 * 16 * 16);
  }
}