//{"buff":0,"d_matching_cost":4,"d_scost":6,"lcost_sh":5,"minCostNext":2,"sh":1,"values":3}
...
inline void init_lcost_sh_128(local ushort2* sh) {
  sh[hook(1, 128 * get_local_id(1) / 2 + get_local_id(0) * 2 + 0)] = (ushort2)(0);
  sh[hook(1, 128 * get_local_id(1) / 2 + get_local_id(0) * 2 + 1)] = (ushort2)(0);
  barrier(0x01);
}
...
inline int min_warp(local ushort* minCostNext) {
  ...
  for (int offset = 32 / 2; offset > 0; offset = offset / 2) {
    if (get_local_id(0) < offset) {
      ushort other = minCostNext[hook(2, local_index + offset)];
      ushort mine = minCostNext[hook(2, local_index)];
      minCostNext[hook(2, local_index)] = (mine < other) ? mine : other;
    }
    barrier(0x01);
  }

  return minCostNext[hook(2, get_local_id(1) * 32)];
}
...
inline int min_warp_int(local int* values) {
  ...
  for (int offset = 32 / 2; offset > 0; offset = offset / 2) {
    if (get_local_id(0) < offset) {
      int other = values[hook(3, local_index + offset)];
      int mine = values[hook(3, local_index)];
      values[hook(3, local_index)] = (mine < other) ? mine : other;
    }
    barrier(0x01);
  }

  return values[hook(3, get_local_id(1) * 32)];
}
...
inline int stereo_loop_128(int i, int j, global const uchar4* d_matching_cost, global ushort* d_scost, int width, int height, int minCost, local ushort2* lcost_sh, local ushort* minCostNext) {
  ...
  uchar4 diff_tmp = d_matching_cost[hook(4, idx * 128 / 4 + k)];

  ushort2 v_diff_L = (ushort2)(diff_tmp.y, diff_tmp.x);
  ushort2 v_diff_H = (ushort2)(diff_tmp.w, diff_tmp.z);

  ushort2 lcost_sh_curr_L = lcost_sh[hook(5, shIdx + 0)];
  ushort2 lcost_sh_curr_H = lcost_sh[hook(5, shIdx + 1)];
  ushort2 lcost_sh_prev, lcost_sh_next;

  if (shIdx + 2 < 128 * 8 / 2)
    lcost_sh_next = lcost_sh[hook(5, shIdx + 2)];
  else
    lcost_sh_next = lcost_sh_curr_H;

  if (shIdx - 1 > 0)
    lcost_sh_prev = lcost_sh[hook(5, shIdx - 1)];
  else
  ...
  d_scost[hook(6, 128 * idx + k * 4 + 0)] += cost_tmp_L.y;
  d_scost[hook(6, 128 * idx + k * 4 + 1)] += cost_tmp_L.x;
  d_scost[hook(6, 128 * idx + k * 4 + 2)] += cost_tmp_H.y;
  d_scost[hook(6, 128 * idx + k * 4 + 3)] += cost_tmp_H.x;
  lcost_sh[hook(5, shIdx + 0)] = cost_tmp_L;
  lcost_sh[hook(5, shIdx + 1)] = cost_tmp_H;

  ushort2 cost_tmp = min(cost_tmp_L, cost_tmp_H);

  minCostNext[hook(2, get_local_id(1) * 32 + get_local_id(0))] = min(cost_tmp.x, cost_tmp.y);
