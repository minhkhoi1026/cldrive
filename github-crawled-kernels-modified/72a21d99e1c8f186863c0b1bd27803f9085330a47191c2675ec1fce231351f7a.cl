//{"<recovery-expr>(v_data)":4,"v_data":1,"v_s0":3,"v_s2_refract":2,"v_sizes":0}
...
kernel void s2_refract_run(global int* v_sizes, global unit_data* v_data, global int* v_s2_refract, global int* v_s0) {
  int global_id = get_global_id(0);
  if (global_id >= v_sizes[hook(0, 2)])
    return;

  int this_id = v_s2_refract[hook(2, global_id)];
  ...
  if (sin_beta <= 1.f) {
    ...
    final_dir = -reverse * cos(beta) * s2.normal + sin_beta * q;
  } else {
  ...
  v_data[hook(4, this_id)].start_p = s2.intersect_p + 1e-5f * final_dir;
  v_data[hook(4, this_id)].in_dir = final_dir;
}