...
kernel void sheet_force(global float4* pos, global float4* force) {
  ...
  if (get_global_id(1) > 0) {
    if (get_global_id(0) > 0) {
      pj = pos[hook(0, i - get_global_size(0) - 1)].xyz;
      f += stickForce(pj, p, -5.0f, 1.41421356237f);
    }
    pj = pos[hook(0, i - get_global_size(0))].xyz;
