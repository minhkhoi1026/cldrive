...
kernel void _XACC_unpack_vector(global char* dst, unsigned long dst_off, global const char* src, unsigned long src_off, unsigned long blocklength, unsigned long stride, unsigned long count) {
  ...
  for (c = c_init; c < count; c += c_step) {
    for (b = b_init; b < blocklength; b += b_step) {
      dst[hook(0, dst_off + stride * c + b)] = src[hook(2, src_off + blocklength * c + b)];
    }
  }
}