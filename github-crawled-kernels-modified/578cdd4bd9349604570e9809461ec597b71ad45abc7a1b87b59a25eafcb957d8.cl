...
kernel void buffer_quantize(private const int global_size_dim0, private const float scale, private const int zero_point, global float* input, private const int input_offset, global uchar* output) {
  ...
  uchar4 output_value = convert_uchar4_sat_rte(vload4(out_idx, input) / scale + zero_point);
