...
void scrypt_core(uint4 X[8], global uint4* restrict lookup) {
  ...
  unsigned int CO = rotate(x, 3U);
  unsigned int CO_tmp = rotate(xSIZE, 3U);

  for (unsigned int y = 0; y < 1024 / 16; ++y, CO += CO_tmp) {
    unsigned int CO_reg = CO;
    for (unsigned int z = 0; z < zSIZE; ++z, ++CO_reg)
      lookup[hook(13, CO_reg)] = X[hook(14, z)];
...

  {
    ...
    CO = CO_tmp + rotate(y * xSIZE, 3U);
    for (unsigned int z = 0; z < zSIZE; ++z, ++CO)
      lookup[hook(13, CO)] = X[hook(14, z)];
  ...
  for (unsigned int i = 0; i < 1024; ++i) {
    uint4 V[8];
    unsigned int j = X[hook(14, 7)].x & K[hook(7, 85)];
    unsigned int y = (j / 16);
    unsigned int CO_reg = CO_tmp + rotate(xSIZE * y, 3U);

    for (unsigned int z = 0; z < zSIZE; ++z, ++CO_reg)
      V[hook(15, z)] = lookup[hook(13, CO_reg)];
...
__attribute__((reqd_work_group_size(128, 1, 1))) kernel void search(global const uint4* restrict input, volatile global unsigned int* restrict output, global uint4* restrict padcache, const uint4 midstate0, const uint4 midstate16, const unsigned int target) {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    pad0 = tstate0;
    pad1 = tstate1;
    X[hook(14, rotate(i, 1U))] = ostate0;
    X[hook(14, rotate(i, 1U) + 1)] = ostate1;

    SHA256(&pad0, &pad1, data, (uint4)(i + 1, K[hook(7, 84)], 0, 0), (uint4)(0, 0, 0, 0), (uint4)(0, 0, 0, K[hook(7, 87)]));
    SHA256(X + rotate(i, 1U), X + rotate(i, 1U) + 1, pad0, pad1, (uint4)(K[hook(7, 84)], 0U, 0U, 0U), (uint4)(0U, 0U, 0U, K[hook(7, 88)]));
  }
