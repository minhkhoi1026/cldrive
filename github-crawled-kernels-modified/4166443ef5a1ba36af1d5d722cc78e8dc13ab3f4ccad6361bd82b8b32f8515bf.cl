...
kernel void trans_unit_lu_forward(global const unsigned int* row_indices, global const unsigned int* column_indices, global const float* elements, global float* vector, unsigned int size) {
  ...
  for (unsigned int i = get_local_id(0); i < loop_end; i += get_local_size(0)) {
    ...
    row_index_lookahead[hook(5, get_local_id(0))] = (row_at_window_start + get_local_id(0) < size) ? row_indices[hook(0, row_at_window_start + get_local_id(0))] : size - 1;

    barrier(0x01);

    if (i < nnz) {
      unsigned int row_index_inc = 0;
      while (i >= row_index_lookahead[hook(5, row_index_inc + 1)])
        ++row_index_inc;
      row_index = row_at_window_start + row_index_inc;
      row_index_buffer[hook(6, get_local_id(0))] = row_index;
    } else {
      row_index = size + 1;
      row_index_buffer[hook(6, get_local_id(0))] = size - 1;
    }
