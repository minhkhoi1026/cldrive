...
kernel void warpAffineNN_C4_D0(global uchar4 const* restrict src, global uchar4* dst, int src_cols, int src_rows, int dst_cols, int dst_rows, int srcStep, int dstStep, int src_offset, int dst_offset, constant F* M, int threadCols) {
  int dx = get_global_id(0);
  int dy = get_global_id(1);

  if (dx < threadCols && dy < dst_rows) {
    int round_delta = ((1 << max(10, (int)5)) >> 1);
    ...
    int sx0 = (short)(X0 >> max(10, (int)5));
    int sy0 = (short)(Y0 >> max(10, (int)5));

    if (dx >= 0 && dx < dst_cols && dy >= 0 && dy < dst_rows)
      dst[hook(1, (dst_offset >> 2) + dy * (dstStep >> 2) + dx)] = (sx0 >= 0 && sx0 < src_cols && sy0 >= 0 && sy0 < src_rows) ? src[hook(0, (src_offset >> 2) + sy0 * (srcStep >> 2) + sx0)] : (uchar4)0;
  }
}