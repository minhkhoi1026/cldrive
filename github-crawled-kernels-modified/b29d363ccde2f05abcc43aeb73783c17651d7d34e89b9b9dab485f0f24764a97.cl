...
kernel void label_step2(global unsigned int* labelImg, global unsigned int* equivTable, unsigned int label_step, unsigned int table_step, global struct SBlobInfo* info, int Iter) {
  ...
  if (ValidPixel) {
    unsigned int MasterLabel = FindMasterLabel(equivTable, table_step, Label);

    if (Label != MasterLabel) {
      equivTable[hook(1, table_index)] = MasterLabel;
      labelImg[hook(0, label_index)] = MasterLabel;
    }
  }
}