...
kernel void fftstep_cpx2cpx(global float2* vec_in, global float2* vec_out, read_only image2d_t twiddle_factors, int Wshift, int Nhalf) {
  ...
  const float2 p1 = vec_in[hook(0, src1_idx)];
  const float2 p2 = cmult(vec_in[hook(0, src2_idx)], twiddle_factor);
