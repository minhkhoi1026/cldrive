...
kernel void pcisph_computeForcesAndInitPressure(global float2* neighborMap, global float* rho, global float* pressure, global float4* sortedPosition, global float4* sortedVelocity, global float4* acceleration, global unsigned int* particleIndexBack, float surfTensCoeff, float mass_mult_divgradWviscosityCoefficient, float hScaled, float mu, float gravity_x, float gravity_y, float gravity_z, global float4* position, global uint2* particleIndex, unsigned int PARTICLE_COUNT, float mass
  ...
  do {
    if ((jd = neighborMap[hook(0, idx + nc)].x) != -1) {
      ...
      if (r_ij < hScaled) {
        ...
        if ((((position[hook(14, id_source_particle)].w > 2.05f) && (position[hook(14, id_source_particle)].w < 2.25f)) && ((position[hook(14, jd_source_particle)].w > 2.25f) && (position[hook(14, jd_source_particle)].w < 2.35f))) || (((position[hook(14, id_source_particle)].w > 2.25f) && (position[hook(14, id_source_particle)].w < 2.35f)) && ((position[hook(14, jd_source_particle)].w > 2.05f) && (position[hook(14, jd_source_particle)].w < 2.25f)))) {
          accel_viscosityForce += 0 * 1.0e-7f * (sortedVelocity[hook(4, jd)] * not_bp - sortedVelocity[hook(4, id)]) * (hScaled - r_ij) / 1000;

          if (((position[hook(14, id_source_particle)].w > 2.25f) && (position[hook(14, id_source_particle)].w < 2.35f))) {
            position[hook(14, id_source_particle)].w = 2.32f;
          }
        } else if (((position[hook(14, id_source_particle)].w > 2.05f) && (position[hook(14, id_source_particle)].w < 2.25f)) && ((int)(position[hook(14, jd_source_particle)].w == 3))) {
          accel_viscosityForce += 0 * 1.0e-7f * (sortedVelocity[hook(4, jd)] * not_bp - sortedVelocity[hook(4, id)]) * (hScaled - r_ij) / 1000;
        } else {
          accel_viscosityForce += 2.5e-4f * (sortedVelocity[hook(4, jd)] * not_bp - sortedVelocity[hook(4, id)]) * (hScaled - r_ij) / 1000;
        }
  ...
  accel_viscosityForce *= mass_mult_divgradWviscosityCoefficient / 1000.0f;
