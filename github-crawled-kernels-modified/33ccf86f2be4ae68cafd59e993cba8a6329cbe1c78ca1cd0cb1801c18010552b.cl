//{"MaxX":16,"MaxY":18,"MinX":17,"MinY":19,"Potential":0,"atZ":32,"atx":30,"aty":31,"clAtomXPos":1,"clAtomYPos":2,"clAtomZNum":4,"clAtomZPos":3,"clBlockStartPositions":6,"clfParams":5,"dz":12,"height":8,"i0a":26,"i0b":27,"k0a":28,"k0b":29,"loadBlocksX":20,"loadBlocksY":21,"loadSlicesZ":22,"pixelscale":13,"sigma":23,"slice":9,"slices":10,"startx":24,"starty":25,"width":7,"xBlocks":14,"yBlocks":15,"z":11}
...
float bessi0(float x) {
  ...
  if (ax <= 3.75f) {
    ...
    for (i = 5; i >= 0; i--)
      sum = sum * t + i0a[hook(26, i)];

  } else {
    t = 3.75f / ax;
    sum = i0b[hook(27, 8)];
    for (i = 7; i >= 0; i--)
      sum = sum * t + i0b[hook(27, i)];
...
kernel void clBinnedAtomicPotentialConventional(global float2* Potential, global const float* restrict clAtomXPos, global const float* restrict clAtomYPos, global const float* restrict clAtomZPos, global const int* restrict clAtomZNum, constant float* clfParams, global const int* restrict clBlockStartPositions, int width, int height, int slice, int slices, float z, float dz, float pixelscale, int xBlocks, int yBlocks, float MaxX, float MinX, float MaxY, float MinY, int loadBlocksX, int loadBlocksY, int loadSlicesZ, float sigma, float startx, float starty) {
  ...
  for (int k = topz; k <= bottomz; k++) {
    for (int j = startj; j <= endj; j++) {
      ...
      if (lid < end - start) {
        atx[hook(30, lid)] = clAtomXPos[hook(1, gid)];
        aty[hook(31, lid)] = clAtomYPos[hook(2, gid)];
        atZ[hook(32, lid)] = clAtomZNum[hook(4, gid)];
      }

      barrier(0x01);

      for (int l = 0; l < end - start; l++) {
        int ZNum = atZ[hook(32, l)];

        float rad = native_sqrt((startx + xid * pixelscale - atx[hook(30, l)]) * (startx + xid * pixelscale - atx[hook(30, l)]) + (starty + yid * pixelscale - aty[hook(31, l)]) * (starty + yid * pixelscale - aty[hook(31, l)]));
