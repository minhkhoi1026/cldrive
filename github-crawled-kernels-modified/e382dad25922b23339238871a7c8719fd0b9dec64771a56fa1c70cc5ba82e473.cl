//{"gHeight":4,"gInputImage":6,"gWidth":3,"gidOfLastValidElement":5,"lsum":2,"sumInput":0,"sumResults":1}
...
float sampleZeroPaddedFloat(const global float* gInputImage, int offset, int x, int y, int width, int height) {
  if (x < 0 || x >= width || y < 0 || y >= height)
    return 0.0f;
  return gInputImage[hook(6, offset + (y * width + x))];
}
...
float getClampedFloat3d(const global float* gInputImage, int x, int y, int z, int width, int height, int depth) {
  ...
  if (z >= depth)
    z = depth - 1;
  return gInputImage[hook(6, z * width * height + y * width + x)];
}
...
kernel void getAvg(global float* sumInput, global float* sumResults, local float* lsum, int gWidth, int gHeight, int gidOfLastValidElement) {
