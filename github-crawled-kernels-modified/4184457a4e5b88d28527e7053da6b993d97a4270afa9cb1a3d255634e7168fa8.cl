//{"diffuse":2,"epsilon":3,"mu":1,"result":0}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void LotsOfMathBuiltins(global uchar4* result, const float4 mu, const float4 diffuse, const float epsilon) {
  ...
  float4 float3 = (float4)(fD, fT0, fT1, fT);
  uchar4 output = convert_uchar4_sat_rte(float3 * 255.0f);
  result[hook(0, index)] = output;
}