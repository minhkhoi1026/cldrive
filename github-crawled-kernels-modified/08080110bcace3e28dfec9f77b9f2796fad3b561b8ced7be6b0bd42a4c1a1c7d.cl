//{"dest":1,"dst_step":3,"height":5,"matrix":7,"matrix_width":6,"source":0,"src_step":2,"width":4}
...
float Convolution(global const uchar* source, int src_step, int dst_step, int width, int height, constant const float* matrix, private int matrix_width) {
  ...
  switch (mask_size) {
    case 1: {
      for (int y = -1; y <= 1; y++)
        for (int x = -1; x <= 1; x++)
          sum += matrix[hook(7, Index++)] * convert_float(source[hook(0, (pos + (int2){x, y}).y * src_step + (pos + (int2){x, y}).x)]);
    } break;
    case 2: {
      for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++)
...
kernel void smooth(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, int height, int matrix_width) {
  const int gx = get_global_id(0);
  const int gy = get_global_id(1);
  const int2 pos = {gx, gy};
  src_step /= sizeof(uchar);
  ...
  switch (mask_size) {
    case 1: {
      for (int y = -1; y <= 1; y++)
        for (int x = -1; x <= 1; x++)
          sum += factor * convert_float(source[hook(0, (pos + (int2){x, y}).y * src_step + (pos + (int2){x, y}).x)]);
    } break;
    case 2: {
      for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++)
