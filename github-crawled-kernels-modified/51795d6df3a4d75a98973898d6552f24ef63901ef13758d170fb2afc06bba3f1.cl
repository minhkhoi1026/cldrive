...
kernel void render(global char* input, global char* output, int width, int height) {
  ...
  output[hook(1, i_mul_4)] = invert(input[hook(0, i_mul_4)]);
  output[hook(1, i_mul_4 + 1)] = invert(input[hook(0, i_mul_4 + 1)]);
  output[hook(1, i_mul_4 + 2)] = invert(input[hook(0, i_mul_4 + 2)]);
