...
kernel void classify_hists_180_kernel(const int cdescr_width, const int cdescr_height, const int cblock_hist_size, const int img_win_width, const int img_block_width, const int win_block_stride_x, const int win_block_stride_y, global const float* block_hists, global const float* coefs, float free_coef, float threshold, global uchar* labels) {
  ...
  if (tid < 90)
    products[hook(14, tid)] = product = product + products[hook(14, tid + 90)];
  barrier(0x01);
  ;
  if (tid < 45)
    products[hook(14, tid)] = product = product + products[hook(14, tid + 45)];
  barrier(0x01);
  ;
  if (tid < 13)
  ...
  if (tid == 0) {
    product = product + products[hook(14, tid + 1)];
