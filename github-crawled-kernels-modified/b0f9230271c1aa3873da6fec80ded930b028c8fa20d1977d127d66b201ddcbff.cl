//{"T->vertices":7,"array":5,"gauss":24,"gauss[j + 1]":23,"ids_behind":15,"mip_level":1,"mipmap_start":2,"num":21,"nums":3,"passback":20,"points":10,"pr":18,"rarray":6,"raw":13,"ret":11,"ret[0]":16,"ret[1]":17,"rotated":14,"size":22,"sizes":4,"tex_id":0,"triangle->vertices":12,"tris":19,"x":8,"y":9}
...
{
  array[hook(5, coord.z * width * height + mul24(coord.y, width) + coord.x)] = convert_uchar4(to_write);
}

uint4 read_image_3d_hardware(int4 coord, global uchar4* array, int width, int height)

{
  return convert_uint4(array[hook(5, coord.z * width * height + mul24(coord.y, width) + coord.x)]);
}
...
float4 read_tex_array(float2 coords, unsigned int tid, global unsigned int* num, global unsigned int* size, image_3d_read array) {
  ...
  float width = size[hook(22, slice)];
...
kernel void generate_mip_mips_aggressive(unsigned int tex_id, unsigned int mip_level, unsigned int mipmap_start, global unsigned int* nums, global unsigned int* sizes, image_3d_write array, image_3d_read rarray) {
  int x = get_global_id(0);
  int y = get_global_id(1);

  unsigned int proper_id = tex_id * 4 + mipmap_start + mip_level;

  int slice = nums[hook(3, proper_id)] >> 16;
  float width = sizes[hook(4, slice)];
  ...
  for (int j = -1; j <= 1; j++) {
    for (int i = -1; i <= 1; i++) {
      ...
      accum += col * gauss[hook(24, j + 1)][hook(23, i + 1)];
      div += gauss[hook(24, j + 1)][hook(23, i + 1)];
    }
  }
  ...
  {
    unsigned int mtexid = proper_id + 1;

    int w2 = nums[hook(3, mtexid)] >> 16;
    float nwidth = sizes[hook(4, w2)];
