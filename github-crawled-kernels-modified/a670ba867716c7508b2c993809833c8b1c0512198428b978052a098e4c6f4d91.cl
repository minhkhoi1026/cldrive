...
inline float3 unpack_fp3(unsigned int u) {
  float3 u3;
  u3.x = (float)(u & 0xff);
  u >>= 8;
  u3.y = (float)(u & 0xff);
...
inline unsigned int pack_fp3(float3 u3) {
  unsigned int u;
  u = (((unsigned int)u3.x)) | (((unsigned int)u3.y) << 8) | (((unsigned int)u3.z) << 16);
  return u;
}
kernel void compiler_box_blur(global const unsigned int* src, global unsigned int* dst, int w, int h, int chunk) {
  ...
  int curr = x + y * (w >> 2);
  float3 curr0, curr1, curr2, curr3;
  do {
    ...
    const float3 l = unpack_fp3(src[hook(0, left)]);
    const float3 r = unpack_fp3(src[hook(0, right)]);
    curr0 = (l + from0 + from1);
    curr1 = (from0 + from1 + from2);
    curr2 = (from1 + from2 + from3);
    curr3 = (from2 + from3 + r);
  } while (0);
  ...
  float3 top0, top1, top2, top3;
  do {
    const uint4 from = vload4(top, src);
    const float3 from0 = unpack_fp3(from.x);
    const float3 from1 = unpack_fp3(from.y);
    const float3 from2 = unpack_fp3(from.z);
    const float3 from3 = unpack_fp3(from.w);
    const float3 l = unpack_fp3(src[hook(0, topLeft)]);
    const float3 r = unpack_fp3(src[hook(0, topRight)]);
    top0 = (l + from0 + from1);
    top1 = (from0 + from1 + from2);
    top2 = (from1 + from2 + from3);
    top3 = (from2 + from3 + r);
  } while (0);
  ...
  for (; y < yend; ++y, curr += (w >> 2), bottom += (w >> 2), bottomLeft += w, bottomRight += w) {
    ...
    float3 bottom0, bottom1, bottom2, bottom3;
    do {
      const uint4 from = vload4(center, src);
      const float3 from0 = unpack_fp3(from.x);
      const float3 from1 = unpack_fp3(from.y);
      const float3 from2 = unpack_fp3(from.z);
      const float3 from3 = unpack_fp3(from.w);
      const float3 l = unpack_fp3(src[hook(0, left)]);
      const float3 r = unpack_fp3(src[hook(0, right)]);
      bottom0 = (l + from0 + from1);
      bottom1 = (from0 + from1 + from2);
      bottom2 = (from1 + from2 + from3);
      bottom3 = (from2 + from3 + r);
    } while (0);
