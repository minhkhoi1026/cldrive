...
kernel void d_vector_norm2(global double* g_idata, global double* g_odata, unsigned int n, local volatile double* sdata) {
  ...
  while (i < n) {
    sdata[hook(3, tid)] += g_idata[hook(0, i)] * g_idata[hook(0, i)];

    if (0 || i + 256 < n)
      sdata[hook(3, tid)] += g_idata[hook(0, i + 256)] * g_idata[hook(0, i + 256)];
    i += gridSize;
  }
