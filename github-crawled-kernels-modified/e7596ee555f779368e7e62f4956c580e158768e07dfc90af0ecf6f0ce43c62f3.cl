    ...
    int x2 = get_global_id(1) * skip;
    {
      ...
      for (int obj = 0; obj < num_obj; obj++) {
        ...
        for (int val = 0; val < num_obj_values; val++) {
          if (fract((float)0.75 + 2.0 / q * sqrt((float)((((in_dist[hook(8, val)])) * ((in_dist[hook(8, val)]))) + (((x1 - input[hook(5, val + obj * num_obj_values)])) * ((x1 - input[hook(5, val + obj * num_obj_values)]))) + (((x2)) * ((x2))))), &dummy) < 0.5
        ...
        gymy_space[hook(0, x1 * height + x2)] += ((sum) * (sum));
      }
    }
  }
}