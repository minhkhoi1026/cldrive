...
kernel void compute_disp_1(global const float* u_, global const float* d_, global const float* l_, global const float* r_, global const float* data_cost_selected, global const float* disp_selected_pyr, global short* disp, int res_step, int cols, int rows, int nr_plane, int cmsg_step1, int cdisp_step1) {
  int x = get_global_id(0);
  int y = get_global_id(1);

  if (y > 0 && y < rows - 1 && x > 0 && x < cols - 1) {
    global const float* data = data_cost_selected + y * cmsg_step1 + x;
    global const float* disp_selected = disp_selected_pyr + y * cmsg_step1 + x;

    global const float* u = u_ + (y + 1) * cmsg_step1 + (x + 0);
    global const float* d = d_ + (y - 1) * cmsg_step1 + (x + 0);
    global const float* l = l_ + (y + 0) * cmsg_step1 + (x + 1);
    global const float* r = r_ + (y + 0) * cmsg_step1 + (x - 1);

    short best = 0;
    short best_val = 32767;
    for (int i = 0; i < nr_plane; ++i) {
      int idx = i * cdisp_step1;
      float val = data[hook(27, idx)] + u[hook(35, idx)] + d[hook(36, idx)] + l[hook(37, idx)] + r[hook(38, idx)];

      if (val < best_val) {
        best_val = val;
        best = convert_short_sat_rte(disp_selected[hook(39, idx)]);
      }
    }
