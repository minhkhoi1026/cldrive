  ...
  if (groupId_x != 0) {
    ...
    if (cond) {
      ...
      for (int k = 0, id = xid; k < lim && id < odim0; k++, id += 128) {
        oData[hook(0, id)] = bin_NOTZERO_OP(accum, oData[hook(0, id)]);
      }
    }
  }
}