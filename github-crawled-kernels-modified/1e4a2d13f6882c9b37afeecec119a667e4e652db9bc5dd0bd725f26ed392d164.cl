//{"BUFFER_SIDE_LENGTH":0,"WORK_GROUP_SIDE_LENGTH":1,"array":3,"booleans":4,"workGroupOut":2}
...
float4 getValueOrInfinity(int2 p, int size, global float4* array) {
  ...
  float4 arrayValue = array[hook(3, clamp(indexFromPosition(p, size), 0, size * size - 1))];
...
int getIntOrZeroRestrict(int2 p, size_t size, global int const* restrict array) {
  ...
  if (isInsideArea) {
    return array[hook(3, indexFromPosition(p, size))];
  } else {
    return 0;
  }
}
...
int reduceAnd(local int* booleans, size_t localId, size_t localSize) {
  for (int offset = localSize / 2; offset > 0; offset >>= 1) {
    if (localId < offset) {
      int isTrueA = booleans[hook(4, localId)];
      int isTrueB = booleans[hook(4, localId + offset)];

      booleans[hook(4, localId)] = isTrueA && isTrueB;
    }

    barrier(0x01);
  }

  return booleans[hook(4, 0)];
}
...
kernel void workGroups(const unsigned int BUFFER_SIDE_LENGTH, const unsigned int WORK_GROUP_SIDE_LENGTH, global float* workGroupOut) {
  ...
  if (((get_group_id(0) % 2)) != (((get_group_id(0) / (BUFFER_SIDE_LENGTH / WORK_GROUP_SIDE_LENGTH)) % 2))) {
    v = 0.0f;
  } else {
    v = 1.0f;
  }

  workGroupOut[hook(2, iGlobal)] = v;
}