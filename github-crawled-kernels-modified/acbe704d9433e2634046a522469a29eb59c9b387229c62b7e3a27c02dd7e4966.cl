...
kernel void amdFHT8192FromSXFade(global const float* in, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int frame_ln, int current_bin, int xfade_span) {
  int lcl_id = get_local_id(0);
