...
kernel void conv2D_vertical(read_only image2d_t src, write_only image2d_t dst, sampler_t sampler, constant float* filter, const int filter_size, const int filter_size_half, float4 bias) {
  ...
  for (int i = 0; i < filter_size; ++i) {
    float4 float3 = read_imagef(src, sampler, coords_src - (int2)(0, i));
