...
kernel void normal_kd_logpost(global unsigned int* labels, global float* data, global unsigned int* uniq_label, global float* mu, global unsigned int* n, global float* determinants, global float* inverses, unsigned int cluster_num, float alpha, unsigned int dim, float v0, global float* logpost, global float* rand) {
  ...
  float loglik = mvt_logpdf(data, i, dim, t_df, mu, c, determinants[hook(5, c)], inverses);

  loglik += (new_size > 0) ? log(new_size / (alpha + data_size)) : log(alpha / (alpha + data_size));
  logpost[hook(11, i * cluster_num + c)] = loglik;
}