...
kernel void repeat_f64(unsigned int expansionDim, unsigned int innerStride, unsigned int repeats, global double* restrict a, global double* restrict out) {
  const unsigned int i = get_global_id(0);
  const unsigned int j = get_global_id(1);
  const unsigned int k = get_global_id(2);
  const double value = a[hook(3, (i * expansionDim + j) * innerStride + k)];
