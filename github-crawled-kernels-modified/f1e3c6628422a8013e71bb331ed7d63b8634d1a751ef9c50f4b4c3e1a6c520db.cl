...
__attribute__((reqd_work_group_size(8, 8, 1))) kernel void bilateral_downsample2(global const float4* input, global float8* grid, int width, int height, int sw, int sh, int depth, int s_sigma, float r_sigma) {
  const int gid_x = get_global_id(0);
  const int gid_y = get_global_id(1);

  local float8 grid_chunk[12][8][8];

  if (gid_x > sw || gid_y > sh)
    return;

  for (int d = 0; d < depth; d += 12) {
    for (int k = 0; k < 12; k++) {
    ...
    for (int s = d, e = d + min(12, depth - d); s < e; s++) {
