...
kernel void prepareFFT_GABAA(global const struct state* states_old, global float* sVals_real, const unsigned int nX, const unsigned int nY, const unsigned int nZ) {
  ...
  sVals_real[hook(1, index_sVals)] = states_old[hook(0, index_states)].s_GABAA;
}