//{"N":1,"base":0,"randomBlock":3,"seed":2}
...
kernel void deviceWriteRandomBlocks(global unsigned int* base, unsigned int N, int seed, local unsigned int* randomBlock) {
  if (seed == 0)
    seed = 123459876 + get_group_id(0);
  unsigned int bitSeed = deviceRan0p(seed + get_local_id(0), get_local_id(0));

  for (unsigned int i = 0; i < N; i++) {
    randomBlock[hook(3, get_local_id(0))] = deviceRan0p(seed, get_local_id(0)) | (deviceIrbit2(&bitSeed) << 31);
    barrier(0x01);

    seed = randomBlock[hook(3, get_local_size(0) - 1)];

    barrier(0x01);

    *((base + get_group_id(0) * N * get_local_size(0) + i * get_local_size(0) + get_local_id(0))) = randomBlock[hook(3, get_local_id(0))];
  }
}