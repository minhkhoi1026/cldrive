...
kernel void amdFHT512FromSXFade(global const float* in1, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int frame_ln, int current_bin, global const float* in2, global const float* out2, int index2, int chnl_stride2) {
  int lcl_id = get_local_id(0);
  int chnl = channels_map[hook(4, get_group_id(1))];
  int chnl_off = chnl_stride * chnl;

  local float data1[512];
  local float data2[512];
  ...
  int out2_off = mul24(chnl_stride2, chnl) + mul24(index2, 512 / 2);
  for (int i = lcl_id; i < 512 / 2; i += 256) {
    float fadeInCoeff = (float)i / (float)(512 / 2 - 1);
