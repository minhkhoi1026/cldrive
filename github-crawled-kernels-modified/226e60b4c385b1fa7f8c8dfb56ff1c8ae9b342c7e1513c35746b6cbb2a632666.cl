...
kernel void amdFHT4096FromQ(global const float* in, global float* out, global const short* gbitreverse, global const float* gsincos, global const int* channels_map, float scale, int chnl_stride, int que_ln, int frame_ln, int current_bin) {
  int lcl_id = get_local_id(0);
  int chnl = channels_map[hook(4, get_group_id(1))];
  int chnl_off = chnl_stride * chnl;
  int bin_off, prev_bin;

  local float data[4096];

  bin_off = current_bin * frame_ln;
  for (int i = lcl_id; i < 4096 / 2; i += 256) {
  ...
  for (int i = lcl_id; i < 4096 / 2; i += 256) {
    data[hook(10, gbitreverse[4hook(2, 4096 / 2 + i))] = in[hook(0, i + chnl_off + bin_off)];
  }
  ...
  int n = 1;
  int n2 = 4096 / 2;
  for (int log2_n = 0, log2_n2 = 12 - 1; log2_n < 12; log2_n++, log2_n2--) {
    n = (1 << log2_n);
    n2 = (1 << log2_n2);

    for (int k = lcl_id; k < 4096; k += 256) {
      FHTIteration_gcs(data, gsincos, n, n2, k);
    }
    barrier(0x01);
  }

  for (int i = lcl_id; i < 4096; i += 256) {
