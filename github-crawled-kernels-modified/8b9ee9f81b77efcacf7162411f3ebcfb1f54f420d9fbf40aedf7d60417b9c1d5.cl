...
kernel void vglClConvolution(read_only image2d_t img_input, write_only image2d_t img_output, constant float* convolution_window, int window_size_x, int window_size_y) {
  ...
  for (int j = -factory; j <= factory; j++) {
    for (int i = -factorx; i <= factorx; i++) {
      ...
      p.xyz *= convolution_window[hook(2, conv_controller)];
      result += p;
      conv_controller++;
    }
  }
