...
kernel void kmeans_kernel_c(global float* restrict feature, global float* restrict clusters, global int* restrict membership, int npoints, int nclusters, int nfeatures) {
  ...
  float p_feature[8];
  p_feature[hook(7, 0)] = feature[hook(0, 0 * npoints + point_id)];
  p_feature[hook(7, 1)] = feature[hook(0, 1 * npoints + point_id)];
  p_feature[hook(7, 2)] = feature[hook(0, 2 * npoints + point_id)];
  p_feature[hook(7, 3)] = feature[hook(0, 3 * npoints + point_id)];
  p_feature[hook(7, 4)] = feature[hook(0, 4 * npoints + point_id)];
  p_feature[hook(7, 5)] = feature[hook(0, 5 * npoints + point_id)];
  p_feature[hook(7, 6)] = feature[hook(0, 6 * npoints + point_id)];
  p_feature[hook(7, 7)] = feature[hook(0, 7 * npoints + point_id)];
