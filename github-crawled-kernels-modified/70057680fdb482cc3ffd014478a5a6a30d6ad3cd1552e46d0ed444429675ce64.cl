//{"bounds":4,"changed":7,"chunk":11,"cones":3,"count":9,"dir":1,"height":12,"index":6,"o":2,"size":10,"stack":8,"tHit":5,"threadsCount":13,"vertex":0}
...
void yetAnotherIntersectAllLeaves(const global float* dir, const global float* o, const global float* bounds, global int* index, global float* tHit, global int* changed, float3 v1, float3 v2, float3 v3, float3 e1, float3 e2, int chunk, int rindex) {
  ...
  for (int i = 0; i < chunk; i++) {
    ...
    tHit[hook(5, rindex + i)] = t;
    index[hook(6, rindex + i)] = get_global_id(0);
    changed[hook(7, get_global_id(0))] = 1;
  }
}
...
kernel void YetAnotherIntersection(const global float* vertex, const global float* dir, const global float* o, const global float* cones, const global float* bounds, global float* tHit, global int* index, global int* changed, local int* stack, int count, int size, int chunk, int height, unsigned int threadsCount) {
  ...
  for (int j = 0; j < levelcount; j++) {
    ...
    if (acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
      stack[hook(8, iLID * (height) + SPindex++)] = begin - 8 * lastlevelnum + 16 * j;
      while (SPindex > 0) {
        --SPindex;
        i = stack[hook(8, iLID * (height) + SPindex)];
        ...
        if (len < 0.000002f || acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
          ...
          if (child < 0) {
            ...
            yetAnotherIntersectAllLeaves(dir, o, bounds, index, tHit, changed, v1, v2, v3, e1, e2, cones[hook(3, i + 7)], rindex);
          } else {
            stack[hook(8, iLID * (height) + SPindex++)] = child;
          }
        }
        ...
        if (len < 0.000002f || acos(dot((center - a) / len, x)) - asin(radius / len) < fi) {
          ...
          if (child < 0) {
            ...
            yetAnotherIntersectAllLeaves(dir, o, bounds, index, tHit, changed, v1, v2, v3, e1, e2, cones[hook(3, i + 15)], rindex);
          } else {
            stack[hook(8, iLID * (height) + SPindex++)] = child;
          }
        }
