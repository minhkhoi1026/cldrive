//{"M":12,"MD":13,"Mx":6,"a":10,"b":11,"deltap":5,"fr_ents":3,"frs":1,"m":7,"max_contacts":0,"r":9,"t":8,"to_ents":4,"tos":2}
...
kernel void calculate_Mx(const int max_contacts, global const int* frs, global const int* tos, global const float8* fr_ents, global const float8* to_ents, global const float8* deltap, global float* Mx) {
  ...
  if (a == 0 && b == 0)
    return;
  float8 to_ents_i = b < 0 ? 0.f : to_ents[hook(4, i)];

  float res0123 = dot(fr_ents[hook(3, i)].s0123, deltap[hook(5, a)].s0123) - dot(to_ents_i.s0123, deltap[hook(5, b)].s0123);
  float res4567 = dot(fr_ents[hook(3, i)].s4567, deltap[hook(5, a)].s4567) - dot(to_ents_i.s4567, deltap[hook(5, b)].s4567);
  Mx[hook(6, i)] = res0123 + res4567;
}