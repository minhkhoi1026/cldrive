//{"data":8,"n_feat":7,"score_idx":6,"score_in":5,"score_out":2,"x_in":3,"x_out":0,"y_in":4,"y_out":1}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
constant int ref_pat[] = {
...
float block_reduce_sum(float val, local float* data) {
  ...
  for (unsigned i = get_local_size(1) / 2; i > 0; i >>= 1) {
    if (get_local_id(1) < i) {
      data[hook(8, idx)] += data[hook(8, idx + i)];
    }

    barrier(0x01);
  }

  return data[hook(8, get_local_id(0) * get_local_size(0))];
}

kernel void keep_features(global float* x_out, global float* y_out, global float* score_out, global const float* x_in, global const float* y_in, global const float* score_in, global const unsigned* score_idx, const unsigned n_feat) {
