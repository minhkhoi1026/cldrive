...
kernel void d_kernel_csr_spmv_vector(const int num_rows, global const int* row_offset, global const int* col, global const double* val, global const double* x, global double* y, local volatile double* sdata) {
  ...
  for (int row = warp_id; row < num_rows; row += num_warps) {
    ...
    for (int j = row_start + thread_lane; j < row_end; j += 32)
      sum += val[hook(3, j)] * x[hook(4, col[jhook(2, j))];
