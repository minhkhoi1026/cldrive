//{"<recovery-expr>(result)":5,"result":4,"sample_x":2,"sample_y":3,"view_p":0,"x_shift":1}
...
kernel void set_viewdirs_vr(const float3 view_p, const int x_shift, const int sample_x, const int sample_y, global unit_data* result) {
  ...
  float lon = convert_float(global_id_x + x_shift) / convert_float(sample_y * 2) * 3.1415926535f * 2 - 3.1415926535f;
  float lat = convert_float(global_id_y) / convert_float(sample_y) * 3.1415926535f - 3.1415926535f / 2.0;
  ...
  result[hook(5, index)].in_dir = normalize(dir);
  result[hook(5, index)].orig_id = index;
  result[hook(5, index)].strength = (float3)(1, 1, 1);
  result[hook(5, index)].start_p = view_p;
}