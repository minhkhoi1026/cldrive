...
kernel void integral_rows_D5(global float4* srcsum, global float4* srcsqsum, global float* sum, global float* sqsum, int rows, int cols, int src_step, int sum_step, int sqsum_step, int sum_offset, int sqsum_offset) {
  ...
  for (int i = 0; i < rows; i = i + 255) {
    src_t[hook(11, 0)] = i + lid < rows ? srcsum[hook(0, (lid + i) * src_step + gid * 2)] : (float4)0;
    sqsrc_t[hook(12, 0)] = i + lid < rows ? srcsqsum[hook(1, (lid + i) * src_step + gid * 2)] : (float4)0;
    src_t[hook(11, 1)] = i + lid < rows ? srcsum[hook(0, (lid + i) * src_step + gid * 2 + 1)] : (float4)0;
    sqsrc_t[hook(12, 1)] = i + lid < rows ? srcsqsum[hook(1, (lid + i) * src_step + gid * 2 + 1)] : (float4)0;

    sum_t[hook(13, 0)] = (i == 0 ? (float4)0 : lm_sum[hook(15, 0)][hook(14, 254 + 8)]);
    sqsum_t[hook(16, 0)] = (i == 0 ? (float4)0 : lm_sqsum[hook(18, 0)][hook(17, 254 + 8)]);
    sum_t[hook(13, 1)] = (i == 0 ? (float4)0 : lm_sum[hook(15, 1)][hook(19, 254 + 8)]);
    ...
    if (lid > 0 && (i + lid) <= rows) {
      ...
      sum_p = (local float*)(&(lm_sum[hook(15, 0)][hook(14, bf_loc)]));
      ...
      sum_p = (local float*)(&(lm_sum[hook(15, 1)][hook(19, bf_loc)]));
