...
kernel void f_dn_dt(global struct state* states_old, global struct state* states_new, const float dt) {
  ...
  states_new[hook(1, idx)].n = state_0.n + dt * (f1 + 2.0f * f2 + 2.0f * f3 + f4) / 6.0f;
}