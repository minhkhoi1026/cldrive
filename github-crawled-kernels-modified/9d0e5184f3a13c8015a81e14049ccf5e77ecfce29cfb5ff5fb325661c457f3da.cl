...
kernel void swapDimensionsFwd(global float* dataIn, global float* dataOut, unsigned int const dim1, unsigned int const dim2, unsigned int const dim3) {
  unsigned int idx = get_global_id(0);

  unsigned int dim1dim2 = mul24(dim1, dim2);
  unsigned int dim3dim1 = mul24(dim3, dim1);

  unsigned int inX = idx % dim1;
  unsigned int inY = (idx % dim1dim2) / dim1;
  unsigned int inZ = idx / dim1dim2;

  unsigned int idxOut = mad24(inY, dim3dim1, mad24(inX, dim3, inZ));
