//{"ablock":7,"ablock[0]":6,"ablock[1]":9,"ablock[2]":10,"ablock[3]":11,"ablock[4]":12,"avec":8,"bblock":16,"bblock[0]":15,"bblock[1]":18,"bblock[2]":20,"bblock[3]":22,"bblock[4]":24,"bvec":5,"c":32,"c[0]":31,"c[1]":33,"c[2]":34,"c[3]":35,"c[4]":36,"cblock":14,"cblock[0]":13,"cblock[1]":17,"cblock[2]":19,"cblock[3]":21,"cblock[4]":23,"g_lhs":1,"g_matrix":41,"g_matrix[0]":40,"g_matrix[1]":43,"g_matrix[2]":45,"g_matrix[3]":47,"g_matrix[4]":49,"g_rhs":0,"g_vector":57,"gp0":2,"gp1":3,"gp2":4,"lhs":26,"lhs[0]":25,"lhs[1]":27,"lhs[2]":28,"lhs[3]":29,"lhs[4]":30,"lhs[i]":61,"lhs[i][2]":66,"lhs[i][2][n]":65,"lhs[isize]":62,"p_matrix":39,"p_matrix[0]":38,"p_matrix[1]":42,"p_matrix[2]":44,"p_matrix[3]":46,"p_matrix[4]":48,"p_rhs":67,"p_rhsm1":63,"p_source":51,"p_source[0]":50,"p_source[1]":52,"p_source[2]":53,"p_source[3]":54,"p_source[4]":55,"p_vector":56,"r":37,"rhs":60,"rhs[k]":59,"rhs[k][j]":58,"rhs[k][j][i]":64}
...
kernel void x_solve(global double* g_rhs, global double* g_lhs, int gp0, int gp1, int gp2) {
  int i, j, k, m, n, isize;
  ...
  load_vector(p_rhs, rhs[hook(60, k)][hook(59, j)][hook(58, 0)]);
  p_binvcrhs(p_lhsBB, p_lhsCC, p_rhs);
  save_matrix(lhs[hook(26, 0)][hook(25, 1)], p_lhsBB);
  save_matrix(lhs[hook(26, 0)][hook(25, 2)], p_lhsCC);
  save_vector(rhs[hook(60, k)][hook(59, j)][hook(58, 0)], p_rhs);

  for (i = 1; i <= isize - 1; i++) {
    copy_matrix(p_lhsm1CC, p_lhsCC);
    copy_vector(p_rhsm1, p_rhs);
    load_matrix(p_lhsAA, lhs[hook(26, i)][hook(61, 0)]);
    load_matrix(p_lhsBB, lhs[hook(26, i)][hook(61, 1)]);
    load_matrix(p_lhsCC, lhs[hook(26, i)][hook(61, 2)]);
    load_vector(p_rhs, rhs[hook(60, k)][hook(59, j)][hook(58, i)]);
    ...
    save_matrix(lhs[hook(26, i)][hook(61, 1)], p_lhsBB);
    save_matrix(lhs[hook(26, i)][hook(61, 2)], p_lhsCC);
    save_vector(rhs[hook(60, k)][hook(59, j)][hook(58, i)], p_rhs);
  }

  copy_matrix(p_lhsm1CC, p_lhsCC);
  copy_vector(p_rhsm1, p_rhs);
  load_matrix(p_lhsAA, lhs[hook(26, isize)][hook(62, 0)]);
  load_matrix(p_lhsBB, lhs[hook(26, isize)][hook(62, 1)]);
  load_matrix(p_lhsCC, lhs[hook(26, isize)][hook(62, 2)]);
  load_vector(p_rhs, rhs[hook(60, k)][hook(59, j)][hook(58, isize)]);
  ...
  save_matrix(lhs[hook(26, isize)][hook(62, 1)], p_lhsBB);
  save_matrix(lhs[hook(26, isize)][hook(62, 2)], p_lhsCC);
  save_vector(rhs[hook(60, k)][hook(59, j)][hook(58, isize)], p_rhs);

  for (i = isize - 1; i >= 0; i--) {
    for (m = 0; m < 5; m++) {
      p_rhsm1[hook(63, m)] = rhs[hook(60, k)][hook(59, j)][hook(58, i)][hook(64, m)];
      for (n = 0; n < 5; n++) {
        p_rhsm1[hook(63, m)] = p_rhsm1[hook(63, m)] - lhs[hook(26, i)][hook(61, 2)][hook(66, n)][hook(65, m)] * p_rhs[hook(67, n)];
      }
      rhs[hook(60, k)][hook(59, j)][hook(58, i)][hook(64, m)] = p_rhsm1[hook(63, m)];
    }
