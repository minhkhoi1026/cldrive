...
kernel void srad_cuda_1(global float* E_C, global float* W_C, global float* N_C, global float* S_C, global float* J_cuda, global float* C_cuda, int cols, int rows, float q0sqr) {
  ...
  local float temp[16][16];
  local float temp_result[16][16];

  local float north[16][16];
  local float south[16][16];
  local float east[16][16];
  local float west[16][16];

  north[hook(10, ty)][hook(9, tx)] = J_cuda[hook(4, index_n)];
  south[hook(12, ty)][hook(11, tx)] = J_cuda[hook(4, index_s)];
  if (by == 0) {
    north[hook(10, ty)][hook(9, tx)] = J_cuda[hook(4, 16 * bx + tx)];
  } else if (by == get_num_groups(1) - 1) {
    south[hook(12, ty)][hook(11, tx)] = J_cuda[hook(4, cols * 16 * (get_num_groups(1) - 1) + 16 * bx + cols * (16 - 1) + tx)];
  }
  barrier(0x01);
  ...
  if (bx == 0) {
    west[hook(14, ty)][hook(13, tx)] = J_cuda[hook(4, cols * 16 * by + cols * ty)];
  } else if (bx == get_num_groups(0) - 1) {
    east[hook(16, ty)][hook(15, tx)] = J_cuda[hook(4, cols * 16 * by + 16 * (get_num_groups(0) - 1) + cols * ty + 16 - 1)];
  }
