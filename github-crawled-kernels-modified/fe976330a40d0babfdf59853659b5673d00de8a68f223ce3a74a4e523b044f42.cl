...
kernel void FFT2(global float2* a, local float2* l, global const unsigned int* points_per_group, global float2* debug, global const int* dir) {
  ...
  for (int i = 0; i < points_per_item; i += 4) {
    ...
    a[hook(0, a_addr)] = sumus;
    a[hook(0, a_addr + 1)] = diffus;
    a[hook(0, a_addr + 2)] = sumvt;
    a[hook(0, a_addr + 3)] = diffvt;
    l_addr += 4;
    a_addr += 4;
  }
  ...
  for (int i = 0; i < points_per_item; i++) {
    debug[hook(3, a_addr + i)] = l[hook(1, l_addr + i)];
  }
  ...
  for (int s = 2; s < lgppi; ++s) {
    m <<= 1;
    l_addr = get_local_id(0) * points_per_item;
    omega = exp_complex((float2)(0.0, (*dir) * -2.0 * 3.14159265358979323846264338327950288f / m));
    for (int k = 0; k < points_per_item; k += m) {
      cur_omega = (float2)(1.0, 0.0);
      for (int j = 0; j < (m >> 1); ++j) {
        t = mul_complex(omega, l[hook(1, l_addr + (m >> 1) + k + j)]);
        u = l[hook(1, l_addr + k + j)];
        l[hook(1, l_addr + k + j)] = u + t;
        l[hook(1, l_addr + (m >> 1) + k + j)] = u - t;
  ...
  a_addr = get_group_id(0) * *points_per_group + l_addr;
  for (int i = 0; i < points_per_item; i++) {
    a[hook(0, a_addr + i)] = l[hook(1, a_addr + i)];
  }
}