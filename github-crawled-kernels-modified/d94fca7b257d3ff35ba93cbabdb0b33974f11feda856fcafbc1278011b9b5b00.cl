    ...
    int x1 = get_global_id(0) * skip;
    {
      int x2 = get_global_id(1) * skip;
      {
        ...
        for (int val = 0; val < probe_num_values; val++) {
          if (fract((float)0.75 + 2.0 / q * (sqrt((float)((((probe_dist[hook(7, val)])) * ((probe_dist[hook(7, val)]))) + (((x1 - probe[hook(5, val)])) * ((x1 - probe[hook(5, val)]))) + (((x2)) * ((x2))))) + sqrt((float)((((out_dist)) * ((out_dist))) + (((x1 - y)) * ((x1 - y))) + (((x2)) * ((x2)))))), &dummy) < 0.5)
