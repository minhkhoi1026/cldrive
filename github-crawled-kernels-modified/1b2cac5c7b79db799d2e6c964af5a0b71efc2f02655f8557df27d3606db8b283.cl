...
kernel void lut_linear(global const uchar* source, global uchar* dest, int src_step, int dst_step, int width, constant const float* levels, constant const float* values, int nb) {
  ...
  if (4 > 1 && gx + 4 > width) {
    for (int x = gx; x < width; x++)
      dest[hook(1, gy * dst_step + x)] = do_lut_linear(source[hook(0, gy * src_step + x)], levels, values, nb);
