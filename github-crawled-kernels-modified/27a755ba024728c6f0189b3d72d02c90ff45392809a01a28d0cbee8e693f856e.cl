...
kernel void compute_data_cost_reduce_0(global const short* selected_disp_pyr, global short* data_cost_, global uchar* cleft, global uchar* cright, local float* smem, int level, int rows, int cols, int h, int nr_plane, int channels, int winsz, int cmsg_step1, int cmsg_step2, int cdisp_step1, int cdisp_step2, float cdata_weight, float cmax_data_term, int cimg_step, int cth)

{
  ...
  global const short* selected_disparity = selected_disp_pyr + y_out / 2 * cmsg_step2 + x_out / 2;
  global short* data_cost = data_cost_ + y_out * cmsg_step1 + x_out;
  ...
  if (d < nr_plane) {
    local volatile float* vdline = smem + winsz * get_local_id(2);
    if (tid == 0)
      data_cost[hook(25, cdisp_step1 * d)] = convert_short_sat_rte(vdline[hook(24, 0)]);
  }
}