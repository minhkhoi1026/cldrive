...
__attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(8, 8, 1))) __attribute__((reqd_work_group_size(8, 8, 1))) kernel void scan1_F64(global const uchar* source, global double* dest, int src_step, int dst_step, int width, int height) {
  ...
  local double TmpSumX[(8 * 8) * 8];
  local double SumX[(8 * 8) * 8];

  double Sum = 0;
  for (int i = 0; i < 8; i++) {
    Sum += convert_double(source[hook(0, gy * src_step + gx + i)]);
  ...
  for (int i = 0; i < 8; i++) {
    double Val = Sum + TmpSumX[hook(6, buf_index + i)];
