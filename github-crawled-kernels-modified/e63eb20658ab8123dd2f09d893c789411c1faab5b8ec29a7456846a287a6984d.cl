...
kernel void normalize3DInput(read_only image3d_t input, global float* output, private float scaleFactor, private float mean, private float std, private int signedInputNormalization, private int horizontalFlip, private int channels, private float minIntensity, private float maxIntensity, private int clipIntensity, private int channelFirst) {
  ...
  if (channelFirst == 0) {
    ...
    output[hook(1, position)] = value.x;
    if (channels > 1)
      output[hook(1, position + 1 * width * height * depth)] = value.y;
    if (channels > 2)
      output[hook(1, position + 2 * width * height * depth)] = value.z;
    if (channels > 3)
      output[hook(1, position + 3 * width * height * depth)] = value.w;
  }
}