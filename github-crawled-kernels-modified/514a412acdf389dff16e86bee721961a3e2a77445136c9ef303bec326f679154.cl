//{"N":0,"alpha":3,"beta":4,"current":1,"gamma":5,"sm":2}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void EM_gamma(const int N, const int current, local float* sm, global const float* alpha, global const float* beta, global float* gamma) {
  ...
  int i;
  for (i = gid; i < N; i += gls) {
    tidsum += alpha[hook(3, current + i)] * beta[hook(4, current + i)];
  }
  ...
  for (i = gid; i < N; i += gls) {
    gamma[hook(5, current + i)] /= sm[hook(2, 0)];
  }
}