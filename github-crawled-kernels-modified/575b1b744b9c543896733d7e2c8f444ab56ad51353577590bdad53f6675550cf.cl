...
kernel void gpu_bdia_nlvec(global int* bandptr, global int* offset, global float* data, int aligned_length, int band_num, global float* vec, global float* result, int vecoffset) {
  ...
  for (int i = 0; i < band_num; i++) {
    ...
    for (int j = 0; j < bandwidth; j++) {
      ...
      float vecelem = vec[hook(5, vecid + j)];
