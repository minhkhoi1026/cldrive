...
kernel void motion_blur(const global float4* src_buf, int src_width, int src_height, int src_x, int src_y, global float4* dst_buf, int dst_x, int dst_y, int num_steps, float offset_x, float offset_y) {
  ...
  dst_buf[hook(5, gidy * get_global_size(0) + gidx)] = sum / num_steps;
}