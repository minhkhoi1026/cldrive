...
kernel void init_data_cost_reduce_0(global short* ctemp, global uchar* cleft, global uchar* cright, local float* smem, int level, int rows, int cols, int h, int winsz, int channels, int cndisp, int cimg_step, float cdata_weight, float cmax_data_term, int cth, int cdisp_step1, int cmsg_step1) {
  ...
  if (d < cndisp) {
    local float* dline = smem + winsz * get_local_id(2);
    global short* data_cost = ctemp + y_out * cmsg_step1 + x_out;
    if (tid == 0)
      data_cost[hook(21, cdisp_step1 * d)] = convert_short_sat_rte(dline[hook(19, 0)]);
  }
}