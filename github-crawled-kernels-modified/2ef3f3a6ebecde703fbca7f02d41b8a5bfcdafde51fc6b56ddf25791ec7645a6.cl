...
kernel void downsample_row(read_only image2d_t input_image, write_only image2d_t output_image) {
  int2 out_coord = (int2)(get_global_id(0), get_global_id(1));
  int2 in_coord = (int2)(out_coord.x * 2, out_coord.y);

  float4 sample = 0.0f;
  for (int i = -2; i < 3; ++i) {
    sample += read_imagef(input_image, g_sampler, in_coord + (int2)(i, 0)) * sampling_kernel[hook(2, 2 + i)];
  }
