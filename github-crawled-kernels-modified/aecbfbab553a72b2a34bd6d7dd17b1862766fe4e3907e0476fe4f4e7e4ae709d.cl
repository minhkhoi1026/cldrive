//{"delta":1,"in":0,"max_delta":4,"out":2,"radius":3}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void cl_gblur_selective(global const float4* in, global const float4* delta, global float4* out, const float radius, const float max_delta) {
  ...
  const float4 center_pix = in[hook(0, center_gid1d)];
  const float3 center_delta = delta[hook(1, center_gid1d)].xyz;
  ...
  for (int v = -iradius; v <= iradius; v++) {
    for (int u = -iradius; u <= iradius; u++) {
      ...
      const float4 src_pix = in[hook(0, gid1d)];
      const float3 delta_pix = delta[hook(1, gid1d)].xyz;
  ...
  const float3 out_v = select(center_pix.xyz, accumulated / count, count != 0.0f);
  out[hook(2, gidx + gidy * dst_width)] = (float4)(out_v, center_pix.w);
}