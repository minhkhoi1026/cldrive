...
KERNEL_EXE_CONFIG = "mem_analysis_data/test_gen_config.json"
...
if __name__ == "__main__":
    ...
    gsize_list = []
    lsize_list = []
    kernel_exe_configs = json.load(open(KERNEL_EXE_CONFIG, "r", encoding="utf-8"))
    for kernel_path, gsize, lsize in kernel_exe_configs[:10] : 
        kernel_list.append(kernel_path)
        gsize_list.append(gsize)
    ...
    with multiprocessing.Pool(processes=NUM_PROCESS, initializer=set_cuda_visible) as pool:
        ...
        # parse list of gsize and lsize
        pool.map(
            get_kernel_args_values, 
            zip(kernel_list, gsize_list, lsize_list),
