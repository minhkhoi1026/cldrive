...
float elliptical_crosssection(float q, float a, float b) {
  ...
  for (i = 0; i < nord; i++) {
    zi = (Gauss76Z[hook(15, i)] * (uplim - lolim) + uplim + lolim) / 2.0f;
    arg = q * sqrt(a * a * sin(zi) * sin(zi) + b * b * cos(zi) * cos(zi));
    yyy = pow((2.0f * J1(arg) / arg), 2);
    yyy *= Gauss76Wt[hook(16, i)];
    summ += yyy;
  }
...
kernel void flexible_cylinder_ex_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,
...

  {
    ...
    for (int length_i = 0; length_i < Nlength; length_i++) {
      const float length = loops[hook(5, 2 * (length_i))];
      const float length_w = loops[hook(5, 2 * (length_i) + 1)];
      for (int kuhn_length_i = 0; kuhn_length_i < Nkuhn_length; kuhn_length_i++) {
        const float kuhn_length = loops[hook(5, 2 * (kuhn_length_i + Nlength))];
        const float kuhn_length_w = loops[hook(5, 2 * (kuhn_length_i + Nlength) + 1)];
        for (int radius_i = 0; radius_i < Nradius; radius_i++) {
          const float radius = loops[hook(5, 2 * (radius_i + Nlength + Nkuhn_length))];
          const float radius_w = loops[hook(5, 2 * (radius_i + Nlength + Nkuhn_length) + 1)];

          const float weight = length_w * kuhn_length_w * radius_w;
          if (weight > cutoff) {
            const float scattering = Iqxy(qxi, qyi, length, kuhn_length, radius, axis_ratio, sld, solvent_sld);
            if (!isnan(scattering)) {
