...
kernel void Ssyr2_naive_lower(unsigned int n, float alpha, const constant float* x, int incx, const constant float* y, int incy, global float* A, unsigned int lda) {
  for (unsigned int row_id = 0; row_id < n; ++row_id) {
    for (unsigned int col_id = 0; col_id <= row_id; ++col_id) {
