...
kernel void decode_bbox_corner_size_no_variance_kernel(const global float* loc_data, const global float* prior_data, const int num_priors, const int share_location, const int num_loc_classes, const int background_label_id, global float* bbox_data) {
  ...
  bbox_data[hook(6, idx)] = p_xmin + loc_data[hook(0, idx)] * variance[hook(7, idx_p)] * prior_width;
  bbox_data[hook(6, idx + 1)] = p_ymin + loc_data[hook(0, idx + 1)] * variance[hook(7, idx_p + 1)] * prior_height;
  bbox_data[hook(6, idx + 2)] = p_xmax + loc_data[hook(0, idx + 2)] * variance[hook(7, idx_p + 2)] * prior_width;
  bbox_data[hook(6, idx + 3)] = p_ymax + loc_data[hook(0, idx + 3)] * variance[hook(7, idx_p + 3)] * prior_height;
}