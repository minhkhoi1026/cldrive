//{"QL":0,"V":1,"size1":2,"size2":3,"strideQ":4,"sums":5}
...
kernel void house_update_QL(global float* QL, constant float* V, unsigned int size1, unsigned int size2, unsigned int strideQ, local float* sums) {
  ...
  for (unsigned int i = grp_id; i < size1; i += grp_nm) {
    ss = 0;
    for (unsigned int j = lcl_id; j < size1; j += lcl_sz)
      ss = ss + (V[hook(1, j)] * QL[hook(0, i * strideQ + j)]);
    ...
    for (unsigned int j = lcl_id; j < size1; j += lcl_sz)
      QL[hook(0, i * strideQ + j)] = QL[hook(0, i * strideQ + j)] - (2 * V[hook(1, j)] * sum_Qv);
  }
}