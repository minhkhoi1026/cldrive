//{"coef0":10,"data":0,"dataLen":1,"degree":11,"gamma":9,"i":4,"kernel_type":5,"len":3,"px":13,"py":14,"start":2,"x":7,"xW":8,"x_square":12,"y":6}
...
double my_dot(global const double* px, global const double* py, const int xW) {
  ...
  while (i < xW - 1) {
    ...
    if (remain >= 8) {
      ...
      double d = px[hook(13, i)] * py[hook(14, i)];
      sum += d;
      i++;
    }
  }
...
double kernel_linear(const int i, const int j, global const double* x, const int xW) {
  return my_dot(&x[hook(7, i * xW)], &x[hook(7, j * xW)], xW);
}
...
float svcQgetQ_f(const int len, const int i, const int kernel_type, global const char* y, global const double* x, const int xW, double gamma, double coef0, int degree, global double* x_square, const int index) {
  float res = y[hook(6, i)] * y[hook(6, index)];
  switch (abs(kernel_type)) {
    case LINEAR:
...
kernel void svcQgetQ(global float* data, const int dataLen, const int start, const int len, const int i, const int kernel_type, global const char* y, global const double* x, const int xW, double gamma, double coef0, int degree, global double* x_square) {
  const int index = get_global_id(0);
  const int realIndex = index + start;
  if (realIndex < dataLen) {
    data[hook(0, realIndex)] = svcQgetQ_f(len, i, kernel_type, y, x, xW, gamma, coef0, degree, x_square, realIndex);
  }
}