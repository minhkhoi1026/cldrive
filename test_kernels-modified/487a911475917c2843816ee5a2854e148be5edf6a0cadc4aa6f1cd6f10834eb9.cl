//{"d_prefixoffsets":2,"indice":1,"input":0,"l_pivotpoints":4,"s_offset":5,"size":3}
...
int addOffset(volatile local unsigned int* s_offset, unsigned int data, unsigned int threadTag) {
  ...
  do {
    count = s_offset[hook(5, data)] & 0x07FFFFFFU;
    count = threadTag | (count + 1);
    s_offset[hook(5, data)] = count;
  } while (s_offset[hook(5, data)] != count);

  return (count & 0x07FFFFFFU) - 1;
}

kernel void bucketcount(global float* input, global int* indice, global unsigned int* d_prefixoffsets, const int size, global float* l_pivotpoints) {
  ...
  for (int i = get_local_id(0); i < ((1 << 10) * (1)); i += get_local_size(0))
    d_prefixoffsets[hook(2, prefixBase + i)] = s_offset[hook(5, i)] & 0x07FFFFFFU;
}