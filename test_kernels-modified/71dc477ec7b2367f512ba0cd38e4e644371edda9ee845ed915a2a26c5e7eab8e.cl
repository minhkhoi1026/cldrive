...
kernel void f_dsNMDA_dt(global struct state* states_old, global struct state* states_new, const float dt) {
  ...
  states_new[hook(1, idx)].s_NMDA = state_0.s_NMDA + dt * (f1 + 2.0f * f2 + 2.0f * f3 + f4) / 6.0f;
}