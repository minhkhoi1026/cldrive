//{"batch_size":5,"mask":1,"num_hidden":3,"out":0,"p":4,"tmp":2}
...
kernel void feed_forward_dropout(global float* out, const global float* mask, local float* tmp, const int num_hidden, const float p, const int batch_size) {
  int GID = get_global_id(0);

  for (int n = 0; n < batch_size; n++)
    out[hook(0, n * get_global_size(0) + GID)] *= mask[hook(1, GID)];
}