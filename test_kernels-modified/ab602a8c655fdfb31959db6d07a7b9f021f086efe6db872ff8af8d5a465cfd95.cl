//{"T0":8,"T1":9,"T3":11,"float2":10,"global_T0":3,"global_T1":4,"global_T2":5,"global_T3":6,"global_sbox":7,"input":0,"key":2,"output":1,"sbox":12}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void encrypt2(const global uint4* input, global uint4* output, const global uint4 key[11], const global unsigned int global_T0[256], const global unsigned int global_T1[256], const global unsigned int global_T2[256], const global unsigned int global_T3[256], const global unsigned int global_sbox[256], local unsigned int T0[256], local unsigned int T1[256], local unsigned int float2[256], local unsigned int T3[256], local unsigned int sbox[256]) {
  ...
  t0 = t0 >> shift_8;
  output[hook(1, global_id)] = (t1 | (uint4)(sbox[hook(12, t0.s0)], sbox[hook(12, t0.s1)], sbox[hook(12, t0.s2)], sbox[hook(12, t0.s3)])) ^ key[hook(2, 10)];
}