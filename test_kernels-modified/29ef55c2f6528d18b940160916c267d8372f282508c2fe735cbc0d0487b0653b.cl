//{"Nedge_separation":8,"Nnumber_of_pearls":10,"Nq":3,"Nradius":7,"Nstring_thickness":9,"background":12,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":11,"sld":13,"solvent_sld":15,"string_sld":14}
...
kernel void pearl_necklace_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int edge_separation_i = 0; edge_separation_i < Nedge_separation; edge_separation_i++) {
        const float edge_separation = loops[hook(5, 2 * (edge_separation_i + Nradius))];
        const float edge_separation_w = loops[hook(5, 2 * (edge_separation_i + Nradius) + 1)];
        for (int string_thickness_i = 0; string_thickness_i < Nstring_thickness; string_thickness_i++) {
          const float string_thickness = loops[hook(5, 2 * (string_thickness_i + Nradius + Nedge_separation))];
          const float string_thickness_w = loops[hook(5, 2 * (string_thickness_i + Nradius + Nedge_separation) + 1)];
          for (int number_of_pearls_i = 0; number_of_pearls_i < Nnumber_of_pearls; number_of_pearls_i++) {
            const float number_of_pearls = loops[hook(5, 2 * (number_of_pearls_i + Nradius + Nedge_separation + Nstring_thickness))];
            const float number_of_pearls_w = loops[hook(5, 2 * (number_of_pearls_i + Nradius + Nedge_separation + Nstring_thickness) + 1)];

            const float weight = radius_w * edge_separation_w * string_thickness_w * number_of_pearls_w;
            if (weight > cutoff) {
              const float scattering = Iqxy(qxi, qyi, radius, edge_separation, string_thickness, number_of_pearls, sld, string_sld, solvent_sld);
              if (!isnan(scattering)) {
