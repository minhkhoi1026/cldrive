...
kernel void remuestreo_total(int n, global struct float4* p, global struct float4* aux, int start) {
  int base = get_global_id(0);
  int tam = get_global_size(0);
  struct float4 cero = p[hook(1, 0)];

  for (int i = base + start; i < n; i += tam)
    aux[hook(2, i)] = cero;
}