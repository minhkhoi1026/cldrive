...
kernel void product_bmdv_q1_f(global float* ll, global float* ld, global float* lu, global float* dl, global float* dd, global float* du, global float* ul, global float* ud, global float* uu, global float* x, global float* y, unsigned long n, unsigned long m, local float* smvf_cache) {
  ...
  if (idx < n) {
    float ytemp1 = dd[hook(4, idx)] * Dcache[hook(14, lindex + 1)];
    if (idx > 0)
