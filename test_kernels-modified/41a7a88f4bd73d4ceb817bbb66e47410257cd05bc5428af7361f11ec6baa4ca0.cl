...
void FI(private ushort* FI_IN, global ushort* FI_KEY) {
 ...
 private
  ...
  d9 = FI_IN[hook(3, 0)] >> 7;
  d7 = FI_IN[hook(3, 0)] & 0x7f;

  d9 = S9TABLE[hook(4, d9)] ^ d7;
  d7 = S7TABLE[hook(5, d7)] ^ d9;

  d7 = d7 & 0x7f;

  d7 = d7 ^ (FI_KEY[hook(6, 0)] >> 9);
  d9 = d9 ^ (FI_KEY[hook(6, 0)] & 0x1ff);

  d9 = S9TABLE[hook(4, d9)] ^ d7;

  FI_IN[hook(3, 0)] = (d7 << 9) | d9;
}

void FO(private unsigned int* FO_IN, global ushort* EK, int k, private unsigned int* F0_OUT) {
 private
  ushort t0[1];
 private
  ushort t1[1];

  t0[hook(7, 0)] = FO_IN[hook(8, 0)] >> 16;
  t1[hook(9, 0)] = FO_IN[hook(8, 0)] & 0xffff;
  t0[hook(7, 0)] = t0[hook(7, 0)] ^ EK[hook(1, k)];
  FI(t0, &EK[hook(1, (k + 5) % 8 + 8)]);
  t0[hook(7, 0)] = t0[hook(7, 0)] ^ t1[hook(9, 0)];
  t1[hook(9, 0)] = t1[hook(9, 0)] ^ EK[hook(1, (k + 2) % 8)];
  FI(t1, &EK[hook(1, (k + 1) % 8 + 8)]);
  t1[hook(9, 0)] = t1[hook(9, 0)] ^ t0[hook(7, 0)];
  t0[hook(7, 0)] = t0[hook(7, 0)] ^ EK[hook(1, (k + 7) % 8)];
  FI(t0, &EK[hook(1, (k + 3) % 8 + 8)]);
  t0[hook(7, 0)] = t0[hook(7, 0)] ^ t1[hook(9, 0)];
  t1[hook(9, 0)] = t1[hook(9, 0)] ^ EK[hook(1, (k + 4) % 8)];

  F0_OUT[hook(10, 0)] = (t1[hook(9, 0)] << 16) | t0[hook(7, 0)];
}

void FL(private unsigned int* FL_IN, global ushort* EK, int k) {
 private
  ushort d0;
  d0 = FL_IN[hook(11, 0)] >> 16;
 private
  ushort d1;
  d1 = FL_IN[hook(11, 0)] & 0xffff;

  if (((k) % 2 == 0)) {
    d1 = d1 ^ (d0 & EK[hook(1, k / 2)]);
    d0 = d0 ^ (d1 | EK[hook(1, (k / 2 + 6) % 8 + 8)]);
  } else {
    d1 = d1 ^ (d0 & EK[hook(1, ((k - 1) / 2 + 2) % 8 + 8)]);
    d0 = d0 ^ (d1 | EK[hook(1, ((k - 1) / 2 + 4) % 8)]);
  }

  FL_IN[hook(11, 0)] = (d0 << 16) | d1;
}

void misty1_encrypt(private ulong* P, global ushort* EK, private ulong* C) {
 private
  unsigned int D0[1];
  D0[hook(12, 0)] = P[hook(0, 0)] >> 32;
 private
  unsigned int D1[1];
  D1[hook(13, 0)] = P[hook(0, 0)] & 0xffffffff;

 private
  unsigned int F0_OUT[1];

  FL(D0, EK, 0);
  FL(D1, EK, 1);
  FO(D0, EK, 0, F0_OUT);
  D1[hook(13, 0)] = D1[hook(13, 0)] ^ F0_OUT[hook(10, 0)];

  FO(D1, EK, 1, F0_OUT);
  D0[hook(12, 0)] = D0[hook(12, 0)] ^ F0_OUT[hook(10, 0)];
  ...
  C[hook(2, 0)] = ((ulong)D1[hook(13, 0)] << 32) | D0[hook(12, 0)];
}

kernel void misty1CtrCipher(global ulong* P, global ushort* EK, global ulong* C) {
 private
  int gid;
  gid = get_global_id(0);
 ...
 private
  ...
  misty1_encrypt(counter, EK, outCipher);

  C[hook(2, gid)] = outCipher[hook(15, 0)] ^ P[hook(0, gid)];
}