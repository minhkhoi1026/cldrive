//{"Gauss76Wt":17,"Gauss76Z":16,"Ncore_radius":8,"Nlength":9,"Nphi":11,"Nq":3,"Nradius":7,"Ntheta":10,"background":13,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":12,"sld":14,"solvent_sld":15}
...
float Iq(float q, float radius, float core_radius, float length, float sld, float solvent_sld) {
  ...
  summ = 0.0f;
  for (i = 0; i < nord; i++) {
    zi = (Gauss76Z[hook(16, i)] * (upper - lower) + lower + upper) / 2.0f;
    inter = Gauss76Wt[hook(17, i)] * _hollow_cylinder_kernel(q, core_radius, radius, length, zi);
    summ += inter;
  }
...
kernel void hollow_cylinder_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                                 global float* loops_g,

                                 local float* loops, const float cutoff, const int Nradius, const int Ncore_radius, const int Nlength, const int Ntheta, const int Nphi,

                                 const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nradius + Ncore_radius + Nlength + Ntheta + Nphi) * 2, 0);
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int core_radius_i = 0; core_radius_i < Ncore_radius; core_radius_i++) {
        const float core_radius = loops[hook(5, 2 * (core_radius_i + Nradius))];
        const float core_radius_w = loops[hook(5, 2 * (core_radius_i + Nradius) + 1)];
        for (int length_i = 0; length_i < Nlength; length_i++) {
          const float length = loops[hook(5, 2 * (length_i + Nradius + Ncore_radius))];
          const float length_w = loops[hook(5, 2 * (length_i + Nradius + Ncore_radius) + 1)];
          for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
            const float theta = loops[hook(5, 2 * (theta_i + Nradius + Ncore_radius + Nlength))];
            const float theta_w = loops[hook(5, 2 * (theta_i + Nradius + Ncore_radius + Nlength) + 1)];
            for (int phi_i = 0; phi_i < Nphi; phi_i++) {
              const float phi = loops[hook(5, 2 * (phi_i + Nradius + Ncore_radius + Nlength + Ntheta))];
              const float phi_w = loops[hook(5, 2 * (phi_i + Nradius + Ncore_radius + Nlength + Ntheta) + 1)];

              const float weight = radius_w * core_radius_w * length_w * theta_w * phi_w;
              if (weight > cutoff) {
                const float scattering = Iqxy(qxi, qyi, radius, core_radius, length, sld, solvent_sld, theta, phi);
                if (!isnan(scattering)) {
