...
inline double d_segreduce_warp(const int thread_lane, int row, double val, local int* rows, local double* vals) {
  int tid = get_local_id(0);
...
inline void d_segreduce_block(local const int* idx, local double* val) {
  double left = 0;
  int tid = get_local_id(0);
...
kernel void d_kernel_coo_spmv_reduce_update(const int num_warps, global const int* temp_rows, global const double* temp_vals, global double* y) {
  local int rows[256 + 1];
  local double vals[256 + 1];
