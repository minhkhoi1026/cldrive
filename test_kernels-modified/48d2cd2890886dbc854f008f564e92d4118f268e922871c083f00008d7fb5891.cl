//{"input":1,"localData":3,"n":0,"output":2}
...
kernel void gpuStress(unsigned int n, const global float4* input, global float4* output) {
  ...
  for (unsigned int i = 0; i < 64; i++) {
    ...
    for (unsigned int j = 0; j < 64; j++) {
      ...
      localData[hook(3, lid)] = (tmpValue4.x + tmpValue4.y + tmpValue4.z + tmpValue4.w) * 0.25f;
      barrier(0x01);
      factor = localData[hook(3, (lid + 7) % 128)];
      barrier(0x01);
      ...
      localData[hook(3, lid)] = (tmpValue2.x + tmpValue2.y + tmpValue2.z + tmpValue2.w) * 0.25f;
      barrier(0x01);
      factor = localData[hook(3, (lid + 55) % 128)];
      barrier(0x01);
