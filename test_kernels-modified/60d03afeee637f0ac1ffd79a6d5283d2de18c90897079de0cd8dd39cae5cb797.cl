...
void zsum_reduce(int n, int i, local float* x);
void zsum_reduce(int n, int i, local float* x) {
  barrier(0x01);
  if (n > 128) {
    if (i < 128 && i + 128 < n) {
...
kernel void magma_strmv_kernel2(global float* T, int T_offset, int ldt, global float* t, int t_offset, global float* y, int y_offset, global float* tau, int tau_offset) {
  T += T_offset;
  t += t_offset;
  y += y_offset;
  tau += tau_offset;
