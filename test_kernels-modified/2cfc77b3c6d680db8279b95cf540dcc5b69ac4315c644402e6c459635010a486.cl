...
void mergeA(global struct float4* p, int start, int m, int stop, global struct float4* aux) {
  for (int i = start; i < m; i++)
    aux[hook(2, i)] = p[hook(3, i)];
  for (int j = m; j < stop; j++)
    aux[hook(2, j)] = p[hook(3, m + stop - j - 1)];
  int i = start, j = stop - 1;
  for (int k = start; k < stop; k++)
    if (aux[hook(2, j)].w > aux[hook(2, i)].w)
      p[hook(3, k)] = aux[hook(2, j--)];
    else
      p[hook(3, k)] = aux[hook(2, i++)];
}

kernel void calc_scan_parcial(int n, global int* pesos) {
  ...
  if ((base + 1) <= mod)
    ...
    stop = (base + 1) * elem + mod;

  for (int i = start + 1; i < stop; ++i) {
    pesos[hook(1, i)] += pesos[hook(1, i - 1)];
  }
}