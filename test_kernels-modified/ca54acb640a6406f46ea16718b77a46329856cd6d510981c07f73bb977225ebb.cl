//{"PARTICLE_COUNT":4,"closest_distances":8,"closest_indexes":9,"d_array":5,"gridCellIndex":6,"neighborMap":10,"particleIndex":2,"particleIndexBack":3,"position":0,"sortedPosition":7,"velocity":1}
...
int getMaxIndex(float* d_array) {
  ...
  float max_d = -1.f;
  for (int i = 0; i < 32; i++) {
    if (d_array[hook(5, i)] > max_d) {
      max_d = d_array[hook(5, i)];
      result = i;
    }
  }
  return result;
...
int searchForNeighbors_b(int searchCell_, global unsigned int* gridCellIndex, float4 position_, int myParticleId, global float4* sortedPosition, global float2* neighborMap, int* closest_indexes, float* closest_distances, int last_farthest, int* found_count) {
  int baseParticleId = gridCellIndex[hook(6, searchCell_)];
  int nextParticleId = gridCellIndex[hook(6, searchCell_ + 1)];
  ...
  while (i < particleCountThisCell) {
    ...
    if (myParticleId != neighborParticleId) {
      float4 d = position_ - sortedPosition[hook(7, neighborParticleId)];
      _distanceSquared = d.x * d.x + d.y * d.y + d.z * d.z;
      if (_distanceSquared <= closest_distances[hook(8, farthest_neighbor)]) {
        closest_distances[hook(8, farthest_neighbor)] = _distanceSquared;
        closest_indexes[hook(9, farthest_neighbor)] = neighborParticleId;
        if (*found_count < 32 - 1) {
          (*found_count)++;
...
void computeInteractionWithBoundaryParticles(int id, float r0, global float2* neighborMap, global unsigned int* particleIndexBack, global uint2* particleIndex, global float4* position, global float4* velocity, float4* pos_, bool tangVel, float4* vel, unsigned int PARTICLE_COUNT) {
  ...
  do {
    if ((id_b = neighborMap[hook(10, idx + nc)].x) != -1) {
      id_b_source_particle = particleIndex[hook(2, id_b)].y;
...
kernel void computeInteractionWithMembranes_finalize(global float4* position, global float4* velocity, global uint2* particleIndex, global unsigned int* particleIndexBack, int PARTICLE_COUNT) {
  int id = get_global_id(0);
  if (id >= PARTICLE_COUNT)
    return;

  id = particleIndexBack[hook(3, id)];

  int id_source_particle = particleIndex[hook(2, id)].y;
  ...
  position[hook(0, id_source_particle)] += position[hook(0, PARTICLE_COUNT + id_source_particle)];
}