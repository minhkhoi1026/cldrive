...
kernel void dilate(global const float* src, global float* dst, constant unsigned int* structure, int structure_size, int width, int height) {
  ...
  if (g_idx < width * height) {
    ...
    for (int i = -(structure_size - 1) / 2; i <= (structure_size - 1) / 2; i++) {
      for (int j = -(structure_size - 1) / 2; j <= (structure_size - 1) / 2; j++) {
        ...
        val_r = min(val_r, src[hook(0, el_idx * 3)]);
        val_g = min(val_g, src[hook(0, el_idx * 3 + 1)]);
        val_b = min(val_b, src[hook(0, el_idx * 3 + 2)]);
      }
    }
