//{"Gauss76Wt":14,"Gauss76Z":13,"Ncap_radius":7,"Nlength":8,"Nq":2,"Nradius":6,"background":10,"cutoff":5,"loops":4,"loops_g":3,"q":0,"result":1,"scale":9,"sld":11,"solvent_sld":12}
...
kernel void capped_cylinder_Iq(global const float* q, global float* result, const int Nq,

                               global float* loops_g,

                               local float* loops, const float cutoff, const int Nradius, const int Ncap_radius, const int Nlength,

                               const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nradius + Ncap_radius + Nlength) * 2, 0);
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int cap_radius_i = 0; cap_radius_i < Ncap_radius; cap_radius_i++) {
        const float cap_radius = loops[hook(4, 2 * (cap_radius_i + Nradius))];
        const float cap_radius_w = loops[hook(4, 2 * (cap_radius_i + Nradius) + 1)];
        for (int length_i = 0; length_i < Nlength; length_i++) {
          const float length = loops[hook(4, 2 * (length_i + Nradius + Ncap_radius))];
          const float length_w = loops[hook(4, 2 * (length_i + Nradius + Ncap_radius) + 1)];

          const float weight = radius_w * cap_radius_w * length_w;
          if (weight > cutoff) {
            const float scattering = Iq(qi, sld, solvent_sld, radius, cap_radius, length);
