//{"Gauss76Wt":13,"Gauss76Z":12,"Nlength":7,"Nq":2,"Nradius":6,"background":9,"cutoff":5,"loops":4,"loops_g":3,"q":0,"result":1,"scale":8,"sld":10,"solvent_sld":11}
...
float Iq(float q, float sld, float solvent_sld, float radius, float length) {
  ...
  for (int i = 0; i < 76; i++) {
    const float alpha = 0x1.921fb54442d18p-1 * (Gauss76Z[hook(12, i)] + 1.0f);
    float sn, cn;
    do {
    ...
    total += Gauss76Wt[hook(13, i)] * fq * fq * sn;
  }
...
kernel void cylinder_Iq(global const float* q, global float* result, const int Nq,

                        global float* loops_g,

                        local float* loops, const float cutoff, const int Nradius, const int Nlength,

                        const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nradius + Nlength) * 2, 0);
...

  {
    ...
    for (int radius_i = 0; radius_i < Nradius; radius_i++) {
      ...
      for (int length_i = 0; length_i < Nlength; length_i++) {
        const float length = loops[hook(4, 2 * (length_i + Nradius))];
        const float length_w = loops[hook(4, 2 * (length_i + Nradius) + 1)];

        const float weight = radius_w * length_w;
        if (weight > cutoff) {
          const float scattering = Iq(qi, sld, solvent_sld, radius, length);
