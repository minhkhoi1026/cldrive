//{"losers":3,"prime":4,"sequence":0,"table":1,"valid":2,"xor":5}
...
kernel void ExtractLosers(global int* sequence, global int* table, global int* valid, global int* losers, const int prime, const int xor) {
  const int id = get_global_id(0);
  const int value = sequence[hook(0, id)];
  const unsigned int hash = DuplicateHash(value, xor, prime);
  if (table[hook(1, hash)] != value) {
    valid[hook(2, id)] = 1;
    losers[hook(3, id)] = value;
  } else {
    valid[hook(2, id)] = 0;
    losers[hook(3, id)] = -1;
  }
