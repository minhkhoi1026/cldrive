...
inline double lookup(double t, global float* sinc_table, float ratio) {
  t = fabs(t);
  t *= 512;
  int index = (int)(t);
  if (index >= 65537 || index + 1 >= 65537)
    return 0.0;
...
kernel void interpolation(global float* buffer_process, int buf_size, global float* out_buf, int out_size, int beg_index, int out_samples, int chn_num, int ori_sr, int des_sr, int w_hn, global float* sinc_table_) {
  global float* sinc_table = (global float*)sinc_table_;
...

  {
    ...
    for (int i = 0; i < w_hn; i++) {
      int nId = a_nR + i;
      float vL = (nId >= 0 && nId <= 2048 - 1) ? buffer_process[hook(0, nId * chn_num)] : 0;
      int fabs_Index = nR + i;
      int index_f = fabs_Index;
      nIVL += (double)((double)vL * lookup(ti - (double)index_f, sinc_table, ratio));
    }
    for (int i = 0; i < w_hn; i++) {
      int nId = a_nL - i;
      float vL = (nId >= 0 && nId <= 2048 - 1) ? buffer_process[hook(0, nId * chn_num)] : 0;
  ...
  }
  {
    ...
    for (int i = 0; i < w_hn; i++) {
      int nId = a_nR + i;
      float vL = (nId >= 0 && nId <= 2048 - 1) ? buffer_process[hook(0, nId * chn_num + 1)] : 0;
      int fabs_Index = nR + i;
      int index_f = fabs_Index;
      nIVL += (double)((double)vL * lookup(ti - (double)index_f, sinc_table, ratio));
    }
