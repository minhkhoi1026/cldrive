//{"colid":1,"float3":4,"over":5,"randlist":2,"rowall":7,"rowallsize":8,"rownum":6,"rowptr":0,"vplist":3}
int hook(int argId, int id) {
	int gID = get_global_id(0);
	printf("%d,%d,%d\n", gID, argId, id);
	return id;
}
kernel void cpu_csr(global int* rowptr, global int* colid, global int* randlist, global int* vplist, int float3, global int* over, int rownum, global int* rowall, int rowallsize) {
  int t = get_global_id(0);

  int getnumsum = get_global_size(0);

  for (; t < rowallsize; t += getnumsum) {
    int tid = rowall[hook(7, t)];
