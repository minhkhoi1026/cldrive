//{"iso":2,"vertexKeys":1,"vertexUnique":0}
...
kernel void countUniqueVertices(global unsigned int* restrict vertexUnique, global const ulong* restrict vertexKeys) {
  const unsigned int gid = get_global_id(0);
  const ulong key = vertexKeys[hook(1, gid)];
  const ulong nextKey = vertexKeys[hook(1, gid + 1)];
  bool last = key != nextKey;
  vertexUnique[hook(0, gid)] = last ? 1 : 0;
}