...
kernel void gpu_sbell14_tx(global int* slice_ptr, global int* col_id, global float4* data, read_only image2d_t vec, global float* result, int slice_num) {
  ...
  for (int i = start; i < end; i += 32) {
    ...
    coord.x = vecid & 1023;
    coord.y = vecid >> 10;
    float4 matrixelem = data[hook(2, i)];
