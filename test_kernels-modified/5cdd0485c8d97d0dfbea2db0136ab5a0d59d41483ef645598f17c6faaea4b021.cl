...
kernel void gpu_coo_s1_tx(global int* row_id, global int* col_id, global float* data, int process_size, int nnz, read_only image2d_t vec, global float* result, global int* tmp_rowid, global float* tmp_data) {
  ...
  for (int iter = 0; iter < iter_num; iter++) {
    ...
    if (i < nnz) {
      ...
      int col = col_id[hook(1, i)];
      int2 coord;
      coord.x = col & 1023;
      coord.y = col >> 10;
      float4 vecelem = read_imagef(vec, smp, coord);
      val = data[hook(2, i)] * vecelem.x;
    }
    if (localid == 0) {
