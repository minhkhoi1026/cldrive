...
kernel void nw1(global int* similarity_d, global int* input_itemsets_d, int cols, int penalty, int blk, int block_width, int offset_r, int offset_c, local int* input_itemsets_l, local int* similarity_l) {
  ...
  for (int m = 0; m < 32; ++m) {
    if (tx <= m) {
      ...
      input_itemsets_l[hook(8, (t_index_x) + (t_index_y) * (32 + 1))] = max3(input_itemsets_l[hook(8, (t_index_x - 1) + (t_index_y - 1) * (32 + 1))] + similarity_l[hook(9, (t_index_y - 1) * 32 + (t_index_x - 1))], input_itemsets_l[hook(8, (t_index_x - 1) + (t_index_y) * (32 + 1))] - penalty, input_itemsets_l[hook(8, (t_index_x) + (t_index_y - 1) * (32 + 1))] - penalty);
    }
    barrier(0x01);
  }
  barrier(0x01);
