//{"Gauss76Wt":18,"Gauss76Z":17,"Na_side":7,"Nb_side":8,"Nc_side":9,"Nphi":11,"Npsi":12,"Nq":3,"Ntheta":10,"background":14,"cutoff":6,"loops":5,"loops_g":4,"qx":0,"qy":1,"result":2,"scale":13,"sld":15,"solvent_sld":16}
...
float Iq(float q, float sld, float solvent_sld, float a_side, float b_side, float c_side) {
  ...
  for (int i = 0; i < 76; i++) {
    float summj = 0.0f;
    float sigma = 0.5f * (Gauss76Z[hook(17, i)] + 1.0f);

    for (int j = 0; j < 76; j++) {
      float uu = 0.5f * (Gauss76Z[hook(17, j)] + 1.0f);
      ...
      yyy = Gauss76Wt[hook(18, j)] * tmp1 * tmp2;
    ...
    yyy = Gauss76Wt[hook(18, i)] * answer;
    summ += yyy;
  }
...
kernel void parallelepiped_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,

                                global float* loops_g,

                                local float* loops, const float cutoff, const int Na_side, const int Nb_side, const int Nc_side, const int Ntheta, const int Nphi, const int Npsi,

                                const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Na_side + Nb_side + Nc_side + Ntheta + Nphi + Npsi) * 2, 0);
...

  {
    ...
    for (int a_side_i = 0; a_side_i < Na_side; a_side_i++) {
      const float a_side = loops[hook(5, 2 * (a_side_i))];
      const float a_side_w = loops[hook(5, 2 * (a_side_i) + 1)];
      for (int b_side_i = 0; b_side_i < Nb_side; b_side_i++) {
        const float b_side = loops[hook(5, 2 * (b_side_i + Na_side))];
        const float b_side_w = loops[hook(5, 2 * (b_side_i + Na_side) + 1)];
        for (int c_side_i = 0; c_side_i < Nc_side; c_side_i++) {
          const float c_side = loops[hook(5, 2 * (c_side_i + Na_side + Nb_side))];
          const float c_side_w = loops[hook(5, 2 * (c_side_i + Na_side + Nb_side) + 1)];
          for (int theta_i = 0; theta_i < Ntheta; theta_i++) {
            const float theta = loops[hook(5, 2 * (theta_i + Na_side + Nb_side + Nc_side))];
            const float theta_w = loops[hook(5, 2 * (theta_i + Na_side + Nb_side + Nc_side) + 1)];
            for (int phi_i = 0; phi_i < Nphi; phi_i++) {
              const float phi = loops[hook(5, 2 * (phi_i + Na_side + Nb_side + Nc_side + Ntheta))];
              const float phi_w = loops[hook(5, 2 * (phi_i + Na_side + Nb_side + Nc_side + Ntheta) + 1)];
              for (int psi_i = 0; psi_i < Npsi; psi_i++) {
                const float psi = loops[hook(5, 2 * (psi_i + Na_side + Nb_side + Nc_side + Ntheta + Nphi))];
                const float psi_w = loops[hook(5, 2 * (psi_i + Na_side + Nb_side + Nc_side + Ntheta + Nphi) + 1)];

                const float weight = a_side_w * b_side_w * c_side_w * theta_w * phi_w * psi_w;
                if (weight > cutoff) {
                  const float scattering = Iqxy(qxi, qyi, sld, solvent_sld, a_side, b_side, c_side, theta, phi, psi);
                  if (!isnan(scattering)) {
