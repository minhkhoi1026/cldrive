//{"learning_rate":2,"params":0,"params_grad":1,"weight_decay":3}
...
kernel void update_parameters_by_stochastic_gradient_descent(global float* params, global float* params_grad, float learning_rate, float weight_decay) {
  const int GID = get_global_id(0);
  params[hook(0, GID)] -= learning_rate * (params_grad[hook(1, GID)] + weight_decay * params[hook(0, GID)]);
