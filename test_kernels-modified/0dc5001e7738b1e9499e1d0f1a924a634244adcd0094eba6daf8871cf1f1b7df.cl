//{"MUSCLE_COUNT":12,"PARTICLE_COUNT":11,"acceleration":3,"closest_distances":18,"closest_indexes":19,"d_array":16,"elasticConnectionsData":10,"elasticityCoefficient":15,"gridCellIndex":17,"mass":7,"max_muscle_force":6,"muscle_activation_signal":13,"neighborMap":0,"numOfElasticP":9,"particleIndex":5,"particleIndexBack":4,"position":14,"simulationScale":8,"sortedPosition":1,"sortedVelocity":2}
...
kernel void pcisph_computeElasticForces(global float2* neighborMap, global float4* sortedPosition, global float4* sortedVelocity, global float4* acceleration, global unsigned int* particleIndexBack, global uint2* particleIndex, float max_muscle_force, float mass, float simulationScale, unsigned int numOfElasticP, global float4* elasticConnectionsData, unsigned int PARTICLE_COUNT, unsigned int MUSCLE_COUNT, global float* muscle_activation_signal, global float4* position, float elasticityCoefficient) {
    ...
    if ((jd = (int)elasticConnectionsData[hook(10, idx + nc)].x) != -1) {
      ...
      if (r_ij != 0.f) {
        if (((position[hook(14, id_sp)].w > 2.05f) && (position[hook(14, id_sp)].w < 2.25f)) && ((position[hook(14, jd_sp)].w > 2.05f) && (position[hook(14, jd_sp)].w < 2.25f))) {
          ...
          acceleration[hook(3, id)] += -(vect_r_ij / r_ij) * delta_r_ij * elasticityCoefficient * 0.25f;
        }

        for (i = 0; i < MUSCLE_COUNT; i++) {
          if ((int)(elasticConnectionsData[hook(10, idx + nc)].z) == (i + 1)) {
            if (muscle_activation_signal[hook(13, i)] > 0.f)
              acceleration[hook(3, id)] += -(vect_r_ij / r_ij) * muscle_activation_signal[hook(13, i)] * max_muscle_force;
          }
        }
      }
    } else
