...
kernel void processNineCellWindow(global float* scanLine1, global float* scanLine2, global float* scanLine3, global uchar4* resultLine, global float* rasterParams) {
  ...
  if (x22 == rasterParams[hook(4, 0)]) {
    ...
    float derX = ((x13 + x23 + x23 + x33) - (x11 + x21 + x21 + x31)) / (8 * rasterParams[hook(4, 3)]);
    float derY = ((x31 + x32 + x32 + x33) - (x11 + x12 + x12 + x13)) / (8 * -rasterParams[hook(4, 4)]);
    ...
    if (derX == rasterParams[hook(4, 0)] || derX == rasterParams[hook(4, 0)]) {
      ...
      if (rasterParams[hook(4, 17)]) {
        ...
        if (xx_plus_yy == 0.0f) {
          ...
          const float cang_mul_127 = ((weight_225 * val225_mul_127 + weight_270 * val270_mul_127 + weight_315 * val315_mul_127 + weight_360 * val360_mul_127) / xx_plus_yy) / (1 + rasterParams[hook(4, 8)] * xx_plus_yy);
          res = clamp(1.0f + cang_mul_127, 0.0f, 255.0f);
        }
      } else {
        res = (rasterParams[hook(4, 9)] - (derY * rasterParams[hook(4, 6)] - derX * rasterParams[hook(4, 7)])) / sqrt(1 + rasterParams[hook(4, 8)] * (derX * derX + derY * derY));
