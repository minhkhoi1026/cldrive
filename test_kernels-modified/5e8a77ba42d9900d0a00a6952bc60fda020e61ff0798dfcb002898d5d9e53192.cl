...
kernel void evalLJC(int nAtoms, global float4* atoms, global float2* cLJs, global float4* poss, global float8* FE) {
  ...
  for (int i0 = 0; i0 < nAtoms; i0 += nL) {
    ...
    for (int j = 0; j < nL; j++) {
      if ((j + i0) < nAtoms)
        fe += getLJC(LATOMS[hook(5, j)], LCLJS[hook(6, j)], pos);
    }
    barrier(0x01);
  }

  fe.hi = fe.hi * 14.399644f;
  FE[hook(4, iG)] = fe;
}