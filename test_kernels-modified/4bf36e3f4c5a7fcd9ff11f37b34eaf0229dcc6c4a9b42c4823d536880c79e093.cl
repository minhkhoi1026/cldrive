...
kernel __attribute__((reqd_work_group_size(1024 / 2, 1, 1))) void bitonicSortLocal(global unsigned int* d_DstKey, global unsigned int* d_DstVal, global unsigned int* d_SrcKey, global unsigned int* d_SrcVal, unsigned int arrayLength, unsigned int sortDir) {
  local unsigned int l_key[1024];
  local unsigned int l_val[1024];

  d_SrcKey += get_group_id(0) * 1024 + get_local_id(0);
  d_SrcVal += get_group_id(0) * 1024 + get_local_id(0);
  d_DstKey += get_group_id(0) * 1024 + get_local_id(0);
  d_DstVal += get_group_id(0) * 1024 + get_local_id(0);
  l_key[hook(8, get_local_id(0) + 0)] = d_SrcKey[hook(2, 0)];
  l_val[hook(9, get_local_id(0) + 0)] = d_SrcVal[hook(3, 0)];
  l_key[hook(8, get_local_id(0) + (1024 / 2))] = d_SrcKey[hook(2, (1024 / 2))];
  l_val[hook(9, get_local_id(0) + (1024 / 2))] = d_SrcVal[hook(3, (1024 / 2))];
  ...
  d_DstKey[hook(0, (1024 / 2))] = l_key[hook(8, get_local_id(0) + (1024 / 2))];
  d_DstVal[hook(1, (1024 / 2))] = l_val[hook(9, get_local_id(0) + (1024 / 2))];
}
kernel __attribute__((reqd_work_group_size(1024 / 2, 1, 1))) void bitonicSortLocal1(global unsigned int* d_DstKey, global unsigned int* d_DstVal, global unsigned int* d_SrcKey, global unsigned int* d_SrcVal) {
  local unsigned int l_key[1024];
  local unsigned int l_val[1024];
