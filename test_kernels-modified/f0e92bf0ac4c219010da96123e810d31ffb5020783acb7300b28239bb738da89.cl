...
kernel void trans_lu_backward(global const unsigned int* row_indices, global const unsigned int* column_indices, global const float* elements, global const float* diagonal_entries, global float* vector, unsigned int size) {
  ...
  for (unsigned int i2 = get_local_id(0); i2 < loop_end; i2 += get_local_size(0)) {
    ...
    row_index_lookahead[hook(6, get_local_id(0))] = (row_at_window_start >= get_local_id(0)) ? row_indices[hook(0, row_at_window_start - get_local_id(0))] : 0;

    barrier(0x01);

    if (i2 < nnz) {
      unsigned int row_index_dec = 0;
      while (row_index_lookahead[hook(6, row_index_dec)] > i)
        ++row_index_dec;
      row_index = row_at_window_start - row_index_dec;
      row_index_buffer[hook(7, get_local_id(0))] = row_index;
    } else {
      row_index = size + 1;
      row_index_buffer[hook(7, get_local_id(0))] = 0;
    }
