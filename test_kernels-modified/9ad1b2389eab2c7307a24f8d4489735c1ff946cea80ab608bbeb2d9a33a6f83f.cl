//{"Gauss76Wt":14,"Gauss76Z":13,"Nq":2,"Nreq_major":7,"Nreq_minor":6,"Nrpolar":8,"background":10,"cutoff":5,"loops":4,"loops_g":3,"q":0,"result":1,"scale":9,"sld":11,"solvent_sld":12}
...
float Iq(float q, float sld, float solvent_sld, float req_minor, float req_major, float rpolar) {
  ...
  float outer = 0.0f;
  for (int i = 0; i < 76; i++) {
    const float x = 0.5f * (Gauss76Z[hook(13, i)] + 1.0f);
    do {
    ...
    float inner = 0.0f;
    for (int j = 0; j < 76; j++) {
      const float y = 0.5f * (Gauss76Z[hook(13, j)] + 1.0f);
      const float t = q * sqrt(acosx2 + bsinx2 * (1.0f - y * y) + c2 * y * y);
      const float fq = sph_j1c(t);
      inner += Gauss76Wt[hook(14, j)] * fq * fq;
    }
    outer += Gauss76Wt[hook(14, i)] * 0.5f * inner;
  }
...
kernel void triaxial_ellipsoid_Iq(global const float* q, global float* result, const int Nq,

                                  global float* loops_g,

                                  local float* loops, const float cutoff, const int Nreq_minor, const int Nreq_major, const int Nrpolar,

                                  const float scale, const float background, const float sld, const float solvent_sld) {
  event_t e = async_work_group_copy(loops, loops_g, (Nreq_minor + Nreq_major + Nrpolar) * 2, 0);
...

  {
    ...
    for (int req_minor_i = 0; req_minor_i < Nreq_minor; req_minor_i++) {
      const float req_minor = loops[hook(4, 2 * (req_minor_i))];
      const float req_minor_w = loops[hook(4, 2 * (req_minor_i) + 1)];
      for (int req_major_i = 0; req_major_i < Nreq_major; req_major_i++) {
        const float req_major = loops[hook(4, 2 * (req_major_i + Nreq_minor))];
        const float req_major_w = loops[hook(4, 2 * (req_major_i + Nreq_minor) + 1)];
        for (int rpolar_i = 0; rpolar_i < Nrpolar; rpolar_i++) {
          const float rpolar = loops[hook(4, 2 * (rpolar_i + Nreq_minor + Nreq_major))];
          const float rpolar_w = loops[hook(4, 2 * (rpolar_i + Nreq_minor + Nreq_major) + 1)];

          const float weight = req_minor_w * req_major_w * rpolar_w;
          if (weight > cutoff) {
            const float scattering = Iq(qi, sld, solvent_sld, req_minor, req_major, rpolar);
