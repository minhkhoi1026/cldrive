...
kernel void minimum(global const float* grid, global float* result, int const num_elements, local float* cache) {
  ...
  cache[hook(3, local_id)] = (global_id < num_elements) ? grid[hook(0, global_id)] : 0x1.fffffep127f;
  barrier(0x01);

  for (unsigned int s = local_size >> 1; s > 0; s >>= 1) {
    if (local_id < s) {
      cache[hook(3, local_id)] = fmin(cache[hook(3, local_id)], cache[hook(3, local_id + s)]);
    }
    barrier(0x01);
  }
