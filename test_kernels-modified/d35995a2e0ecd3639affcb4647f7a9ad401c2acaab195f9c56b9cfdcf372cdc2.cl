//{"Neffect_radius":7,"Nq":3,"background":9,"cutoff":6,"loops":5,"loops_g":4,"perturb":11,"qx":0,"qy":1,"result":2,"scale":8,"stickiness":12,"volfraction":10}
...
kernel void stickyhardsphere_Iqxy(global const float* qx, global const float* qy, global float* result, const int Nq,
...

  {
    ...
    for (int effect_radius_i = 0; effect_radius_i < Neffect_radius; effect_radius_i++) {
      ...
      if (weight > cutoff) {
        const float scattering = Iqxy(qxi, qyi, effect_radius, volfraction, perturb, stickiness);
        if (!isnan(scattering)) {
