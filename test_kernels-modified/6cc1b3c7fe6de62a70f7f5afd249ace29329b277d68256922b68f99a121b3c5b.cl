//{"Ps":4,"bounds":3,"force":2,"nx":0,"pos":1}
...
kernel void force_Tiled(unsigned int nx, global float2* pos, global float2* force, global int2* bounds) {
  local float2 Ps[256];
  ...
  int njs = 0;
  {
    ...
    for (int i0 = 0; i0 < bound.y; i0 += nL) {
      ...
      if (i >= bound.y)
        break;
      Ps[hook(4, njs + i)] = pos[hook(1, bound.x + i)];
    }
    njs += bound.y;
  };
  {
  ...
  for (int i0 = 0; i0 < bound0.y; i0 += nL) {
    ...
    if (i >= bound0.y)
      break;
    float2 pi = Ps[hook(4, i)];
    float2 f = (float2)(0.0f, 0.0f);
    for (int j = 0; j < njs; j++) {
      f += pair_force(pi, Ps[hook(4, j)]);
    }
