...
__attribute__((reqd_work_group_size(64, 1, 1))) kernel void correlate(global float8* correlation, const int corr_size, constant float4* base, constant float4* mask, const int sample_size, const int stride) {
  float4 l_base[((1 * 8) + 8)], l_mask[((1 * 8) + 8)];
  float4 sums[(1 * 8)];
  for (int i = 0; i < (1 * 8); i++)
    sums[hook(6, i)] = 0;
  int gid = get_global_id(0);
  ...
  for (int y = 0; y < sample_size - offset_y; y++) {
    ...
    for (int i = 0; i < ((1 * 8) + 8); i++) {
      l_base[hook(7, i)] = base[hook(2, base_idx + i)];
      l_mask[hook(8, i)] = mask[hook(3, mask_idx + i)];
    }
