                  ...
                  uint4 *nextRand)
{
    ...
    for(i = 0; i < mulFactor; ++i)
    {
        ...
        temp[hook(6, i)].x = a.x ^ e.x ^ ((b.x >> thirteen) & mask11) ^ f.x ^ (r2.x << fifteen);
        temp[hook(6, i)].y = a.y ^ e.y ^ ((b.y >> thirteen) & mask12) ^ f.y ^ (r2.y << fifteen);
        temp[hook(6, i)].z = a.z ^ e.z ^ ((b.z >> thirteen) & mask13) ^ f.z ^ (r2.z << fifteen);
        temp[hook(6, i)].w = a.w ^ e.w ^ ((b.w >> thirteen) & mask14) ^ f.w ^ (r2.w << fifteen);
    }        

    temp1 = convert_double4(temp[hook(6, 0)]) * one / intMax;
    temp2 = convert_double4(temp[hook(6, 1)]) * one / intMax;
			...
			__global double4 *pathDeriv)
{
		...
		uint4 nextRand = randArray[hook(3, yPos * width + xPos)];
		...
		priceSamples[hook(4, (yPos * width + xPos) * 2)] = price1;
		priceSamples[hook(4, (yPos * width + xPos) * 2 + 1)] = price2;
		pathDeriv[hook(5, (yPos * width + xPos) * 2)] = pathDeriv1; 
		pathDeriv[hook(5, (yPos * width + xPos) * 2 + 1)] = pathDeriv2;				
