    ...
    uint4 *nextRand)
{
    ...
    for(i = 0; i < mulFactor; ++i)
    {
        ...
        temp[hook(8, i)].x = a.x ^ e.x ^ ((b.x >> thirteen) & mask[hook(9, 0)]) ^ f.x ^ (r2.x << fifteen);
        temp[hook(8, i)].y = a.y ^ e.y ^ ((b.y >> thirteen) & mask[hook(9, 1)]) ^ f.y ^ (r2.y << fifteen);
        temp[hook(8, i)].z = a.z ^ e.z ^ ((b.z >> thirteen) & mask[hook(9, 2)]) ^ f.z ^ (r2.z << fifteen);
        temp[hook(8, i)].w = a.w ^ e.w ^ ((b.w >> thirteen) & mask[hook(9, 3)]) ^ f.w ^ (r2.w << fifteen);
    }        

    temp1 = convert_float4(temp[hook(8, 0)]) * one / intMax;
    temp2 = convert_float4(temp[hook(8, 1)]) * one / intMax;
    ...
    __local float8 *sData1)
{
    ...
    uint4 nextRand = randArray[hook(2, gidx)];
    ...
    calOutputs_Vector(strikePrice, meanDeriv1, meanDeriv2, meanPrice1,
     ...
     sData1[hook(5, lidx)]=(float8)(price1+price2,pathDeriv1+pathDeriv2);
    ...
    for(unsigned int s = (groupDim.x*groupDim.y)>> 1; s > 0; s >>= 1) 
    {
        if(lidx < s) 
        {
            sData1[hook(5, lidx)] += sData1[hook(5, lidx + s)];
        }
          barrier(CLK_LOCAL_MEM_FENCE);	
    }
    ...
    priceSamples[hook(3, bidx)] = sData1[hook(5, 0)].lo;
    pathDeriv[hook(4, bidx)] = sData1[hook(5, 0)].hi;
    ...
    uint lidx)
{
    ...
    state[hook(10, 0)] = stateMask * (seed	 ^ (seed	 >> thirty)) + 1u;
    state[hook(10, 1)] = stateMask * (state[hook(10, 0)] ^ (state[hook(10, 0)] >> thirty)) + 2u;
    state[hook(10, 2)] = stateMask * (state[hook(10, 1)] ^ (state[hook(10, 1)] >> thirty)) + 3u;
    state[hook(10, 3)] = stateMask * (state[hook(10, 2)] ^ (state[hook(10, 2)] >> thirty)) + 4u;
    ...
    for(uint i = 0; i < mulFactor; ++i)
    {
        ...
        randPrice[hook(6, lidx)] = ((i==0) ? state[hook(10, 2)] : r2);
        r2 = ((i==0) ? state[hook(10, 3)] : temp[hook(8, i - 1)]);
        randVega[hook(7, lidx)] = ((i==0) ? seed : state[hook(10, i - 1)]);
        b = ((i==3) ? seed : state[hook(10, i + 1)]);

        barrier(CLK_LOCAL_MEM_FENCE);	

        
        uint e = randVega[hook(7, lidx)] << threeBytes;
        if (lidx % 4) {
            e |= (randVega[hook(7, lidx - 1)] >> (32u - threeBytes));
        }

        
        uint f = randPrice[hook(6, lidx)] >> threeBytes;
        if ((lidx+1) % 4) {
            f |= (randPrice[hook(6, lidx + 1)] << (32u - threeBytes));
        }

        temp[hook(8, i)] = randVega[hook(7, lidx)] ^ e ^ ((b >> thirteen) & mask[hook(9, lidx % 4)]) ^ f ^ (r2 << fifteen);
    }        

    
    float r = sqrt((-two) * log(temp[hook(8, 0)] * one / intMax));
    float phi  = two * PI * temp[hook(8, 1)] * one / intMax;
    *gaussianRand1 = r * cos(phi);
    *gaussianRand2 = r * sin(phi);
    *nextRand = temp[hook(8, 2)];
}
        ...
        __local uint *randVega)
{
    ...
    uint nextRand = randArray[hook(2, gidx)];
    ...
    sData1[hook(5, lidx)] = (float2)(price1+price2, pathDeriv1+pathDeriv2);
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (lidx < 4){
        priceSamples[hook(3, bidx * 4 + lidx)] = sData1[hook(5, lidx)].lo;
        pathDeriv[hook(4, bidx * 4 + lidx)] = sData1[hook(5, lidx)].hi;
    }
}
