//{"layerSize":0,"normFacts":1,"outputs":2}
...
kernel void sml_normalizeOutputsFn(int layerSize, global float* normFacts, global float* outputs) {
  int outputIdx = get_global_id(0);

  int patIdx = outputIdx / layerSize;

  float normFact = normFacts[hook(1, patIdx)];
  if (normFact != 3.4028235e+038f) {
    float x = outputs[hook(2, outputIdx)] / normFact;

    outputs[hook(2, outputIdx)] = x;
  }
}