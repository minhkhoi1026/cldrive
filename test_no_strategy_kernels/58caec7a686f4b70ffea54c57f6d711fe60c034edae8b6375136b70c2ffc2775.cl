...
kernel void lu_backward(global const unsigned int* row_indices, global const unsigned int* column_indices, global const float* elements, local int* buffer, local float* vec_entries, global float* vector, unsigned int size) {
  int waiting_for;
  ...
  for (int block_num = size / get_global_size(0); block_num > -1; --block_num) {
    ...
    if (row < size) {
      ...
      diagonal_entry = column_indices[hook(1, waiting_for_index)];
    }
      ...
      if (row < size) {
        ...
        if (waiting_for == -1) {
          for (unsigned int j = waiting_for_index; j < row_index_end; ++j) {
            ...
            barrier(0x01);
            if (col >= block_offset + get_global_size(0))
              ...
              diagonal_entry = elements[hook(2, j)];
          }

          if (waiting_for == -1) {
            if (row == 0)
              vec_entries[hook(4, get_global_id(0))] /= elements[hook(2, 0)];
            else
              vec_entries[hook(4, get_global_id(0))] /= diagonal_entry;
