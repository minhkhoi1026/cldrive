//{"divisionSize":3,"input1":0,"input2":1,"inputSize":4,"mean1":5,"mean2":6,"output":2}
...
kernel void covarianceKernel(global float* input1, global float* input2, global float* output, unsigned int const divisionSize, unsigned int const inputSize, float const mean1, float const mean2) {
  unsigned int n = get_global_id(0);
  float outputInternal = 0;
  if ((n + 1) * divisionSize <= inputSize) {
    for (unsigned int i = 0; i < divisionSize; i++) {
      outputInternal += (input1[hook(0, n * divisionSize + i)] - mean1) * (input2[hook(1, n * divisionSize + i)] - mean2);
    }
  } else {
  ...
  output[hook(2, n)] = outputInternal;
}