...
kernel void decode_bbox_corner_no_variance_kernel(const global float* loc_data, const global float* prior_data, const int num_priors, const int share_location, const int num_loc_classes, const int background_label_id, global float* bbox_data) {
  ...
  bbox_data[hook(6, idx)] = prior_data[hook(1, idx_p)] + loc_data[hook(0, idx)] * variance[hook(7, idx_p)];
  bbox_data[hook(6, idx + 1)] = prior_data[hook(1, idx_p + 1)] + loc_data[hook(0, idx + 1)] * variance[hook(7, idx_p + 1)];
  bbox_data[hook(6, idx + 2)] = prior_data[hook(1, idx_p + 2)] + loc_data[hook(0, idx + 2)] * variance[hook(7, idx_p + 2)];
  bbox_data[hook(6, idx + 3)] = prior_data[hook(1, idx_p + 3)] + loc_data[hook(0, idx + 3)] * variance[hook(7, idx_p + 3)];
}