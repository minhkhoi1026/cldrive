...
kernel void bitonicMergeGlobal(global float* d_DstKey, global unsigned int* d_DstVal, global float* d_SrcKey, global unsigned int* d_SrcVal, unsigned int arrayLength, unsigned int size, unsigned int stride, unsigned int dir) {
  ...
  float keyA = d_SrcKey[hook(2, pos + 0)];
  unsigned int valA = d_SrcVal[hook(3, pos + 0)];
  float keyB = d_SrcKey[hook(2, pos + stride)];
